...
## Executor Recovery Plan
Recovery plans for 8 failure types generated by vibe_executor_recovery.py:
- model_error: switch model or abort
- timeout: cancel and log
- dirty_worktree: inspect and decide
- gate_blocked/wrapper_blocked: abort and log
- test_failed: fix or rollback

## Executor Unfreeze Plan
Graduated unfreeze: Level 0 (current) → Level 1 (fixture write) → Level 2 (fixture push) → Level 3 (low-risk PR) → Level 4 (broader execution).
Each level requires explicit human approval and all entry conditions GREEN.


## V1 Freeze Integration

Before any autonomous operation, verify V1 freeze is intact:


If verdict is BLOCK, stop all operations and investigate.
