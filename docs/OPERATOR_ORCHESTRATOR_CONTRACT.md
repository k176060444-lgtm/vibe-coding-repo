...
|---|---|---|---|---|---|---|---|---|---|
| `GLOBAL_READINESS_RECEIPT` | vibedev/orchestrator | Work Order approval | scope packet triple | scope packet, Work Order | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT) | COMMAND_RESULT, FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | PASS / FAIL | GLOBAL_READINESS_PASS / GLOBAL_READINESS_FAIL |
| `ROLE_ACTIVATION_READINESS_RECEIPT` | vibedev/orchestrator (FULL/LIGHTWEIGHT only — not Consultation) | Global Readiness + assignment approval | activation packet triple | activation packet | `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`, `GLOBAL_READINESS_RECEIPT` | AND | READINESS_CHECK, NODE_ROUTE_PROBE, MODEL_INVOCATION_RECORD | READY / BLOCKED | ROLE_ACTIVATION_READY / ROLE_ACTIVATION_BLOCKED |
| `ROLE_CROSS_VERIFICATION_RECEIPT` | vibedev/orchestrator | role completion report Artifact | ROLE_COMPLETION_REPORT triple | role completion Artifact | `GLOBAL_READINESS_RECEIPT` | AND | GIT_OBJECT, TEST_RESULT, MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | VERIFIED / INCOMPLETE / REJECTED | ROLE_CROSS_VERIFIED / ROLE_CROSS_VERIFICATION_INCOMPLETE / ROLE_CROSS_VERIFICATION_REJECTED |
| `EXPLORER_VALIDATION_RECEIPT` | vibedev/orchestrator | explorer Artifact | EXPLORER_ARTIFACT triple | explorer Artifact | `ROLE_CROSS_VERIFICATION_RECEIPT` | AND | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS / FAIL | EXPLORER_VALIDATED / EXPLORER_VALIDATION_FAILED |
| `PLAN_VALIDATION_RECEIPT` | vibedev/orchestrator | implementation plan Artifact | IMPLEMENTATION_PLAN triple | implementation plan | `EXPLORER_VALIDATION_RECEIPT` | AND | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS / FAIL | PLAN_VALIDATED / PLAN_VALIDATION_FAILED |
| `ARTIFACT_VALIDATION_RECEIPT` | vibedev/orchestrator or validator per mode | Artifact type + mode | any Artifact triple | TRUE | TRUE | TRUE | mode-discriminated: per Artifact type and execution mode | VALID / INVALID | ARTIFACT_VALIDATED / ARTIFACT_INVALID |
| `PACKET_ASSEMBLY_RECEIPT` | assembler (vibedev/orchestrator) | assembly completion | assembled Packet triple | mode-discriminated: per Packet type | AND of all preceding validation Receipts bound to applicable Packet member object types (canonical Receipt type names only; per-mode validation) | AND | DIFF_MANIFEST, TREE_MANIFEST | ASSEMBLED | PACKET_ASSEMBLED |
| `PACKET_FREEZE_RECEIPT` | vibedev/orchestrator | Packet assembly | Packet triple | assembled Packet | `PACKET_ASSEMBLY_RECEIPT` | AND | FILE_SNAPSHOT, GIT_OBJECT | FROZEN | PACKET_FROZEN |
| `PACKET_VALIDATION_RECEIPT` | vibedev/orchestrator | Packet freeze | Packet triple | frozen Packet | `PACKET_FREEZE_RECEIPT` | AND | DIFF_MANIFEST, TREE_MANIFEST, TEST_RESULT | VALIDATED / INVALID | PACKET_VALIDATED / PACKET_VALIDATION_FAILED |
| `PACKET_CONSUMPTION_RECEIPT` | consumer role/stage controller or vibedev/orchestrator | frozen (optionally validated) Packet | Packet triple | frozen Packet | `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT` | AND | per consumer stage Evidence classes | CONSUMED | PACKET_CONSUMED |
| `TEST_GATE_PASS_RECEIPT` | vibedev/orchestrator | tester-a + tester-b cross-verification + same candidate + input Packet freeze | test evidence packet triple | `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (tester-a), `ROLE_CROSS_VERIFICATION_RECEIPT` (tester-b), `PACKET_FREEZE_RECEIPT` (TESTER_A_INPUT_PACKET), `PACKET_FREEZE_RECEIPT` (TESTER_B_INPUT_PACKET) | AND(ROLE_CROSS_VERIFICATION_RECEIPT(tester-a, VERIFIED), ROLE_CROSS_VERIFICATION_RECEIPT(tester-b, VERIFIED), PACKET_FREEZE_RECEIPT(TESTER_A_INPUT_PACKET), PACKET_FREEZE_RECEIPT(TESTER_B_INPUT_PACKET), SAME_OBJECT(candidate)) | TEST_RESULT, MODEL_INVOCATION_RECORD | PASS | GATE_PASS |
| `TEST_GATE_FAIL_RECEIPT` | vibedev/orchestrator | tester-a or tester-b failure Evidence + same candidate + input Packet freeze | test evidence packet triple | `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, candidate | `PACKET_FREEZE_RECEIPT` (TESTER_A_INPUT_PACKET), `PACKET_FREEZE_RECEIPT` (TESTER_B_INPUT_PACKET) | AND(PACKET_FREEZE_RECEIPT(TESTER_A_INPUT_PACKET), PACKET_FREEZE_RECEIPT(TESTER_B_INPUT_PACKET), SAME_OBJECT(candidate), OR(DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REJECTED))) | TEST_RESULT, MODEL_INVOCATION_RECORD | FAIL | GATE_FAIL |
| `REVIEW_GATE_PASS_RECEIPT` | vibedev/orchestrator | reviewer-a + reviewer-b cross-verification + same Review Packet/candidate + TEST_GATE_PASS_RECEIPT | review input packet triple | `REVIEW_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (reviewer-a), `ROLE_CROSS_VERIFICATION_RECEIPT` (reviewer-b), `PACKET_FREEZE_RECEIPT` (REVIEW_INPUT_PACKET), `TEST_GATE_PASS_RECEIPT` | AND(ROLE_CROSS_VERIFICATION_RECEIPT(reviewer-a, VERIFIED), ROLE_CROSS_VERIFICATION_RECEIPT(reviewer-b, VERIFIED), PACKET_FREEZE_RECEIPT(REVIEW_INPUT_PACKET), TEST_GATE_PASS_RECEIPT, SAME_OBJECT(candidate)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS | GATE_PASS |
| `REVIEW_GATE_FAIL_RECEIPT` | vibedev/orchestrator | reviewer-a or reviewer-b failure Evidence + same Review Packet/candidate + TEST_GATE_PASS_RECEIPT | review input packet triple | `REVIEW_INPUT_PACKET`, candidate | `PACKET_FREEZE_RECEIPT` (REVIEW_INPUT_PACKET), `TEST_GATE_PASS_RECEIPT` | AND(PACKET_FREEZE_RECEIPT(REVIEW_INPUT_PACKET), TEST_GATE_PASS_RECEIPT, SAME_OBJECT(candidate), OR(DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REJECTED))) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | FAIL | GATE_FAIL |
| `GIT_INTEGRATION_GATE_PASS_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_PASS_RECEIPT + Integration Packet freeze + validation + git-integrator binding | INTEGRATION_INPUT_FROZEN triple | `INTEGRATION_INPUT_FROZEN` | `REVIEW_GATE_PASS_RECEIPT`, `PACKET_FREEZE_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PACKET_VALIDATION_RECEIPT` (INTEGRATION_INPUT_FROZEN), `POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | AND(REVIEW_GATE_PASS_RECEIPT, PACKET_FREEZE_RECEIPT(INTEGRATION_INPUT_FROZEN), PACKET_VALIDATION_RECEIPT(INTEGRATION_INPUT_FROZEN), POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT) | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | PASS | GATE_PASS |
| `GIT_INTEGRATION_GATE_FAIL_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_PASS_RECEIPT + Integration Packet freeze + validation + git-integrator binding + integration failure Evidence | INTEGRATION_INPUT_FROZEN triple | `INTEGRATION_INPUT_FROZEN` | `REVIEW_GATE_PASS_RECEIPT`, `PACKET_FREEZE_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PACKET_VALIDATION_RECEIPT` (INTEGRATION_INPUT_FROZEN), `POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | AND(REVIEW_GATE_PASS_RECEIPT, PACKET_FREEZE_RECEIPT(INTEGRATION_INPUT_FROZEN), PACKET_VALIDATION_RECEIPT(INTEGRATION_INPUT_FROZEN), POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, OR(test failure Evidence, integration failure Evidence)) — Review FAIL (`REVIEW_GATE_FAIL_RECEIPT`) must NOT substitute as prerequisite; Review FAIL stage does not instantiate Git Integration | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | FAIL | GATE_FAIL |
| `DRAFT_PR_DELIVERY_RECEIPT` | git-integrator | GIT_INTEGRATION_GATE_PASS_RECEIPT | candidate + PR triple | candidate, PR | `GIT_INTEGRATION_GATE_PASS_RECEIPT` | AND | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELIVERED / FAILED | DRAFT_DELIVERED / DRAFT_DELIVERY_FAILED |
| `READY_TRANSITION_RECEIPT` | vibedev/orchestrator | DRAFT_PR_DELIVERY_RECEIPT + DRAFT_TO_READY_APPROVAL_PACKET freeze/validation + operator Draft→Ready authorization | PR triple | `DRAFT_TO_READY_APPROVAL_PACKET` | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `DRAFT_PR_DELIVERY_RECEIPT`, `PACKET_FREEZE_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET) | AND(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_PR_DELIVERY_RECEIPT, PACKET_FREEZE_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)) | PR_METADATA_SNAPSHOT, OPERATOR_DECISION_RECORD | READY / BLOCKED | PR_READY / PR_READY_BLOCKED |
| `MERGE_DELIVERY_RECEIPT` | git-integrator | READY_TRANSITION_RECEIPT + MERGE_APPROVAL_PACKET freeze/validation + operator Merge authorization | PR triple | `MERGE_APPROVAL_PACKET` | `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (MERGE_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (MERGE_APPROVAL_PACKET) | AND(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, READY_TRANSITION_RECEIPT, PACKET_FREEZE_RECEIPT(MERGE_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(MERGE_APPROVAL_PACKET)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | MERGED / FAILED | PR_MERGED / MERGE_FAILED |
| `BRANCH_DELETION_RECEIPT` | git-integrator | MERGE_DELIVERY_RECEIPT + BRANCH_DELETION_APPROVAL_PACKET freeze/validation + operator Branch Deletion authorization | branch triple | `BRANCH_DELETION_APPROVAL_PACKET` | `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT`, `PACKET_FREEZE_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET) | AND(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, MERGE_DELIVERY_RECEIPT, PACKET_FREEZE_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELETED / FAILED | BRANCH_DELETED / BRANCH_DELETION_FAILED |
| `POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | vibedev/orchestrator | DRAFT_PR_DELIVERY_RECEIPT + post-draft binding evidence | git-integrator role completion triple | git-integrator role Artifact | `DRAFT_PR_DELIVERY_RECEIPT` | AND | MODEL_INVOCATION_RECORD, GIT_OBJECT | BOUND | GIT_INTEGRATOR_BOUND |
| `CONSULTATION_ENDPOINT_RECEIPT` | vibedev/orchestrator | consultation scope + Work Order | CONSULTATION_DELIVERABLE triple | consultation source packet, Work Order | `GLOBAL_READINESS_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(GLOBAL_READINESS_RECEIPT, OPERATOR_WORK_ORDER_APPROVAL_RECEIPT) | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | DELIVERED / FAILED | CONSULTATION_DELIVERED / CONSULTATION_FAILED |
| `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT` | vibedev/orchestrator (public-safe verification function) | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT + source Evidence | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT triple | source Evidence | upstream Evidence validation Receipt | AND | FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | SAFE / UNSAFE | PROJECTION_SAFE / PROJECTION_UNSAFE |
| `STOP_RECEIPT` | vibedev/orchestrator (only) | STOP_REPORT Artifact + Evidence; or hard STOP signal; or operator instruction | subject STOP reason triple | `STOP_REPORT` (if applicable) | per stop class Evidence | mode-discriminated | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD, STOP signal catalog entry | STOPPED | TASK_STOPPED / WORKFLOW_STOPPED |
| `INVALIDATION_RECEIPT` | vibedev/orchestrator | Evidence of invalidation condition | single Artifact, Evidence, Packet, or Receipt triple | subject object | subject-class-specific Evidence of invalidation | AND | mode-discriminated: per subject type | INVALIDATED | SUBJECT_INVALIDATED |
| `SUPERSESSION_RECEIPT` | vibedev/orchestrator | Evidence of supersession + superseding object | single Artifact, Evidence, Packet, or Receipt triple | superseding object | superseding object receipt + subject-class-specific Evidence | AND | FILE_SNAPSHOT, GIT_OBJECT | SUPERSEDED | OBJECT_SUPERSEDED |
...
| `OPERATOR_SUBMODE_SELECTION_RECEIPT` | operator (only) | operator authority | sub-mode declaration triple | sub-mode declaration | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | AND | OPERATOR_DECISION_RECORD | SELECTED | OPERATOR_SELECTED_SUBMODE |
| `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT` | operator (only) | operator authority | assignment baseline packet triple | assignment baseline packet | `OPERATOR_SUBMODE_SELECTION_RECEIPT` | AND | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT_BASELINE |
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | operator (only) | operator authority | Work Order triple | Work Order declaration | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT` | AND(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, OPERATOR_SUBMODE_SELECTION_RECEIPT, mode-discriminated: FULL/LIGHTWEIGHT → AND(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT); CONSULTATION_ONLY → NOT_APPLICABLE_IF(MODE_IS(CONSULTATION_ONLY))) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_WORK_ORDER |
| `OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT` | operator (only) | operator authority | implementation plan approval packet triple | implementation plan | `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_IMPLEMENTATION_PLAN |
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | operator (only, FULL/LIGHTWEIGHT only — not Consultation) | operator authority | `DRAFT_TO_READY_APPROVAL_PACKET` triple | `DRAFT_TO_READY_APPROVAL_PACKET` | `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT`, `OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT`, `PACKET_FREEZE_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `GIT_INTEGRATION_GATE_PASS_RECEIPT` | AND(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT, PACKET_FREEZE_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET), GIT_INTEGRATION_GATE_PASS_RECEIPT, mode-discriminated: CONSULTATION_ONLY → NOT_APPLICABLE_IF(MODE_IS(CONSULTATION_ONLY))) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_DRAFT_TO_READY |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | `MERGE_APPROVAL_PACKET` triple | `MERGE_APPROVAL_PACKET` | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (MERGE_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (MERGE_APPROVAL_PACKET) | AND(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, PACKET_FREEZE_RECEIPT(MERGE_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(MERGE_APPROVAL_PACKET)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_MERGE |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | `BRANCH_DELETION_APPROVAL_PACKET` triple | `BRANCH_DELETION_APPROVAL_PACKET` | `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET) | AND(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, PACKET_FREEZE_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET), PACKET_VALIDATION_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_BRANCH_DELETION |
| `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | operator (only) | operator authority (§3.8 V3) | VERSION_PREPARATION_PACKET triple | `VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_INVENTORY_ARTIFACT`, `VERSION_CHANGE_PROPOSAL_ARTIFACT` | `PACKET_VALIDATION_RECEIPT` (VERSION_GOVERNANCE_SCOPE_PACKET) | AND(PACKET_VALIDATION_RECEIPT(VERSION_GOVERNANCE_SCOPE_PACKET), VALID(VERSION_INVENTORY_ARTIFACT), VALID(VERSION_CHANGE_PROPOSAL_ARTIFACT)) | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | APPROVED | VERSION_PREPARATION_APPROVED |
| `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | operator (only) | operator checkpoint decision (§3.8 V4) | VERSION_PRE_EXECUTION_CHECKPOINT_PACKET triple | `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | AND | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | REVIEWED / BLOCKED | VERSION_CHECKPOINT_REVIEWED / VERSION_CHECKPOINT_BLOCKED |
| `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | operator (only) | operator authority (§3.8 V5) | VERSION_EXECUTION_AUTHORIZATION_PACKET triple | `VERSION_EXECUTION_AUTHORIZATION_PACKET` | `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | AND(STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_CHECKPOINT_REVIEWED)) — `VERSION_CHECKPOINT_BLOCKED` does not satisfy V5 | OPERATOR_DECISION_RECORD | AUTHORIZED | VERSION_EXECUTION_AUTHORIZED |
| `VERSION_QUALIFICATION_PASS_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | PASS | VERSION_QUALIFICATION_PASS |
| `VERSION_QUALIFICATION_FAIL_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | FAIL | VERSION_QUALIFICATION_FAIL |
| `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT` | vibedev/orchestrator | V7 qualification receipt + all V0–V7 object Receipts (§3.8 V8) | VERSION_CLOSEOUT_REPORT triple | `VERSION_CLOSEOUT_REPORT`, `VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_INVENTORY_ARTIFACT`, `VERSION_CHANGE_PROPOSAL_ARTIFACT`, `VERSION_PREPARATION_PACKET`, `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET`, `VERSION_EXECUTION_AUTHORIZATION_PACKET`, `VERSION_QUALIFICATION_PACKET` | `VERSION_QUALIFICATION_PASS_RECEIPT`, `VERSION_QUALIFICATION_FAIL_RECEIPT`, `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT`, `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`, `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | EXACTLY_ONE_OF(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_FAIL_RECEIPT) ANDed with CURRENT Receipt for each V0–V7 object — FAIL path: produces only `VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP` (audit-only, no qualification success, no recovery); produces this state through its exactly_one_of expression with `VERSION_QUALIFICATION_FAIL_RECEIPT` and current `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | all V0–V7 Evidence classes per their corresponding Receipt prerequisites | CLOSED / CLOSED_AFTER_FAILURE_STOP | VERSION_GOVERNANCE_CLOSED_PASS / VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP |
