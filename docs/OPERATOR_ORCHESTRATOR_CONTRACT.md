...
**`budget_contract`** — mode-discriminated:
  ...
  - **`FULL_9_ROLE_VIBECODING`** and **`LIGHTWEIGHT_OPERATION`**: additionally, non-orchestrator assignment baseline failure, non-orchestrator Activation failure, and Gate / candidate / remote / Git / push / PR API / proxy bypass failures trigger immediate `STOP`.
...
- `PREMATURE_GIT_INTEGRATION` — `git-integrator` performing formal Git write before the mode-appropriate pre-Git gates are all valid for the current candidate (§4.16.7): FULL without both `TEST_GATE_PASS` and `REVIEW_GATE_PASS`; LIGHTWEIGHT without all Work Order `required_pre_git_gates` valid, with any Gate member empty, without at least one independent verifier Gate `PASS`, or with any source artifact invalidated. [signal_kind=DRIFT_STOP]
...
- `OPERATOR_PLAN_APPROVAL_BYPASSED` — candidate formed without valid operator approval evidence where required (§4.16.2). **Immediate STOP.** [signal_kind=DRIFT_STOP]
- `LIGHTWEIGHT_PLAN_SOURCE_MISMATCH` — `LIGHTWEIGHT` candidate plan source does not match the Work Order's Plan checkpoint setting (§4.16.2). Return to correct binding; if unresolvable within pre-approved budget, STOP. [signal_kind=DRIFT_STOP]
...
- `LIGHTWEIGHT_DIRECT_IMPLEMENTATION_PROVENANCE_MISSING` — LIGHTWEIGHT without Planner / Plan checkpoint but candidate missing `OPERATOR_APPROVED_WORK_ORDER` binding or `direct_to_implementer=true` approval field (§4.16.2). **Immediate STOP.** [signal_kind=DRIFT_STOP]
...
- `BROAD_STAGE_OPERATION_DETECTED` — `git add .`, `git add -A`, or `git commit -am` used instead of exact staging per candidate manifest (§4.16.10). **Immediate STOP.** [signal_kind=DRIFT_STOP]
- `CANDIDATE_COMMIT_TREE_MISMATCH` — final commit tree does not match frozen candidate content tree (§4.16.10). **Immediate STOP.** [signal_kind=DRIFT_STOP]
...
- `APPROVED_NEW_FILE_OMITTED_FROM_STAGE` — candidate manifest includes approved new paths but `git-integrator` omitted them from staging (§4.16.10). **Immediate STOP.** [signal_kind=DRIFT_STOP]
- `UNAPPROVED_UNTRACKED_PATH_STAGED` — any untracked path not in the candidate manifest was staged (§4.16.10). Entry-time untracked inventory and Work Order-protected paths are reference information only and do **not** constitute stage permission. Manifest-approved new paths remain permitted. **Immediate STOP.** [signal_kind=DRIFT_STOP]
...
- `CONSULTATION_RUNTIME_STATE_FABRICATED` — a Consultation Work Order's runtime state creates or references fictional Gate, candidate, or Git state (§4.17.12). **Immediate STOP.** [signal_kind=DRIFT_STOP]
...
- `CONSULTATION_REQUIRED_EVIDENCE_OR_AUTHORIZATION_OBJECT_FORBIDDEN` — a required common governance object (operator approval Receipt, Evidence, model invocation Evidence, public projection, invalidation/supersession Receipt) is missing in Consultation mode, or a forbidden object (non-orchestrator assignment/Activation, candidate, test/review Gate, Integration/Git/Post-Draft) is present; [signal_kind=DRIFT_STOP]
...
The Registry prerequisite expressions use a **closed, machine-parseable grammar**. Only the following predicates, operators, and parameter types are permitted. Undefined predicates trigger `RECEIPT_REGISTRY_PREREQUISITE_EXPRESSION_UNPARSEABLE → STOP`.

**Predicates (canonical):**

| Predicate | Semantics | Argument |
|---|---|---|
| `TRUE` | unconditional / no prerequisite | — |
| `CURRENT(receipt_type)` | subject has a `CURRENT` Receipt of the given type | canonical Receipt type or `(canonical_Receipt_type, OBJECT_TRIPLE)` |
| `STATE_CREATED(receipt_type, value)` | subject's `state_created` equals `value` | canonical Receipt type, enum value |
| `DECISION(receipt_type, value)` | subject's `decision_result` equals `value` | canonical Receipt type, enum value |
| `VALID(object_type)` | object is `VALID` per its lifecycle registry | canonical object type |
| `SAME_OBJECT(refs...)` | all refs point to the same object triple | canonical refs |
| `MODE_IS(mode)` | execution mode is `mode` | `FULL`, `LIGHTWEIGHT`, `CONSULTATION_ONLY` |
| `STAGE_IS(stage)` | execution stage is `stage` | canonical stage name |
| `SUBJECT_MATCH(receipt_type, ref)` | receipt subject triple equals `ref` | canonical Receipt type, ref |
| `NOT_APPLICABLE_IF(predicate)` | prerequisite is inapplicable when predicate true | any canonical predicate |
| `NOT(predicate)` | boolean negation of predicate | any canonical predicate |
| `EVIDENCE_OF(condition)` | Evidence of `condition` exists in Evidence chain | canonical condition string |

**Operators (canonical):** `AND(...)`, `OR(...)`, `EXACTLY_ONE_OF(...)`.

**Object types (canonical):** every value in `Prerequisite Object Types` must be a canonical object type from the §10.3 object taxonomy (`ARTIFACT`, `EVIDENCE`, `PACKET`, `RECEIPT`, or a subtype such as `INTAKE_SCOPE_PACKET`, `INTEGRATION_INPUT_FROZEN`, etc.).

**Canonical Receipt types only.** Every value in `Prerequisite Receipt Types` must be a canonical Receipt type from the registry. Deleted types, signal IDs, or natural-language phrases are forbidden.

**Bare natural-language phrases forbidden (examples):** `as applicable`, `all preceding validation Receipts`, `upstream Evidence validation Receipt`, `per stop class Evidence`, `subject-class-specific Evidence`, `superseding object receipt`, `CURRENT Receipt for each/all object`, `mode-discriminated` (as a stand-alone phrase), `test failure Evidence`, `integration failure Evidence`.

**Citation example:** `AND(CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT), MODE_IS(FULL) OR MODE_IS(LIGHTWEIGHT))`.

#### §10.3.10.1 Authority Matrix — Registry Issuer Closure

The Authority Matrix must cover every issuer class used in the Registry. `Registry_issuers − Matrix_issuers = 0` and `Matrix_issuers − Registry_issuers = 0`. Recognised issuer classes: `operator`, `vibedev/orchestrator`, `git-integrator`, `validator per mode`, `consumer role/stage controller (vibedev/orchestrator)`, `assembler (vibedev/orchestrator)`.
...
|---|---|---|---|---|---|---|---|---|---|
| `GLOBAL_READINESS_RECEIPT` | vibedev/orchestrator | operator Work Order approval | scope packet triple | `INTAKE_SCOPE_PACKET`, `INTAKE_EVIDENCE_PACKET`, Work Order declaration | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT)) | COMMAND_RESULT, FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | PASS / FAIL | GLOBAL_READINESS_PASS / GLOBAL_READINESS_FAIL |
| `ROLE_ACTIVATION_READINESS_RECEIPT` | vibedev/orchestrator (FULL/LIGHTWEIGHT only — not Consultation) | Global Readiness + role/node/model assignment approval | activation packet triple | `ROLE_ACTIVATION_PACKET` | `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`, `GLOBAL_READINESS_RECEIPT` | AND(CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT), CURRENT(GLOBAL_READINESS_RECEIPT), MODE_IS(FULL) OR MODE_IS(LIGHTWEIGHT)) | READINESS_CHECK, NODE_ROUTE_PROBE, MODEL_INVOCATION_RECORD | READY / BLOCKED | ROLE_ACTIVATION_READY / ROLE_ACTIVATION_BLOCKED |
...
| `EXPLORER_VALIDATION_RECEIPT` | vibedev/orchestrator | explorer Artifact | EXPLORER_ARTIFACT triple | `EXPLORER_ARTIFACT` | `ROLE_CROSS_VERIFICATION_RECEIPT` | AND(CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS / FAIL | EXPLORER_VALIDATED / EXPLORER_VALIDATION_FAILED |
| `PLAN_VALIDATION_RECEIPT` | vibedev/orchestrator | implementation plan Artifact | IMPLEMENTATION_PLAN triple | `IMPLEMENTATION_PLAN` | `EXPLORER_VALIDATION_RECEIPT` | AND(CURRENT(EXPLORER_VALIDATION_RECEIPT)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS / FAIL | PLAN_VALIDATED / PLAN_VALIDATION_FAILED |
| `ARTIFACT_VALIDATION_RECEIPT` | vibedev/orchestrator or validator per mode | Artifact type + mode | any Artifact triple | `TRUE` | `TRUE` | TRUE | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | VALID / INVALID | ARTIFACT_VALIDATED / ARTIFACT_INVALID |
| `PACKET_ASSEMBLY_RECEIPT` | assembler (vibedev/orchestrator) | assembly completion | assembled Packet triple | `INTAKE_EVIDENCE_PACKET`, `EXPLORER_OUTPUT_PACKET`, `IMPLEMENTATION_PLAN_APPROVAL_PACKET`, `CANDIDATE_FROZEN_FOR_TEST`, `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, `TEST_EVIDENCE_PACKET`, `REVIEW_INPUT_PACKET`, `INTEGRATION_INPUT_FROZEN`, `DRAFT_TO_READY_APPROVAL_PACKET`, `MERGE_APPROVAL_PACKET`, `BRANCH_DELETION_APPROVAL_PACKET`, `CONSULTATION_SOURCE_PACKET`, `VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_PREPARATION_PACKET`, `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET`, `VERSION_EXECUTION_AUTHORIZATION_PACKET`, `VERSION_QUALIFICATION_PACKET` | per Packet: upstream role completion + validation Receipts specific to that Packet type (e.g. INTEGRATION_INPUT_FROZEN requires GIT_INTEGRATION_GATE_PASS_RECEIPT and TEST_GATE_PASS_RECEIPT and REVIEW_GATE_PASS_RECEIPT) | AND(CURRENT(upstream role completion), CURRENT(upstream validation)) | DIFF_MANIFEST, TREE_MANIFEST | ASSEMBLED | PACKET_ASSEMBLED |
| `PACKET_FREEZE_RECEIPT` | vibedev/orchestrator | Packet assembly | Packet triple | same Packet triple as subject | `PACKET_ASSEMBLY_RECEIPT` | AND(CURRENT(PACKET_ASSEMBLY_RECEIPT)) | FILE_SNAPSHOT, GIT_OBJECT | FROZEN | PACKET_FROZEN |
| `PACKET_VALIDATION_RECEIPT` | vibedev/orchestrator | Packet freeze | Packet triple | same Packet triple as subject | `PACKET_FREEZE_RECEIPT` | AND(CURRENT(PACKET_FREEZE_RECEIPT)) | DIFF_MANIFEST, TREE_MANIFEST, TEST_RESULT | VALIDATED / INVALID | PACKET_VALIDATED / PACKET_VALIDATION_FAILED |
| `PACKET_CONSUMPTION_RECEIPT` | consumer role/stage controller (vibedev/orchestrator) | frozen (optionally validated) Packet | Packet triple | same Packet triple as subject | `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT` | AND(CURRENT(PACKET_FREEZE_RECEIPT), CURRENT(PACKET_VALIDATION_RECEIPT)) | CONSUMER_EVIDENCE | CONSUMED | PACKET_CONSUMED |
| `TEST_GATE_PASS_RECEIPT` | vibedev/orchestrator | tester-a + tester-b cross-verification + same candidate + input Packet freeze | test evidence packet triple | `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (tester-a), `ROLE_CROSS_VERIFICATION_RECEIPT` (tester-b), `PACKET_FREEZE_RECEIPT` (TESTER_A_INPUT_PACKET), `PACKET_FREEZE_RECEIPT` (TESTER_B_INPUT_PACKET) | AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(tester-a), VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(tester-b), VERIFIED), CURRENT(PACKET_FREEZE_RECEIPT(TESTER_A_INPUT_PACKET)), CURRENT(PACKET_FREEZE_RECEIPT(TESTER_B_INPUT_PACKET)), SAME_OBJECT(candidate)) | TEST_RESULT, MODEL_INVOCATION_RECORD | PASS | GATE_PASS |
| `TEST_GATE_FAIL_RECEIPT` | vibedev/orchestrator | tester-a or tester-b cross-verification failure state + same candidate + input Packet freeze | test evidence packet triple | `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (tester-a or tester-b), `PACKET_FREEZE_RECEIPT` (TESTER_A_INPUT_PACKET), `PACKET_FREEZE_RECEIPT` (TESTER_B_INPUT_PACKET) | AND(CURRENT(PACKET_FREEZE_RECEIPT(TESTER_A_INPUT_PACKET)), CURRENT(PACKET_FREEZE_RECEIPT(TESTER_B_INPUT_PACKET)), SAME_OBJECT(candidate), OR(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REJECTED))) | TEST_RESULT, MODEL_INVOCATION_RECORD | FAIL | GATE_FAIL |
| `REVIEW_GATE_PASS_RECEIPT` | vibedev/orchestrator | reviewer-a + reviewer-b cross-verification + same Review Packet/candidate + TEST_GATE_PASS_RECEIPT | review input packet triple | `REVIEW_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (reviewer-a), `ROLE_CROSS_VERIFICATION_RECEIPT` (reviewer-b), `PACKET_FREEZE_RECEIPT` (REVIEW_INPUT_PACKET), `TEST_GATE_PASS_RECEIPT` | AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(reviewer-a), VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(reviewer-b), VERIFIED), CURRENT(PACKET_FREEZE_RECEIPT(REVIEW_INPUT_PACKET)), CURRENT(TEST_GATE_PASS_RECEIPT), SAME_OBJECT(candidate)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS | GATE_PASS |
| `REVIEW_GATE_FAIL_RECEIPT` | vibedev/orchestrator | reviewer-a or reviewer-b failure Evidence + same Review Packet/candidate + TEST_GATE_PASS_RECEIPT | review input packet triple | `REVIEW_INPUT_PACKET`, candidate | `ROLE_CROSS_VERIFICATION_RECEIPT` (reviewer-a or reviewer-b), `PACKET_FREEZE_RECEIPT` (REVIEW_INPUT_PACKET), `TEST_GATE_PASS_RECEIPT` | AND(CURRENT(PACKET_FREEZE_RECEIPT(REVIEW_INPUT_PACKET)), CURRENT(TEST_GATE_PASS_RECEIPT), SAME_OBJECT(candidate), OR(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REJECTED))) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | FAIL | GATE_FAIL |
| `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | git-integrator (or vibedev if 5bao/9bao cannot execute) | pre-Draft git-integrator binding for Git Integration stage | git-integrator role Assignment triple + Git Integration stage Evidence | candidate, git-integrator role Activation Evidence | `ROLE_ACTIVATION_READINESS_RECEIPT` (git-integrator) | AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT(git-integrator)), MODE_IS(FULL) OR MODE_IS(LIGHTWEIGHT)) | GIT_OBJECT, PR_METADATA_SNAPSHOT, MODEL_INVOCATION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_PRE_DRAFT_BOUND / GIT_INTEGRATOR_PRE_DRAFT_UNBOUND |
| `GIT_INTEGRATION_GATE_PASS_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_PASS_RECEIPT + Integration Packet freeze/validation + PRE_DRAFT_GIT_INTEGRATOR_BINDING | INTEGRATION_INPUT_FROZEN triple | `INTEGRATION_INPUT_FROZEN` | `REVIEW_GATE_PASS_RECEIPT`, `PACKET_FREEZE_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PACKET_VALIDATION_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | AND(CURRENT(REVIEW_GATE_PASS_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(INTEGRATION_INPUT_FROZEN)), CURRENT(PACKET_VALIDATION_RECEIPT(INTEGRATION_INPUT_FROZEN)), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT)) | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | PASS | GATE_PASS |
| `GIT_INTEGRATION_GATE_FAIL_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_PASS_RECEIPT + Integration Packet freeze/validation + PRE_DRAFT_GIT_INTEGRATOR_BINDING + integration-stage failure Evidence | INTEGRATION_INPUT_FROZEN triple | `INTEGRATION_INPUT_FROZEN` | `REVIEW_GATE_PASS_RECEIPT`, `PACKET_FREEZE_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PACKET_VALIDATION_RECEIPT` (INTEGRATION_INPUT_FROZEN), `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | AND(CURRENT(REVIEW_GATE_PASS_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(INTEGRATION_INPUT_FROZEN)), CURRENT(PACKET_VALIDATION_RECEIPT(INTEGRATION_INPUT_FROZEN)), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT), OR(DECISION(TEST_RESULT, FAIL), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT(integration-role), INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT(integration-role), REJECTED))) — `REVIEW_GATE_FAIL_RECEIPT` does not substitute; on Review FAIL Git Integration stage is not instantiated | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | FAIL | GATE_FAIL |
| `DRAFT_PR_DELIVERY_RECEIPT` | git-integrator | GIT_INTEGRATION_GATE_PASS_RECEIPT + candidate + PR | candidate + PR triple | candidate, PR | `GIT_INTEGRATION_GATE_PASS_RECEIPT`, `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | AND(CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELIVERED / FAILED | DRAFT_DELIVERED / DRAFT_DELIVERY_FAILED |
| `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization) | stage-specific post-Draft binding for Draft→Ready operation | `DRAFT_TO_READY_APPROVAL_PACKET` triple + git-integrator stage Activation Evidence | `DRAFT_TO_READY_APPROVAL_PACKET`, stage-specific git-integrator Activation Evidence | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `DRAFT_PR_DELIVERY_RECEIPT` | AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT), CURRENT(DRAFT_PR_DELIVERY_RECEIPT)) | MODEL_INVOCATION_RECORD (git-integrator, stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_DRAFT_TO_READY_BOUND / GIT_INTEGRATOR_DRAFT_TO_READY_UNBOUND |
| `READY_TRANSITION_RECEIPT` | vibedev/orchestrator (cross-verification of git-integrator completion) | operator Draft→Ready authorization + DRAFT_PR_DELIVERY + DRAFT_TO_READY_APPROVAL_PACKET freeze/validation + POST_DRAFT_DRAFT_TO_READY_BINDING + git-integrator completion Evidence + cross-verification | PR triple | `DRAFT_TO_READY_APPROVAL_PACKET` | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `DRAFT_PR_DELIVERY_RECEIPT`, `PACKET_FREEZE_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` | AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT), CURRENT(DRAFT_PR_DELIVERY_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(git-integrator), VERIFIED)) | PR_METADATA_SNAPSHOT, OPERATOR_DECISION_RECORD | READY / BLOCKED | PR_READY / PR_READY_BLOCKED |
| `POST_DRAFT_MERGE_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization) | stage-specific post-Draft binding for Merge operation | `MERGE_APPROVAL_PACKET` triple + git-integrator stage Activation Evidence | `MERGE_APPROVAL_PACKET`, stage-specific git-integrator Activation Evidence | `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT` | AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT), CURRENT(READY_TRANSITION_RECEIPT)) | MODEL_INVOCATION_RECORD (git-integrator, stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_MERGE_BOUND / GIT_INTEGRATOR_MERGE_UNBOUND |
| `MERGE_DELIVERY_RECEIPT` | git-integrator | operator Merge authorization + READY_TRANSITION_RECEIPT + MERGE_APPROVAL_PACKET freeze/validation + POST_DRAFT_MERGE_BINDING + git-integrator completion Evidence | PR triple | `MERGE_APPROVAL_PACKET` | `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (MERGE_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (MERGE_APPROVAL_PACKET), `POST_DRAFT_MERGE_BINDING_RECEIPT` | AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT), CURRENT(READY_TRANSITION_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(MERGE_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(MERGE_APPROVAL_PACKET)), CURRENT(POST_DRAFT_MERGE_BINDING_RECEIPT), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(git-integrator), VERIFIED)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | MERGED / FAILED | PR_MERGED / MERGE_FAILED |
| `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization) | stage-specific post-Draft binding for Branch Deletion operation | `BRANCH_DELETION_APPROVAL_PACKET` triple + git-integrator stage Activation Evidence | `BRANCH_DELETION_APPROVAL_PACKET`, stage-specific git-integrator Activation Evidence | `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT` | AND(CURRENT(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT), CURRENT(MERGE_DELIVERY_RECEIPT)) | MODEL_INVOCATION_RECORD (git-integrator, stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_BRANCH_DELETION_BOUND / GIT_INTEGRATOR_BRANCH_DELETION_UNBOUND |
| `BRANCH_DELETION_RECEIPT` | git-integrator | operator Branch Deletion authorization + MERGE_DELIVERY + BRANCH_DELETION_APPROVAL_PACKET freeze/validation + POST_DRAFT_BRANCH_DELETION_BINDING + git-integrator completion Evidence | branch triple | `BRANCH_DELETION_APPROVAL_PACKET` | `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT`, `PACKET_FREEZE_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET), `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` | AND(CURRENT(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT), CURRENT(MERGE_DELIVERY_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET)), CURRENT(POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT(git-integrator), VERIFIED)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELETED / FAILED | BRANCH_DELETED / BRANCH_DELETION_FAILED |
| `CONSULTATION_ENDPOINT_RECEIPT` | vibedev/orchestrator | consultation scope + Work Order | CONSULTATION_DELIVERABLE triple | `CONSULTATION_SOURCE_PACKET`, Work Order | `GLOBAL_READINESS_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(CURRENT(GLOBAL_READINESS_RECEIPT), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT), MODE_IS(CONSULTATION_ONLY)) | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | DELIVERED / FAILED | CONSULTATION_DELIVERED / CONSULTATION_FAILED |
| `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT` | vibedev/orchestrator (public-safe verification function) | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT + source Evidence | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT triple | source Evidence objects (per projection) | `TRUE` (no specific Receipt; verification function self-validates) | TRUE | FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | SAFE / UNSAFE | PROJECTION_SAFE / PROJECTION_UNSAFE |
| `STOP_RECEIPT` | vibedev/orchestrator (only) | STOP_REPORT Artifact + Evidence; or hard STOP signal; or operator instruction | subject STOP reason triple | `STOP_REPORT` (if applicable) | STOP_REPORT Receipt or specific stop signal | STOP_REPORT or STOP signal catalog entry | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD, STOP signal catalog entry | STOPPED | TASK_STOPPED / WORKFLOW_STOPPED |
| `INVALIDATION_RECEIPT` | vibedev/orchestrator | Evidence of invalidation condition | single Artifact, Evidence, Packet, or Receipt triple | subject object | per subject type: Artifact → no prerequisite; Evidence → no prerequisite; Packet → PACKET_FREEZE_RECEIPT; Receipt → no prerequisite | AND(EVIDENCE_OF(invalidation)) | COMMAND_RESULT, MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | INVALIDATED | SUBJECT_INVALIDATED |
| `SUPERSESSION_RECEIPT` | vibedev/orchestrator | Evidence of supersession + superseding object Receipt | single Artifact, Evidence, Packet, or Receipt triple | superseding object | per subject type: subject is CURRENT | AND(EVIDENCE_OF(supersession), CURRENT(superseding object Receipt)) | FILE_SNAPSHOT, GIT_OBJECT | SUPERSEDED | OBJECT_SUPERSEDED |
| `OPERATOR_REVOCATION_RECEIPT` | operator (only) | operator authority | single operator-authorisation Receipt triple | `TRUE` | `TRUE` | TRUE | OPERATOR_DECISION_RECORD | REVOKED | AUTHORISATION_REVOKED |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | operator (only) | operator authority | intake scope packet triple | `TRUE` | `TRUE` | TRUE | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_INTAKE_SCOPE |
| `OPERATOR_SUBMODE_SELECTION_RECEIPT` | operator (only) | operator authority | sub-mode declaration triple | sub-mode declaration | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT)) | OPERATOR_DECISION_RECORD | SELECTED | OPERATOR_SELECTED_SUBMODE |
...
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | operator (only) | operator authority | Work Order triple | Work Order declaration | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT` | AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT), CURRENT(OPERATOR_SUBMODE_SELECTION_RECEIPT), AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT), CURRENT(OPERATOR_SUBMODE_SELECTION_RECEIPT), FULL → AND(CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT)) OR LIGHTWEIGHT → AND(CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT)) OR (CONSULTATION_ONLY → NOT_APPLICABLE_IF(MODE_IS(CONSULTATION_ONLY)))) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_WORK_ORDER |
...
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | operator (only, FULL/LIGHTWEIGHT only — not Consultation) | operator authority | `DRAFT_TO_READY_APPROVAL_PACKET` triple | `DRAFT_TO_READY_APPROVAL_PACKET` | `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT`, `PACKET_FREEZE_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (DRAFT_TO_READY_APPROVAL_PACKET), `GIT_INTEGRATION_GATE_PASS_RECEIPT` | AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT), AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(DRAFT_TO_READY_APPROVAL_PACKET)), CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT), OR(FULL AND CURRENT(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT), LIGHTWEIGHT with plan checkpoint enabled AND CURRENT(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT), LIGHTWEIGHT with plan checkpoint disabled, LIGHTWEIGHT with direct_to_implementer=true AND no Planner role instantiated), CONSULTATION_ONLY → NOT_APPLICABLE_IF(MODE_IS(CONSULTATION_ONLY))) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_DRAFT_TO_READY |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | `MERGE_APPROVAL_PACKET` triple | `MERGE_APPROVAL_PACKET` | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (MERGE_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (MERGE_APPROVAL_PACKET) | AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(MERGE_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(MERGE_APPROVAL_PACKET))) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_MERGE |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | `BRANCH_DELETION_APPROVAL_PACKET` triple | `BRANCH_DELETION_APPROVAL_PACKET` | `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `PACKET_FREEZE_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET), `PACKET_VALIDATION_RECEIPT` (BRANCH_DELETION_APPROVAL_PACKET) | AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT), CURRENT(PACKET_FREEZE_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET)), CURRENT(PACKET_VALIDATION_RECEIPT(BRANCH_DELETION_APPROVAL_PACKET))) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_BRANCH_DELETION |
| `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | operator (only) | operator authority (§3.8 V3) | VERSION_PREPARATION_PACKET triple | `VERSION_PREPARATION_PACKET` | `PACKET_VALIDATION_RECEIPT` (VERSION_GOVERNANCE_SCOPE_PACKET) | AND(CURRENT(PACKET_VALIDATION_RECEIPT(VERSION_GOVERNANCE_SCOPE_PACKET)), VALID(VERSION_INVENTORY_ARTIFACT), VALID(VERSION_CHANGE_PROPOSAL_ARTIFACT)) | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | APPROVED | VERSION_PREPARATION_APPROVED |
| `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | operator (only) | operator checkpoint decision (§3.8 V4) | VERSION_PRE_EXECUTION_CHECKPOINT_PACKET triple | `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | AND(CURRENT(OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT)) | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | REVIEWED / BLOCKED | VERSION_CHECKPOINT_REVIEWED / VERSION_CHECKPOINT_BLOCKED |
| `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | operator (only) | operator authority (§3.8 V5) | VERSION_EXECUTION_AUTHORIZATION_PACKET triple | `VERSION_EXECUTION_AUTHORIZATION_PACKET` | `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | AND(STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, REVIEWED), NOT(STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, BLOCKED))) | OPERATOR_DECISION_RECORD | AUTHORIZED | VERSION_EXECUTION_AUTHORIZED |
| `VERSION_QUALIFICATION_PASS_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution Evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, AUTHORIZED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | PASS | VERSION_QUALIFICATION_PASS |
| `VERSION_QUALIFICATION_FAIL_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution Evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, AUTHORIZED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | FAIL | VERSION_QUALIFICATION_FAIL |
| `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT` | vibedev/orchestrator | V7 qualification receipt + all V0–V7 objects (§3.8 V8) | VERSION_CLOSEOUT_REPORT triple | `VERSION_CLOSEOUT_REPORT`, `VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_INVENTORY_ARTIFACT`, `VERSION_CHANGE_PROPOSAL_ARTIFACT`, `VERSION_PREPARATION_PACKET`, `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET`, `VERSION_EXECUTION_AUTHORIZATION_PACKET`, `VERSION_QUALIFICATION_PACKET` | `VERSION_QUALIFICATION_PASS_RECEIPT`, `VERSION_QUALIFICATION_FAIL_RECEIPT`, `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT`, `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`, `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT`, `PACKET_VALIDATION_RECEIPT` (VERSION_GOVERNANCE_SCOPE_PACKET), `PACKET_VALIDATION_RECEIPT` (VERSION_PREPARATION_PACKET), `PACKET_VALIDATION_RECEIPT` (VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), `PACKET_VALIDATION_RECEIPT` (VERSION_EXECUTION_AUTHORIZATION_PACKET), `PACKET_VALIDATION_RECEIPT` (VERSION_QUALIFICATION_PACKET), `VALID(VERSION_INVENTORY_ARTIFACT)`, `VALID(VERSION_CHANGE_PROPOSAL_ARTIFACT)` | EXACTLY_ONE_OF(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_FAIL_RECEIPT) ANDed with CURRENT state for each V0–V7 prerequisite above. V4 BLOCKED state forbids V5 and V8 success closeout. FAIL path produces only `VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP` (audit-only) | all V0–V7 Evidence classes per their corresponding Receipt prerequisites | CLOSED / CLOSED_AFTER_FAILURE_STOP | VERSION_GOVERNANCE_CLOSED_PASS / VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP |
...
**VIBECODING_CONSULTATION_ONLY**: must not fabricate candidate, Gate, or Git objects. Consultation-specific work product and endpoint objects are limited to:
...
The following are **forbidden** in Consultation mode: non-orchestrator role assignment / Activation, candidate objects, test Gate objects, review Gate objects, Integration Packet, Git delivery objects, and Post-Draft objects.
