...
| `OPERATOR_REVOCATION_RECEIPT` | operator (only) | operator authority | single operator-authorisation Receipt triple | `AUTHORISATION_RECEIPT_TRIPLE` | ∅ | TRUE | OPERATOR_DECISION_RECORD | REVOKED | AUTHORISATION_REVOKED |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | operator (only) | operator authority (may be followed by `OPERATOR_REVOCATION_RECEIPT`) | intake scope packet triple | `INTAKE_SCOPE_PACKET` | ∅ | TRUE | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_INTAKE_SCOPE |
