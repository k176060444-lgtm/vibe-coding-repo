...
**`MERGE_COMPLETION_REPORT`.** Must record at minimum:
  ...
  - **`POST_MERGE_VERIFICATION_PASS` / `POST_MERGE_VERIFICATION_FAIL` claim** and operations **not** executed;
...
**Three Post-Draft operations — unified report model.** Each Post-Draft operation has its own structured completion report and endpoint state. All three follow the same rule: only after the `*_COMPLETION_REPORT` passes `vibedev` cross-verification may the corresponding `*_VERIFIED` and endpoint state be formed; PASS / FAIL paths are mutually exclusive; FAIL does **not** form any success endpoint.

  - Ready → Post-Ready verification → `READY_TRANSITION_PASS_CLAIM` / `READY_TRANSITION_FAIL_CLAIM` → `READY_TRANSITION_COMPLETION_REPORT` → cross-verification → PASS: `PR_READY_VERIFIED` → STOP; FAIL: `READY_TRANSITION_VERIFICATION_FAIL` → STOP;
  - Merge → method-specific + post-merge verification → `POST_MERGE_VERIFICATION_PASS_CLAIM` / `POST_MERGE_VERIFICATION_FAIL_CLAIM` → `MERGE_COMPLETION_REPORT` → cross-verification → PASS: `MERGE_DELIVERY_VERIFIED` + `POST_MERGE_VERIFICATION_PASS` → `MERGE_OPERATION_ENDPOINT_REACHED` → STOP; FAIL: `POST_MERGE_VERIFICATION_FAIL` → STOP;
  - Branch Deletion → post-delete verification → `BRANCH_DELETION_COMPLETION_REPORT` → cross-verification → `BRANCH_DELETION_VERIFIED` → `BRANCH_DELETION_OPERATION_ENDPOINT_REACHED` → STOP.
