...
**`budget_contract`** — mode-discriminated. **All modes**: automatic model substitution, quota retry, and quota-reset wait are all `false`. `MODEL_QUOTA_EXHAUSTED`, credential / control-plane / assignment / evidence failure, or any evidence forgery triggers immediate `STOP`.
  ...
  - **`VIBECODING_CONSULTATION_ONLY`**: must include orchestrator model / invocation budget, read-source / tool budget, evidence / output capability budget, and operator-approved time / invocation count limits. Corrective loop budget, push budget, PR API budget, and proxy bypass budget fields **must** be `NOT_APPLICABLE` or `disabled / 0`. Their presence **must not** be interpreted as Git delivery capability.
...
### §10.1 Drift Signals
  ...
  - (ssssss) `WORK_ORDER_ARTIFACT_SCHEMA_INCOMPLETE` — mode-discriminated artifact schema check:
    - **`FULL_9_ROLE_VIBECODING`** and **`LIGHTWEIGHT_OPERATION`**: artifact lacks `artifact_id` / `version` / `digest` / `producer_role` / `role_invocation` / `status`, or status value is outside `VALID / INVALIDATED / SUPERSEDED / REVALIDATION_REQUIRED`, or frozen artifact modified in place (§4.17.7).
    - **`VIBECODING_CONSULTATION_ONLY`**: deliverable / report lacks `artifact_id` / `version` / `digest` / source references / orchestrator invocation / evidence / status. `candidate_digest` is **not** required for Consultation. **Immediate STOP.**
