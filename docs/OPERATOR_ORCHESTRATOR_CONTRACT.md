...
`vibedev` performs only cross-verification (§4.13); it **must not** execute the merge, fabricate missing tests, fabricate results, or treat missing evidence as PASS. Only after the report passes cross-verification are `MERGE_DELIVERY_VERIFIED` and `POST_MERGE_VERIFICATION_PASS` formed, followed by `MERGE_OPERATION_ENDPOINT_REACHED` and **immediate STOP**.
...
**Three Post-Draft operations — unified report model.** Each Post-Draft operation has its own structured completion report and endpoint state:

  - Ready → `READY_TRANSITION_COMPLETION_REPORT` → cross-verification → `PR_READY_VERIFIED` → STOP;
  - Merge → `MERGE_COMPLETION_REPORT` → cross-verification → `MERGE_DELIVERY_VERIFIED` + `POST_MERGE_VERIFICATION_PASS` → `MERGE_OPERATION_ENDPOINT_REACHED` → STOP;
  - Branch Deletion → `BRANCH_DELETION_COMPLETION_REPORT` → cross-verification → `BRANCH_DELETION_VERIFIED` → `BRANCH_DELETION_OPERATION_ENDPOINT_REACHED` → STOP.
