...
| `GENERIC_PACKET_REF` (only as Registry-row-bound typed variable; resolves to current row's concrete Packet ref) | packet_variable | row-bound typed variable per §10.3.10.0.5 | resolves at Registry row execution | any (row-bound) | typed variable |
...
Refer to the Instance Key Mapping column (col-3) in each Registry row for per-type instance key rules.
