...
**Stage-specific Post-Draft Executor Bindings — three canonical types.** Each Post-Draft operation (Ready, Merge, Branch Deletion) requires its own stage-specific executor binding Receipt. There is **no** shared `POST_DRAFT_GIT_INTEGRATOR_BINDING` operator-facing identifier; the canonical Receipt types are:

  - **Ready (Draft → Ready)** — `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT`. `OPERATOR_AUTHORIZED_DRAFT_TO_READY` **must** bind this Receipt (and not a generic equivalent).
  - **Merge** — `POST_DRAFT_MERGE_BINDING_RECEIPT`. `OPERATOR_AUTHORIZED_MERGE` **must** bind this Receipt.
  - **Branch Deletion** — `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT`. `OPERATOR_AUTHORIZED_BRANCH_DELETION` **must** bind this Receipt.

Each stage-specific binding carries its own `id + version + digest` triple and **must not** be substituted for another stage's binding. Binding field minimums per stage:

  - at minimum: original `OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT_BASELINE` ID / version / digest; git-integrator role identity; node; model; canonical provider; runtime provider; credential identity; permitted permissions; this stage's `PACKET_OPERATION_REF` and stage-specific git-integrator role-invocation ID;
  ...
  - any change to role / node / model / provider / credential identity / permission **immediately invalidates** the stage-specific authorisation and forces **immediate STOP**; a new operator-approved stage-specific binding is required;
  - each Post-Draft operation **must** have a new `ROLE_ACTIVATION_READINESS`, a new attempt ID, a formal and attributable role invocation, and an independent completion evidence packet. The Draft delivery attempt and its report **must not** be reused. Each stage's binding is **independent of the other two stages' bindings**; no cross-stage reuse is permitted.
...
OPERATOR_AUTHORIZED_DRAFT_TO_READY
  (must bind POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT)
  → git-integrator ROLE_ACTIVATION_READINESS (§4.10.2)
...
**`READY_TRANSITION_COMPLETION_REPORT`.** Must record at minimum:
  ...
  - `OPERATOR_AUTHORIZED_DRAFT_TO_READY` ID / version / digest and `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` ID / version / digest;
  - git-integrator `ROLE_ACTIVATION_READINESS` result and attempt ID (stage-specific, Ready operation, not Draft delivery);
...
OPERATOR_AUTHORIZED_MERGE
  (must bind POST_DRAFT_MERGE_BINDING_RECEIPT)
  → git-integrator ROLE_ACTIVATION_READINESS (§4.10.2)
...
**`MERGE_COMPLETION_REPORT`.** Must record at minimum:
  ...
  - `OPERATOR_AUTHORIZED_MERGE` ID / version / digest and `POST_DRAFT_MERGE_BINDING_RECEIPT` ID / version / digest;
  - git-integrator `ROLE_ACTIVATION_READINESS` result and attempt ID (stage-specific, Merge operation);
...
OPERATOR_AUTHORIZED_BRANCH_DELETION
  (must bind POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT)
...
The authorisation **must** bind at minimum: repository, source branch, PR number, merged PR head, merge commit / target head, `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` (stage-specific to Branch Deletion — **not** the Merge `POST_DRAFT_MERGE_BINDING_RECEIPT` and **not** the Ready `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT`), authorization time.
...
**`BRANCH_DELETION_COMPLETION_REPORT`.** Must record at minimum:
  ...
  - `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` ID / version / digest (stage-specific to Branch Deletion; **not** the Draft delivery `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT`, **not** the Merge `POST_DRAFT_MERGE_BINDING_RECEIPT`, **not** the Ready `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT`);
  - git-integrator `ROLE_ACTIVATION_READINESS` result and attempt ID (stage-specific, Branch Deletion operation);
...
- `POST_DRAFT_EXECUTOR_BINDING_MISSING` — `OPERATOR_AUTHORIZED_DRAFT_TO_READY` / `OPERATOR_AUTHORIZED_MERGE` / `OPERATOR_AUTHORIZED_BRANCH_DELETION` lacks the corresponding stage-specific Post-Draft Executor Binding (`POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` for Ready, `POST_DRAFT_MERGE_BINDING_RECEIPT` for Merge, `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` for Branch Deletion) (§4.16.14). **Immediate STOP.** [signal_kind=DRIFT_STOP]
...
| `INSTANCE_SUBJECT_MATCH(RECEIPT_TYPE, INSTANCE_REF, SUBJECT_REF)` | three required params: RECEIPT_TYPE (a Receipt Type identifier), INSTANCE_REF (a canonical instance ref = `id + version + digest` triple), SUBJECT_REF (a canonical object triple to match against) | the receipt instance identified by INSTANCE_REF (of the given RECEIPT_TYPE) holds an INSTANCE_REF that references the exact SUBJECT_REF triple as its subject. This is a **content-membership check of the instance's evidence**: it verifies the cross-verification instance has the completion report / subject artifact as its subject. This predicate MAY ONLY be invoked from a downstream consumer's expression; an issuance expression MUST NOT invoke it on its own Receipt type. | registry row | n/a | no | TRUE if the subject report/artifact matches the instance's subject_ref; missing-INSTANCE_REF or missing-SUBJECT_REF returns FALSE | n/a |
| `ROW_INSTANCE_IS(INSTANCE_REF)` | a canonical instance key ref per §10.3.10.0.5 Mapping | the current Receipt's instance key equals INSTANCE_REF. This is an **issuance-time** predicate: it checks the to-be-issued Receipt against its own Instance Key Mapping BEFORE the Receipt is created. INSTANCE_REF MUST appear in the allowed-concrete-key-class set for this Receipt Type, OR be a Registry-row-bound typed variable that resolves to such a ref. Missing or out-of-mapping returns FALSE. | registry row | n/a | no | FALSE if INSTANCE_REF not allowed for this Receipt Type | n/a |
...
| `BRANCH_OF_PR(BRANCH_TRIPLE, PR_TRIPLE)` | two canonical refs: BRANCH_TRIPLE (`id + version + digest` of a branch ref), PR_TRIPLE (`id + version + digest` of a PR) | the branch head is reachable from PR's merge commit OR the PR is the historical source that introduced the branch head; returns FALSE if missing or unrelated | registry row | n/a | no | FALSE if branch not of pr | n/a |
| `REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, TARGET_RECEIPT_TYPE, TARGET_INSTANCE_REF)` | three required params: AUTHORISATION_RECEIPT_TRIPLE (the full Receipt triple of the revoked Receipt), TARGET_RECEIPT_TYPE (a Receipt Type identifier), TARGET_INSTANCE_REF (the canonical instance key ref per §10.3.10.0.5 Mapping for TARGET_RECEIPT_TYPE) | the AUTHORISATION_RECEIPT_TRIPLE points to a Receipt whose TARGET_RECEIPT_TYPE matches the given type AND whose recorded TARGET_INSTANCE_REF equals the given canonical instance key. Returns FALSE if AUTHORISATION_RECEIPT_TRIPLE is missing, type doesn't match, or digest doesn't match. Revocation does NOT change the target Receipt's canonical instance key. | registry row | n/a | no | FALSE if missing, type mismatch, or digest mismatch; TRUE iff all three match | n/a |
| `OPERATION_MATCH(REF, OPERATION_REF)` | REF (a ref with `operation_binding = yes` per §10.3.10.0.6: invocation/cross-verification/completion report/approval packet/activation evidence/role triple/typed variable) and OPERATION_REF (any `*_OPERATION_REF`) | the `operation_id + version + digest` recorded by REF equals that of OPERATION_REF. Returns FALSE if REF has `operation_binding = no`, or if REF is missing, or if the operation ids/versions/digests differ. | registry row | n/a | no | FALSE if REF lacks operation_binding or values differ; TRUE if all three match | n/a |
...
| `PACKET_BINDS(PACKET_REF, OBJECT_REF)` | two canonical refs: PACKET_REF (a Packet `id + version + digest` triple), OBJECT_REF (a canonical object `id + version + digest` triple) | the Packet's `deterministic_ordered_membership` entry at any depth includes the exact OBJECT_REF triple (same `id + version + digest`). This is a **content-membership** check: it verifies the Packet actually contains the target object, not just that both belong to the same operation. | registry row | n/a | no | FALSE if Packet does not contain the object; `lifecycle_state_at_creation` always-Packet-content check (not subject to CURRENT semantics of the OBJECT_REF alone) | n/a |
...
| `SOURCE_EVIDENCE_OBJECTS` | evidence | id+version+digest | EVIDENCE lifecycle | SRC | concrete ref |
| `STOP_REPORT` | report | id+version+digest | REPORT lifecycle | all | concrete ref |
| `GENERIC_PACKET_REF` (only as Registry-row-bound typed variable; resolves to current row's concrete Packet ref) | packet_variable | row-bound typed variable per Instance Key Mapping | resolves at Registry row execution | any (row-bound) | typed variable |
| `GIT_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | GIT | concrete ref |
| `PACKET_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | PACKET | concrete ref |
| `DRAFT_TO_READY_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | DRAFT_TO_READY | concrete ref |
| `MERGE_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | MERGE | concrete ref |
| `BRANCH_DELETION_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | BRANCH_DELETION | concrete ref |
| `VERSION_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | VERSION | concrete ref |
| `TEST_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | TEST | concrete ref |
| `REVIEW_OPERATION_REF` | operation_ref | id+version+digest | n/a (operation) | REVIEW | concrete ref |
...
| `INTEGRATION_INPUT_FROZEN` | packet | id+version+digest | PACKET lifecycle | GIT_INTEGRATION | concrete ref |
| `DRAFT_TO_READY_APPROVAL_PACKET` | approval_packet | id+version+digest | PACKET lifecycle | POST_DRAFT | concrete ref |
| `MERGE_APPROVAL_PACKET` | approval_packet | id+version+digest | PACKET lifecycle | POST_DRAFT | concrete ref |
| `BRANCH_DELETION_APPROVAL_PACKET` | approval_packet | id+version+digest | PACKET lifecycle | POST_DRAFT | concrete ref |
| `TESTER_A_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | TEST | concrete ref |
| `TESTER_B_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | TEST | concrete ref |
| `REVIEWER_A_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | REVIEW | concrete ref |
| `REVIEWER_B_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | REVIEW | concrete ref |
| `DRAFT_TO_READY_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | DRAFT_TO_READY | concrete ref |
| `MERGE_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | MERGE | concrete ref |
| `BRANCH_DELETION_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | BRANCH_DELETION | concrete ref |
| `GIT_INTEGRATOR_INVOCATION_REF` | invocation_ref | id+version+digest | n/a (execution) | GIT_INTEGRATION | concrete ref |
| `TESTER_A_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | TEST | concrete ref |
| `TESTER_B_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | TEST | concrete ref |
| `REVIEWER_A_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | REVIEW | concrete ref |
| `REVIEWER_B_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | REVIEW | concrete ref |
| `GIT_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | GIT_INTEGRATION | concrete ref |
| `READY_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | DRAFT_TO_READY | concrete ref |
| `MERGE_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | MERGE | concrete ref |
| `BD_CROSS_VERIFICATION_REF` | cross_verification_ref | id+version+digest | n/a (execution) | BRANCH_DELETION | concrete ref |
| `TESTER_A_COMPLETION_REPORT` | completion_report | per-tester completion artifact | issued at end of tester-a run | TEST | post-stage evidence |
| `TESTER_B_COMPLETION_REPORT` | completion_report | per-tester completion artifact | issued at end of tester-b run | TEST | post-stage evidence |
| `REVIEWER_A_COMPLETION_REPORT` | completion_report | per-reviewer completion artifact | issued at end of reviewer-a run | REVIEW | post-stage evidence |
| `REVIEWER_B_COMPLETION_REPORT` | completion_report | per-reviewer completion artifact | issued at end of reviewer-b run | REVIEW | post-stage evidence |
| `GIT_INTEGRATION_COMPLETION_REPORT` | completion_report | git-integration completion artifact | issued at end of git-integrator run | GIT_INTEGRATION | post-stage evidence |
| `READY_TRANSITION_COMPLETION_REPORT` | completion_report | Ready transition completion artifact | issued at Ready-run end | DRAFT_TO_READY | post-stage evidence |
| `MERGE_COMPLETION_REPORT` | completion_report | Merge completion artifact | issued at merge-run end | MERGE | post-stage evidence |
| `BRANCH_DELETION_COMPLETION_REPORT` | completion_report | Branch deletion completion artifact | issued at BD-run end | BRANCH_DELETION | post-stage evidence |
| `GIT_INTEGRATOR_ROLE_TRIPLE` | role_triple | id+version+digest of GIT role activation | n/a (pre-activation) | GIT_INTEGRATION | type token |
| `TESTER_A_ROLE_TRIPLE` | role_triple | id+version+digest | n/a (pre-activation) | TEST | type token |
| `TESTER_B_ROLE_TRIPLE` | role_triple | id+version+digest | n/a (pre-activation) | TEST | type token |
| `REVIEWER_A_ROLE_TRIPLE` | role_triple | id+version+digest | n/a (pre-activation) | REVIEW | type token |
| `REVIEWER_B_ROLE_TRIPLE` | role_triple | id+version+digest | n/a (pre-activation) | REVIEW | type token |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | GIT | concrete ref |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY` | activation_evidence | id+version+digest | EVIDENCE lifecycle | DRAFT_TO_READY | concrete ref |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | MERGE | concrete ref |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD` | activation_evidence | id+version+digest | EVIDENCE lifecycle | BD | concrete ref |
| `TESTER_A_ACTIVATION_EVIDENCE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | TEST | concrete ref |
| `TESTER_B_ACTIVATION_EVIDENCE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | TEST | concrete ref |
| `REVIEWER_A_ACTIVATION_EVIDENCE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | REVIEW | concrete ref |
| `REVIEWER_B_ACTIVATION_EVIDENCE` | activation_evidence | id+version+digest | EVIDENCE lifecycle | REVIEW | concrete ref |
| `PRE_DRAFT_GIT_BINDING_REF` | binding_ref | per-operation Pre-Draft binding ref | issued at Pre-Draft binding | GIT_INTEGRATION | concrete ref |
| `VERSION_INVENTORY_ARTIFACT` | version_artifact | id+version+digest | ARTIFACT lifecycle | V0 | concrete ref |
| `VERSION_CHANGE_PROPOSAL_ARTIFACT` | version_artifact | id+version+digest | ARTIFACT lifecycle | V1 | concrete ref |
| `VERSION_PREPARATION_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V2 | concrete ref |
| `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V3 | concrete ref |
| `VERSION_EXECUTION_AUTHORIZATION_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V4 | concrete ref |
| `VERSION_EXECUTION_REPORT` | version_report | id+version+digest | REPORT lifecycle | V5 | concrete ref |
| `VERSION_EXECUTION_EVIDENCE_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V6 | concrete ref |
| `VERSION_QUALIFICATION_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V7 | concrete ref |
| `VERSION_CLOSEOUT_REPORT` | version_report | id+version+digest | REPORT lifecycle | V8 | concrete ref |
| `VERSION_GOVERNANCE_SCOPE_PACKET` | version_packet | id+version+digest | PACKET lifecycle | V0 | concrete ref |
| `ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET` | approval_packet | id+version+digest | PACKET lifecycle | F, LL | concrete ref |
| `IMPLEMENTATION_PLAN_APPROVAL_PACKET` | approval_packet | id+version+digest | PACKET lifecycle | F, LL | concrete ref |
...
| `validator per mode` (operator-approved mapping decides object class → validator identity) | `ARTIFACT_VALIDATION_RECEIPT` | per artifact class / per mode | n/a |
...
#### §10.3.10.0.5 Canonical Receipt Instance Key Mapping

| Receipt Type | instance_key_type | allowed key class/ref | canonical subject class | instance = subject | INSTANCE_SUBJECT_MATCH required |
|---|---|---|---|---|---|
| `ARTIFACT_VALIDATION_RECEIPT` | ARTIFACT | `CANDIDATE_TRIPLE` | artifact triple | yes | no |
| `BRANCH_DELETION_RECEIPT` | BRANCH_TRIPLE | `BRANCH_TRIPLE` | branch triple | yes | no |
| `CONSULTATION_ENDPOINT_RECEIPT` | CONSULTATION_PACKET | `CONSULTATION_SOURCE_PACKET` | consultation-source triple | yes | no |
| `DRAFT_PR_DELIVERY_RECEIPT` | PR_TRIPLE | `PR_TRIPLE` | PR triple | yes | no |
| `EXPLORER_VALIDATION_RECEIPT` | EXPLORER_ARTIFACT | `EXPLORER_ARTIFACT` | subject artifact | yes | no |
| `GIT_INTEGRATION_GATE_FAIL_RECEIPT` | INTEGRATION_PACKET | `INTEGRATION_INPUT_FROZEN` | integration packet triple | yes | no |
| `GIT_INTEGRATION_GATE_PASS_RECEIPT` | INTEGRATION_PACKET | `INTEGRATION_INPUT_FROZEN` | integration packet triple | yes | no |
| `GLOBAL_READINESS_RECEIPT` | WORK_ORDER | `WORK_ORDER_DECLARATION` | scope-packet triple | no | yes |
| `INVALIDATION_RECEIPT` | SUBJECT_ARTIFACT | `SUBJECT_ARTIFACT_TRIPLE` | artifact triple | no | yes |
| `MERGE_DELIVERY_RECEIPT` | PR_TRIPLE | `PR_TRIPLE` | PR triple | yes | no |
| `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | VERSION_PACKET | `VERSION_PREPARATION_PACKET` | version-preparation triple | yes | no |
| `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | VERSION_PACKET | `VERSION_EXECUTION_AUTHORIZATION_PACKET` | version-execution-auth triple | yes | no |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | APPROVAL_PACKET | `BRANCH_DELETION_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | APPROVAL_PACKET | `DRAFT_TO_READY_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT` | PLAN | `IMPLEMENTATION_PLAN` | plan triple | yes | no |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | INTAKE_PACKET | `INTAKE_SCOPE_PACKET` | intake-scope triple | yes | no |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | APPROVAL_PACKET | `MERGE_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_REVOCATION_RECEIPT` | AUTHORISATION_TRIPLE | `AUTHORISATION_RECEIPT_TRIPLE` | authorisation triple | yes | no |
| `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT` | BASELINE_PACKET | `ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET` | baseline packet triple | yes | no |
| `OPERATOR_SUBMODE_SELECTION_RECEIPT` | SUBMODE | `SUBMODE_DECLARATION` | submode declaration | yes | no |
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | WORK_ORDER | `WORK_ORDER_DECLARATION` | work-order triple | yes | no |
| `PACKET_ASSEMBLY_RECEIPT` | PACKET_REF | `any class=packet Catalog ref per PACKET_CLASS` | packet triple | yes | no |
| `PACKET_CONSUMPTION_RECEIPT` | PACKET_REF | `any class=packet Catalog ref per PACKET_CLASS` | packet triple | yes | no |
| `PACKET_FREEZE_RECEIPT` | PACKET_REF | `any class=packet Catalog ref per PACKET_CLASS` | packet triple | yes | no |
| `PACKET_VALIDATION_RECEIPT` | PACKET_REF | `any class=packet Catalog ref per PACKET_CLASS` | packet triple | yes | no |
| `PLAN_VALIDATION_RECEIPT` | PLAN | `IMPLEMENTATION_PLAN` | plan triple | yes | no |
| `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` | APPROVAL_PACKET | `BRANCH_DELETION_APPROVAL_PACKET` | approval packet triple | yes | no |
| `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` | APPROVAL_PACKET | `DRAFT_TO_READY_APPROVAL_PACKET` | approval packet triple | yes | no |
| `POST_DRAFT_MERGE_BINDING_RECEIPT` | APPROVAL_PACKET | `MERGE_APPROVAL_PACKET` | approval packet triple | yes | no |
| `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | BINDING_REF | `PRE_DRAFT_GIT_BINDING_REF` | GIT_INTEGRATOR_ACTIVATION_EVIDENCE | no | yes |
| `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT` | SOURCE_OBJECTS | `SOURCE_EVIDENCE_OBJECTS` | subject artifact triple | no | yes |
| `READY_TRANSITION_RECEIPT` | PR_TRIPLE | `PR_TRIPLE` | PR triple | yes | no |
| `REVIEW_GATE_FAIL_RECEIPT` | REVIEW_INPUT_PACKET | `REVIEW_INPUT_PACKET` | review input packet triple | yes | no |
| `REVIEW_GATE_PASS_RECEIPT` | REVIEW_INPUT_PACKET | `REVIEW_INPUT_PACKET` | review input packet triple | yes | no |
| `ROLE_ACTIVATION_READINESS_RECEIPT` | INVOCATION_REF | `specific per-role *_INVOCATION_REF` | activation packet triple per pair | no | yes |
| `ROLE_CROSS_VERIFICATION_RECEIPT` | CROSS_VERIFICATION_REF | `specific per-role *_CROSS_VERIFICATION_REF` | completion report per pair | no | yes |
| `STOP_RECEIPT` | STOP_REPORT | `STOP_REPORT` | stop report triple | yes | no |
| `SUPERSESSION_RECEIPT` | SUPERSEDING_OBJECT | `SUPERSEDING_OBJECT_TRIPLE` | superseded-object triple | no | yes |
| `TEST_GATE_FAIL_RECEIPT` | TEST_EVIDENCE_PACKET | `TEST_EVIDENCE_PACKET` | test evidence packet triple | yes | no |
| `TEST_GATE_PASS_RECEIPT` | TEST_EVIDENCE_PACKET | `TEST_EVIDENCE_PACKET` | test evidence packet triple | yes | no |
| `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT` | VERSION_PACKET | `VERSION_CLOSEOUT_REPORT` | version-closeout triple | yes | no |
| `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | VERSION_PACKET | `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | version-checkpoint triple | yes | no |
| `VERSION_QUALIFICATION_FAIL_RECEIPT` | VERSION_PACKET | `VERSION_QUALIFICATION_PACKET` | version-qualification triple | yes | no |
| `VERSION_QUALIFICATION_PASS_RECEIPT` | VERSION_PACKET | `VERSION_QUALIFICATION_PACKET` | version-qualification triple | yes | no |
#### §10.3.10.0.6 Canonical Operation Binding Mapping

Refer to the Operation Binding Mapping table within the Object Catalog section below.
...
| `GLOBAL_READINESS_RECEIPT` | vibedev/orchestrator | operator Work Order approval | scope packet triple | `INTAKE_SCOPE_PACKET`, `INTAKE_EVIDENCE_PACKET`, `WORK_ORDER_DECLARATION` | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(ROW_INSTANCE_IS(WORK_ORDER_DECLARATION), ROW_SUBJECT_IS(WORK_ORDER_DECLARATION), CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), STATE_CREATED(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET, OPERATOR_APPROVED_INTAKE_SCOPE), CURRENT(OPERATOR_SUBMODE_SELECTION_RECEIPT, SUBMODE_DECLARATION), STATE_CREATED(OPERATOR_SUBMODE_SELECTION_RECEIPT, SUBMODE_DECLARATION, SUBMODE_SELECTED), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER)) | COMMAND_RESULT, FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | APPROVED / BLOCKED | GLOBAL_READINESS_PASS / GLOBAL_READINESS_FAIL |
|`ROLE_ACTIVATION_READINESS_RECEIPT`|vibedev/orchestrator (FULL/LIGHTWEIGHT only — not Consultation)|Global Readiness + role/node/model assignment approval|activation packet triple (per invocation)|`TESTER_A_INVOCATION_REF`, `TESTER_B_INVOCATION_REF`, `REVIEWER_A_INVOCATION_REF`, `REVIEWER_B_INVOCATION_REF`, `GIT_INTEGRATOR_INVOCATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `MERGE_INVOCATION_REF`, `BRANCH_DELETION_INVOCATION_REF`, `TESTER_A_ACTIVATION_EVIDENCE`, `TESTER_B_ACTIVATION_EVIDENCE`, `REVIEWER_A_ACTIVATION_EVIDENCE`, `REVIEWER_B_ACTIVATION_EVIDENCE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD`|`GLOBAL_READINESS_RECEIPT`, `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`| AND(EXACTLY_ONE_OF(AND(ROW_INSTANCE_IS(TESTER_A_INVOCATION_REF), ROW_SUBJECT_IS(TESTER_A_ACTIVATION_EVIDENCE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(TESTER_A_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_ACTIVATION_EVIDENCE, TEST_OPERATION_REF)), AND(ROW_INSTANCE_IS(TESTER_B_INVOCATION_REF), ROW_SUBJECT_IS(TESTER_B_ACTIVATION_EVIDENCE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(TESTER_B_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_ACTIVATION_EVIDENCE, TEST_OPERATION_REF)), AND(ROW_INSTANCE_IS(REVIEWER_A_INVOCATION_REF), ROW_SUBJECT_IS(REVIEWER_A_ACTIVATION_EVIDENCE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(REVIEWER_A_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_ACTIVATION_EVIDENCE, REVIEW_OPERATION_REF)), AND(ROW_INSTANCE_IS(REVIEWER_B_INVOCATION_REF), ROW_SUBJECT_IS(REVIEWER_B_ACTIVATION_EVIDENCE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(REVIEWER_B_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_ACTIVATION_EVIDENCE, REVIEW_OPERATION_REF)), AND(ROW_INSTANCE_IS(GIT_INTEGRATOR_INVOCATION_REF), ROW_SUBJECT_IS(GIT_INTEGRATOR_ACTIVATION_EVIDENCE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(GIT_INTEGRATOR_INVOCATION_REF, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE, GIT_OPERATION_REF)), AND(ROW_INSTANCE_IS(DRAFT_TO_READY_INVOCATION_REF), ROW_SUBJECT_IS(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(DRAFT_TO_READY_INVOCATION_REF, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY, DRAFT_TO_READY_OPERATION_REF)), AND(ROW_INSTANCE_IS(MERGE_INVOCATION_REF), ROW_SUBJECT_IS(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(MERGE_INVOCATION_REF, MERGE_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE, MERGE_OPERATION_REF)), AND(ROW_INSTANCE_IS(BRANCH_DELETION_INVOCATION_REF), ROW_SUBJECT_IS(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(BRANCH_DELETION_INVOCATION_REF, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD, BRANCH_DELETION_OPERATION_REF)))) |READINESS_CHECK, NODE_ROUTE_PROBE, MODEL_INVOCATION_RECORD|READY / BLOCKED|ROLE_ACTIVATION_READY / ROLE_ACTIVATION_BLOCKED|
|`ROLE_CROSS_VERIFICATION_RECEIPT`|vibedev/orchestrator|role completion report Artifact|completion report (per pair)|`BD_CROSS_VERIFICATION_REF`, `BRANCH_DELETION_COMPLETION_REPORT`, `BRANCH_DELETION_INVOCATION_REF`, `BRANCH_DELETION_OPERATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `DRAFT_TO_READY_OPERATION_REF`, `GIT_CROSS_VERIFICATION_REF`, `GIT_INTEGRATION_COMPLETION_REPORT`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE`, `GIT_INTEGRATOR_INVOCATION_REF`, `GIT_OPERATION_REF`, `MERGE_COMPLETION_REPORT`, `MERGE_CROSS_VERIFICATION_REF`, `MERGE_INVOCATION_REF`, `MERGE_OPERATION_REF`, `READY_CROSS_VERIFICATION_REF`, `READY_TRANSITION_COMPLETION_REPORT`, `REVIEWER_A_ACTIVATION_EVIDENCE`, `REVIEWER_A_COMPLETION_REPORT`, `REVIEWER_A_CROSS_VERIFICATION_REF`, `REVIEWER_A_INVOCATION_REF`, `REVIEWER_B_ACTIVATION_EVIDENCE`, `REVIEWER_B_COMPLETION_REPORT`, `REVIEWER_B_CROSS_VERIFICATION_REF`, `REVIEWER_B_INVOCATION_REF`, `REVIEW_OPERATION_REF`, `TESTER_A_ACTIVATION_EVIDENCE`, `TESTER_A_COMPLETION_REPORT`, `TESTER_A_CROSS_VERIFICATION_REF`, `TESTER_A_INVOCATION_REF`, `TESTER_B_ACTIVATION_EVIDENCE`, `TESTER_B_COMPLETION_REPORT`, `TESTER_B_CROSS_VERIFICATION_REF`, `TESTER_B_INVOCATION_REF`, `TEST_OPERATION_REF`|`ROLE_ACTIVATION_READINESS_RECEIPT`, `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`| AND(EXACTLY_ONE_OF(AND(ROW_INSTANCE_IS(TESTER_A_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(TESTER_A_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF, TESTER_A_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(TESTER_A_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_ACTIVATION_EVIDENCE, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_COMPLETION_REPORT, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_CROSS_VERIFICATION_REF, TEST_OPERATION_REF)), AND(ROW_INSTANCE_IS(TESTER_B_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(TESTER_B_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF, TESTER_B_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(TESTER_B_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_ACTIVATION_EVIDENCE, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_COMPLETION_REPORT, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_CROSS_VERIFICATION_REF, TEST_OPERATION_REF)), AND(ROW_INSTANCE_IS(REVIEWER_A_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(REVIEWER_A_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF, REVIEWER_A_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(REVIEWER_A_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_ACTIVATION_EVIDENCE, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_COMPLETION_REPORT, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF)), AND(ROW_INSTANCE_IS(REVIEWER_B_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(REVIEWER_B_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF, REVIEWER_B_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(REVIEWER_B_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_ACTIVATION_EVIDENCE, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_COMPLETION_REPORT, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF)), AND(ROW_INSTANCE_IS(GIT_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(GIT_INTEGRATION_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(GIT_INTEGRATOR_INVOCATION_REF, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATION_COMPLETION_REPORT, GIT_OPERATION_REF), OPERATION_MATCH(GIT_CROSS_VERIFICATION_REF, GIT_OPERATION_REF)), AND(ROW_INSTANCE_IS(READY_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(READY_TRANSITION_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(DRAFT_TO_READY_INVOCATION_REF, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(READY_TRANSITION_COMPLETION_REPORT, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(READY_CROSS_VERIFICATION_REF, DRAFT_TO_READY_OPERATION_REF)), AND(ROW_INSTANCE_IS(MERGE_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(MERGE_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(MERGE_INVOCATION_REF, MERGE_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_COMPLETION_REPORT, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_CROSS_VERIFICATION_REF, MERGE_OPERATION_REF)), AND(ROW_INSTANCE_IS(BD_CROSS_VERIFICATION_REF), ROW_SUBJECT_IS(BRANCH_DELETION_COMPLETION_REPORT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), OPERATION_MATCH(BRANCH_DELETION_INVOCATION_REF, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_COMPLETION_REPORT, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BD_CROSS_VERIFICATION_REF, BRANCH_DELETION_OPERATION_REF)))) |GIT_OBJECT, TEST_RESULT, MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD|VERIFIED / INCOMPLETE / REJECTED|ROLE_CROSS_VERIFIED / ROLE_CROSS_VERIFICATION_INCOMPLETE / ROLE_CROSS_VERIFICATION_REJECTED|
| `EXPLORER_VALIDATION_RECEIPT` | vibedev/orchestrator | explorer Artifact | EXPLORER_ARTIFACT triple | `EXPLORER_ARTIFACT` | `ROLE_CROSS_VERIFICATION_RECEIPT` | AND(ROW_INSTANCE_IS(EXPLORER_ARTIFACT), ROW_SUBJECT_IS(EXPLORER_ARTIFACT), CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), STATE_CREATED(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET, OPERATOR_APPROVED_INTAKE_SCOPE)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | VALID / INVALID | EXPLORER_VALIDATED / EXPLORER_VALIDATION_FAILED |
| `PLAN_VALIDATION_RECEIPT` | vibedev/orchestrator | implementation plan Artifact | IMPLEMENTATION_PLAN triple | `IMPLEMENTATION_PLAN` | `EXPLORER_VALIDATION_RECEIPT` | AND(ROW_INSTANCE_IS(IMPLEMENTATION_PLAN), ROW_SUBJECT_IS(IMPLEMENTATION_PLAN), CURRENT(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT, IMPLEMENTATION_PLAN), STATE_CREATED(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT, IMPLEMENTATION_PLAN, OPERATOR_APPROVED_IMPLEMENTATION_PLAN), CURRENT(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT), STATE_CREATED(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT, EXPLORER_VALIDATED)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | VALID / INVALID | PLAN_VALIDATED / PLAN_VALIDATION_FAILED |
...
| `PACKET_ASSEMBLY_RECEIPT` | assembler (vibedev/orchestrator) | assembly completion of the operation-specific Packet |GENERIC_PACKET_REF triple (current row's bound Packet)|`GENERIC_PACKET_REF`, `PACKET_OPERATION_REF`, `GIT_OPERATION_REF`, `DRAFT_TO_READY_OPERATION_REF`, `MERGE_OPERATION_REF`, `BRANCH_DELETION_OPERATION_REF`, `VERSION_OPERATION_REF`, `EXPLORER_ARTIFACT`, `IMPLEMENTATION_PLAN`, `VERSION_GOVERNANCE_SCOPE_PACKET`, `DRAFT_TO_READY_APPROVAL_PACKET`, `MERGE_APPROVAL_PACKET`, `BRANCH_DELETION_APPROVAL_PACKET`, `CONSULTATION_SOURCE_PACKET`|`EXPLORER_VALIDATION_RECEIPT`, `PLAN_VALIDATION_RECEIPT`| AND(ROW_SUBJECT_IS(GENERIC_PACKET_REF), ROW_OPERATION_IS(PACKET_OPERATION_REF), OR(AND(PACKET_CLASS_IS(FULL_EXECUTION), CURRENT(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT), STATE_CREATED(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT, EXPLORER_VALIDATED), PACKET_BINDS(GENERIC_PACKET_REF, EXPLORER_ARTIFACT), CURRENT(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN), STATE_CREATED(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN, PLAN_VALIDATED), PACKET_BINDS(GENERIC_PACKET_REF, IMPLEMENTATION_PLAN)), AND(PACKET_CLASS_IS(LIGHTWEIGHT_WITH_PLANNER), CURRENT(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT), STATE_CREATED(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT, EXPLORER_VALIDATED), PACKET_BINDS(GENERIC_PACKET_REF, EXPLORER_ARTIFACT), CURRENT(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN), STATE_CREATED(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN, PLAN_VALIDATED), PACKET_BINDS(GENERIC_PACKET_REF, IMPLEMENTATION_PLAN)), AND(PACKET_CLASS_IS(LIGHTWEIGHT_DIRECT_TO_IMPLEMENTER)), AND(PACKET_CLASS_IS(VERSION_GOVERNANCE), PACKET_BINDS(GENERIC_PACKET_REF, VERSION_GOVERNANCE_SCOPE_PACKET)), AND(PACKET_CLASS_IS(DRAFT_TO_READY_APPROVAL), PACKET_BINDS(GENERIC_PACKET_REF, DRAFT_TO_READY_APPROVAL_PACKET)), AND(PACKET_CLASS_IS(MERGE_APPROVAL), PACKET_BINDS(GENERIC_PACKET_REF, MERGE_APPROVAL_PACKET)), AND(PACKET_CLASS_IS(BRANCH_DELETION_APPROVAL), PACKET_BINDS(GENERIC_PACKET_REF, BRANCH_DELETION_APPROVAL_PACKET)), AND(PACKET_CLASS_IS(CONSULTATION_SOURCE), PACKET_BINDS(GENERIC_PACKET_REF, CONSULTATION_SOURCE_PACKET)))) | DIFF_MANIFEST, TREE_MANIFEST | ASSEMBLED | PACKET_ASSEMBLED |
| `PACKET_FREEZE_RECEIPT` | vibedev/orchestrator | Packet assembly confirmation; freezes the same Packet for its stage operation |GENERIC_PACKET_REF triple (current row's bound Packet)|`GENERIC_PACKET_REF`, `PACKET_OPERATION_REF`|`PACKET_ASSEMBLY_RECEIPT`| AND(CURRENT(PACKET_ASSEMBLY_RECEIPT, GENERIC_PACKET_REF), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, GENERIC_PACKET_REF, PACKET_ASSEMBLED), ROW_SUBJECT_IS(GENERIC_PACKET_REF), ROW_OPERATION_IS(PACKET_OPERATION_REF)) | FILE_SNAPSHOT, GIT_OBJECT | FROZEN | PACKET_FROZEN |
| `PACKET_VALIDATION_RECEIPT` | vibedev/orchestrator | Packet freeze confirmation; validates the same Packet for its stage operation |GENERIC_PACKET_REF triple (current row's bound Packet)|`GENERIC_PACKET_REF`, `PACKET_OPERATION_REF`|`PACKET_FREEZE_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, GENERIC_PACKET_REF), STATE_CREATED(PACKET_FREEZE_RECEIPT, GENERIC_PACKET_REF, PACKET_FROZEN), ROW_SUBJECT_IS(GENERIC_PACKET_REF), ROW_OPERATION_IS(PACKET_OPERATION_REF)) | DIFF_MANIFEST, TREE_MANIFEST, TEST_RESULT | VALIDATED / INVALID | PACKET_VALIDATED / PACKET_VALIDATION_FAILED |
| `PACKET_CONSUMPTION_RECEIPT` | consumer role/stage controller (vibedev/orchestrator) | consumes the frozen (optionally validated) Packet for the downstream stage; Packet consumption is bound to the consumer's stage operation |GENERIC_PACKET_REF triple (current row's bound Packet)|`GENERIC_PACKET_REF`, `PACKET_OPERATION_REF`|`PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, GENERIC_PACKET_REF), STATE_CREATED(PACKET_FREEZE_RECEIPT, GENERIC_PACKET_REF, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, GENERIC_PACKET_REF), STATE_CREATED(PACKET_VALIDATION_RECEIPT, GENERIC_PACKET_REF, PACKET_VALIDATED), ROW_SUBJECT_IS(GENERIC_PACKET_REF), ROW_OPERATION_IS(PACKET_OPERATION_REF)) | CONSUMER_EVIDENCE | CONSUMED | PACKET_CONSUMED |
| `TEST_GATE_PASS_RECEIPT` | vibedev/orchestrator | tester-a + tester-b independent cross-verification on the same CANDIDATE_TRIPLE, input Packet freeze, stage operation, and role/model invocation refs |TEST_EVIDENCE_PACKET triple|`TEST_EVIDENCE_PACKET`, `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, `CANDIDATE_TRIPLE`, `TESTER_A_INVOCATION_REF`, `TESTER_B_INVOCATION_REF`, `TESTER_A_CROSS_VERIFICATION_REF`, `TESTER_B_CROSS_VERIFICATION_REF`, `TESTER_A_COMPLETION_REPORT`, `TESTER_B_COMPLETION_REPORT`, `TEST_OPERATION_REF`|`ROLE_CROSS_VERIFICATION_RECEIPT`, `PACKET_FREEZE_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, TEST_EVIDENCE_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TEST_EVIDENCE_PACKET, PACKET_FROZEN), CURRENT(PACKET_FREEZE_RECEIPT, TESTER_A_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TESTER_A_INPUT_PACKET, PACKET_FROZEN), CURRENT(PACKET_FREEZE_RECEIPT, TESTER_B_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TESTER_B_INPUT_PACKET, PACKET_FROZEN), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, TESTER_A_COMPLETION_REPORT), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, TESTER_B_COMPLETION_REPORT), PACKET_BINDS(TEST_EVIDENCE_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TESTER_A_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TESTER_B_INPUT_PACKET, CANDIDATE_TRIPLE), STAGE_IS(TEST), ROW_OPERATION_IS(TEST_OPERATION_REF), OPERATION_MATCH(TEST_EVIDENCE_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_INPUT_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_INPUT_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_CROSS_VERIFICATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_CROSS_VERIFICATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_COMPLETION_REPORT, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_COMPLETION_REPORT, TEST_OPERATION_REF)) | TEST_RESULT, MODEL_INVOCATION_RECORD | PASS | GATE_PASS |
| `TEST_GATE_FAIL_RECEIPT` | vibedev/orchestrator | at least one of tester-a / tester-b reports an instance-level INCOMPLETE or REJECTED on the same CANDIDATE_TRIPLE, with the other tester explicitly resolved |TEST_EVIDENCE_PACKET triple|`TEST_EVIDENCE_PACKET`, `TESTER_A_INPUT_PACKET`, `TESTER_B_INPUT_PACKET`, `CANDIDATE_TRIPLE`, `TESTER_A_INVOCATION_REF`, `TESTER_B_INVOCATION_REF`, `TESTER_A_CROSS_VERIFICATION_REF`, `TESTER_B_CROSS_VERIFICATION_REF`, `TESTER_A_COMPLETION_REPORT`, `TESTER_B_COMPLETION_REPORT`, `TEST_OPERATION_REF`|`ROLE_CROSS_VERIFICATION_RECEIPT`, `PACKET_FREEZE_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, TEST_EVIDENCE_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TEST_EVIDENCE_PACKET, PACKET_FROZEN), CURRENT(PACKET_FREEZE_RECEIPT, TESTER_A_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TESTER_A_INPUT_PACKET, PACKET_FROZEN), CURRENT(PACKET_FREEZE_RECEIPT, TESTER_B_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, TESTER_B_INPUT_PACKET, PACKET_FROZEN), EXACTLY_ONE_OF(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED)), EXACTLY_ONE_OF(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED)), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, TESTER_A_COMPLETION_REPORT), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, TESTER_B_COMPLETION_REPORT), PACKET_BINDS(TEST_EVIDENCE_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TESTER_A_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TESTER_B_INPUT_PACKET, CANDIDATE_TRIPLE), STAGE_IS(TEST), ROW_OPERATION_IS(TEST_OPERATION_REF), OPERATION_MATCH(TEST_EVIDENCE_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_INPUT_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_INPUT_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_INVOCATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_CROSS_VERIFICATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_CROSS_VERIFICATION_REF, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_A_COMPLETION_REPORT, TEST_OPERATION_REF), OPERATION_MATCH(TESTER_B_COMPLETION_REPORT, TEST_OPERATION_REF), OR(AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, INCOMPLETE)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_CROSS_VERIFICATION_REF, REJECTED)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, INCOMPLETE)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_B_CROSS_VERIFICATION_REF, REJECTED)))) | TEST_RESULT, MODEL_INVOCATION_RECORD | FAIL | GATE_FAIL |
| `REVIEW_GATE_PASS_RECEIPT` | vibedev/orchestrator | reviewer-a + reviewer-b independent cross-verification on the same frozen REVIEW_INPUT_PACKET and CANDIDATE_TRIPLE, with prior TEST_GATE_PASS_RECEIPT for the same stage operation, plus role/model invocation refs |REVIEW_INPUT_PACKET triple|`REVIEW_INPUT_PACKET`, `TEST_EVIDENCE_PACKET`, `CANDIDATE_TRIPLE`, `REVIEWER_A_INVOCATION_REF`, `REVIEWER_B_INVOCATION_REF`, `REVIEWER_A_CROSS_VERIFICATION_REF`, `REVIEWER_B_CROSS_VERIFICATION_REF`, `REVIEWER_A_COMPLETION_REPORT`, `REVIEWER_B_COMPLETION_REPORT`, `REVIEW_OPERATION_REF`, `TEST_OPERATION_REF`|`ROLE_CROSS_VERIFICATION_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `TEST_GATE_PASS_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, REVIEW_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, REVIEW_INPUT_PACKET, PACKET_FROZEN), CURRENT(TEST_GATE_PASS_RECEIPT, TEST_EVIDENCE_PACKET), STATE_CREATED(TEST_GATE_PASS_RECEIPT, TEST_EVIDENCE_PACKET, GATE_PASS), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, REVIEWER_A_COMPLETION_REPORT), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, REVIEWER_B_COMPLETION_REPORT), PACKET_BINDS(REVIEW_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TEST_EVIDENCE_PACKET, CANDIDATE_TRIPLE), STAGE_IS(REVIEW), ROW_OPERATION_IS(REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEW_INPUT_PACKET, REVIEW_OPERATION_REF), OPERATION_MATCH(TEST_EVIDENCE_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_COMPLETION_REPORT, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_COMPLETION_REPORT, REVIEW_OPERATION_REF)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS | GATE_PASS |
| `REVIEW_GATE_FAIL_RECEIPT` | vibedev/orchestrator | at least one of reviewer-a / reviewer-b reports an instance-level INCOMPLETE or REJECTED on the same REVIEW_INPUT_PACKET and CANDIDATE_TRIPLE, with the other reviewer explicitly resolved; TEST_GATE_PASS remains valid |REVIEW_INPUT_PACKET triple|`REVIEW_INPUT_PACKET`, `TEST_EVIDENCE_PACKET`, `CANDIDATE_TRIPLE`, `REVIEWER_A_INVOCATION_REF`, `REVIEWER_B_INVOCATION_REF`, `REVIEWER_A_CROSS_VERIFICATION_REF`, `REVIEWER_B_CROSS_VERIFICATION_REF`, `REVIEWER_A_COMPLETION_REPORT`, `REVIEWER_B_COMPLETION_REPORT`, `REVIEW_OPERATION_REF`, `TEST_OPERATION_REF`|`ROLE_CROSS_VERIFICATION_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `TEST_GATE_PASS_RECEIPT`| AND(CURRENT(PACKET_FREEZE_RECEIPT, REVIEW_INPUT_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, REVIEW_INPUT_PACKET, PACKET_FROZEN), CURRENT(TEST_GATE_PASS_RECEIPT, TEST_EVIDENCE_PACKET), STATE_CREATED(TEST_GATE_PASS_RECEIPT, TEST_EVIDENCE_PACKET, GATE_PASS), EXACTLY_ONE_OF(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED)), EXACTLY_ONE_OF(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED)), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, REVIEWER_A_COMPLETION_REPORT), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, REVIEWER_B_COMPLETION_REPORT), PACKET_BINDS(REVIEW_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(TEST_EVIDENCE_PACKET, CANDIDATE_TRIPLE), STAGE_IS(REVIEW), ROW_OPERATION_IS(REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEW_INPUT_PACKET, REVIEW_OPERATION_REF), OPERATION_MATCH(TEST_EVIDENCE_PACKET, TEST_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_INVOCATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_CROSS_VERIFICATION_REF, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_A_COMPLETION_REPORT, REVIEW_OPERATION_REF), OPERATION_MATCH(REVIEWER_B_COMPLETION_REPORT, REVIEW_OPERATION_REF), OR(AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, INCOMPLETE)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_A_CROSS_VERIFICATION_REF, REJECTED)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, INCOMPLETE)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, REVIEWER_B_CROSS_VERIFICATION_REF, REJECTED)))) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | FAIL | GATE_FAIL |
| `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | git-integrator | pre-Draft git-integrator binding for Git Integration stage + Draft PR delivery |GIT_INTEGRATOR_ACTIVATION_EVIDENCE|`PRE_DRAFT_GIT_BINDING_REF`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE`, `GIT_OPERATION_REF`|`OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`, `ROLE_ACTIVATION_READINESS_RECEIPT`| AND(ROW_INSTANCE_IS(PRE_DRAFT_GIT_BINDING_REF), ROW_SUBJECT_IS(GIT_INTEGRATOR_ACTIVATION_EVIDENCE), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF, ROLE_ACTIVATION_READY), OPERATION_MATCH(PRE_DRAFT_GIT_BINDING_REF, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE, GIT_OPERATION_REF)) |GIT_OBJECT, PR_METADATA_SNAPSHOT, MODEL_INVOCATION_RECORD|BOUND / UNBOUND|GIT_INTEGRATOR_PRE_DRAFT_BOUND / GIT_INTEGRATOR_PRE_DRAFT_UNBOUND|
|`GIT_INTEGRATION_GATE_PASS_RECEIPT`|vibedev/orchestrator|REVIEW_GATE_PASS_RECEIPT for the same Git Integration operation + Integration Packet assembly + freeze + validation + PRE_DRAFT_GIT_INTEGRATOR_BINDING, where the canonical row subject is the Git Integration Packet (`INTEGRATION_INPUT_FROZEN`)|INTEGRATION_INPUT_FROZEN (the Git Integration Packet)|`INTEGRATION_INPUT_FROZEN`, `CANDIDATE_TRIPLE`, `REVIEW_INPUT_PACKET`, `TEST_EVIDENCE_PACKET`, `GIT_OPERATION_REF`, `GIT_INTEGRATOR_ROLE_TRIPLE`, `GIT_CROSS_VERIFICATION_REF`, `GIT_INTEGRATION_COMPLETION_REPORT`, `PRE_DRAFT_GIT_BINDING_REF`|`REVIEW_GATE_PASS_RECEIPT`, `PACKET_ASSEMBLY_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`, `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT`, `ARTIFACT_VALIDATION_RECEIPT`, `ROLE_CROSS_VERIFICATION_RECEIPT`| AND(CURRENT(REVIEW_GATE_PASS_RECEIPT, REVIEW_INPUT_PACKET), STATE_CREATED(REVIEW_GATE_PASS_RECEIPT, REVIEW_INPUT_PACKET, GATE_PASS), CURRENT(PACKET_ASSEMBLY_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_ASSEMBLED), CURRENT(PACKET_FREEZE_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_FREEZE_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_VALIDATION_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_VALIDATED), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF), STATE_CREATED(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_PRE_DRAFT_BOUND), INSTANCE_SUBJECT_MATCH(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE), CURRENT(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE), STATE_CREATED(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE, ARTIFACT_VALIDATED), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, GIT_INTEGRATION_COMPLETION_REPORT), PACKET_BINDS(REVIEW_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(INTEGRATION_INPUT_FROZEN, CANDIDATE_TRIPLE), STAGE_IS(GIT_INTEGRATION), ROW_SUBJECT_IS(INTEGRATION_INPUT_FROZEN), ROW_OPERATION_IS(GIT_OPERATION_REF), OPERATION_MATCH(INTEGRATION_INPUT_FROZEN, GIT_OPERATION_REF), OPERATION_MATCH(REVIEW_INPUT_PACKET, REVIEW_OPERATION_REF), OPERATION_MATCH(GIT_CROSS_VERIFICATION_REF, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ROLE_TRIPLE, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATION_COMPLETION_REPORT, GIT_OPERATION_REF), OPERATION_MATCH(PRE_DRAFT_GIT_BINDING_REF, GIT_OPERATION_REF)) |GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT|PASS|GATE_PASS|
|`GIT_INTEGRATION_GATE_FAIL_RECEIPT`|vibedev/orchestrator|integration-stage Evidence of stop or rejection on the same Git Integration operation; the canonical row subject remains the Git Integration Packet (`INTEGRATION_INPUT_FROZEN`)|INTEGRATION_INPUT_FROZEN (the Git Integration Packet)|`INTEGRATION_INPUT_FROZEN`, `CANDIDATE_TRIPLE`, `REVIEW_INPUT_PACKET`, `TEST_EVIDENCE_PACKET`, `GIT_OPERATION_REF`, `GIT_INTEGRATOR_ROLE_TRIPLE`, `GIT_CROSS_VERIFICATION_REF`, `GIT_INTEGRATION_COMPLETION_REPORT`, `PRE_DRAFT_GIT_BINDING_REF`|`ARTIFACT_VALIDATION_RECEIPT`, `PACKET_ASSEMBLY_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`, `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT`, `REVIEW_GATE_PASS_RECEIPT`, `ROLE_CROSS_VERIFICATION_RECEIPT`| AND(CURRENT(REVIEW_GATE_PASS_RECEIPT, REVIEW_INPUT_PACKET), STATE_CREATED(REVIEW_GATE_PASS_RECEIPT, REVIEW_INPUT_PACKET, GATE_PASS), CURRENT(PACKET_ASSEMBLY_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_ASSEMBLED), CURRENT(PACKET_FREEZE_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_FREEZE_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(PACKET_VALIDATION_RECEIPT, INTEGRATION_INPUT_FROZEN, PACKET_VALIDATED), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF), STATE_CREATED(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_PRE_DRAFT_BOUND), INSTANCE_SUBJECT_MATCH(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE), PACKET_BINDS(REVIEW_INPUT_PACKET, CANDIDATE_TRIPLE), PACKET_BINDS(INTEGRATION_INPUT_FROZEN, CANDIDATE_TRIPLE), STAGE_IS(GIT_INTEGRATION), ROW_SUBJECT_IS(INTEGRATION_INPUT_FROZEN), ROW_OPERATION_IS(GIT_OPERATION_REF), OPERATION_MATCH(INTEGRATION_INPUT_FROZEN, GIT_OPERATION_REF), OPERATION_MATCH(REVIEW_INPUT_PACKET, REVIEW_OPERATION_REF), OPERATION_MATCH(GIT_CROSS_VERIFICATION_REF, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ROLE_TRIPLE, GIT_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATION_COMPLETION_REPORT, GIT_OPERATION_REF), OPERATION_MATCH(PRE_DRAFT_GIT_BINDING_REF, GIT_OPERATION_REF), EXACTLY_ONE_OF(STATE_CREATED(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE, ARTIFACT_VALIDATED), STATE_CREATED(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE, ARTIFACT_VALIDATION_INVALID)), EXACTLY_ONE_OF(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED)), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, GIT_INTEGRATION_COMPLETION_REPORT), OR(AND(STATE_CREATED(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE, ARTIFACT_VALIDATION_INVALID), DECISION(ARTIFACT_VALIDATION_RECEIPT, CANDIDATE_TRIPLE, INVALID)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_INCOMPLETE), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, INCOMPLETE)), AND(STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFICATION_REJECTED), DECISION(ROLE_CROSS_VERIFICATION_RECEIPT, GIT_CROSS_VERIFICATION_REF, REJECTED)))) |GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT|FAIL|GATE_FAIL|
|`DRAFT_PR_DELIVERY_RECEIPT`|git-integrator|GIT_INTEGRATION_GATE_PASS_RECEIPT + CANDIDATE_TRIPLE + PR|PR_TRIPLE|`PR_TRIPLE`, `GIT_OPERATION_REF`, `PRE_DRAFT_GIT_BINDING_REF`|`GIT_INTEGRATION_GATE_PASS_RECEIPT`, `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT`| AND(CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN, GATE_PASS), CURRENT(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF), STATE_CREATED(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_PRE_DRAFT_BOUND), INSTANCE_SUBJECT_MATCH(PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT, PRE_DRAFT_GIT_BINDING_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE), ROW_OPERATION_IS(GIT_OPERATION_REF)) |GIT_OBJECT, PR_METADATA_SNAPSHOT|DELIVERED / FAILED|DRAFT_DELIVERED / DRAFT_DELIVERY_FAILED|
| `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization for Draft→Ready operation) | stage-specific post-Draft binding for Draft→Ready operation |DRAFT_TO_READY_APPROVAL_PACKET triple|`DRAFT_TO_READY_APPROVAL_PACKET`, `DRAFT_TO_READY_OPERATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY`, `PR_TRIPLE`|`OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `DRAFT_PR_DELIVERY_RECEIPT`, `ROLE_ACTIVATION_READINESS_RECEIPT`| AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, OPERATOR_AUTHORIZED_DRAFT_TO_READY), CURRENT(DRAFT_PR_DELIVERY_RECEIPT, PR_TRIPLE), STATE_CREATED(DRAFT_PR_DELIVERY_RECEIPT, PR_TRIPLE, DRAFT_DELIVERED), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY), PACKET_BINDS(DRAFT_TO_READY_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(DRAFT_TO_READY_APPROVAL_PACKET, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(DRAFT_TO_READY_INVOCATION_REF, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY, DRAFT_TO_READY_OPERATION_REF)) | MODEL_INVOCATION_RECORD (stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_DRAFT_TO_READY_BOUND / GIT_INTEGRATOR_DRAFT_TO_READY_UNBOUND |
| `READY_TRANSITION_RECEIPT` | vibedev/orchestrator | operator Draft→Ready authorization + DRAFT_PR_DELIVERY + DRAFT_TO_READY_APPROVAL_PACKET assembly + freeze + validation + POST_DRAFT_DRAFT_TO_READY_BINDING + git-integrator completion Evidence + cross-verification + STAGE_IS(DRAFT_TO_READY) (`BRANCH_DELETION_RECEIPT` is terminal post-BD cleanup) |PR_TRIPLE|`PR_TRIPLE`, `DRAFT_TO_READY_APPROVAL_PACKET`, `DRAFT_TO_READY_OPERATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `READY_CROSS_VERIFICATION_REF`, `READY_TRANSITION_COMPLETION_REPORT`|`OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `DRAFT_PR_DELIVERY_RECEIPT`, `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT`, `PACKET_ASSEMBLY_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`, `ROLE_CROSS_VERIFICATION_RECEIPT`| AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, OPERATOR_AUTHORIZED_DRAFT_TO_READY), CURRENT(DRAFT_PR_DELIVERY_RECEIPT, PR_TRIPLE), STATE_CREATED(DRAFT_PR_DELIVERY_RECEIPT, PR_TRIPLE, DRAFT_DELIVERED), CURRENT(POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, GIT_INTEGRATOR_DRAFT_TO_READY_BOUND), CURRENT(PACKET_ASSEMBLY_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, PACKET_ASSEMBLED), CURRENT(PACKET_FREEZE_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(PACKET_VALIDATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, PACKET_VALIDATED), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, READY_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, READY_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, READY_CROSS_VERIFICATION_REF, READY_TRANSITION_COMPLETION_REPORT), PACKET_BINDS(DRAFT_TO_READY_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(DRAFT_TO_READY_APPROVAL_PACKET, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(DRAFT_TO_READY_INVOCATION_REF, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(READY_CROSS_VERIFICATION_REF, DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(READY_TRANSITION_COMPLETION_REPORT, DRAFT_TO_READY_OPERATION_REF)) | PR_METADATA_SNAPSHOT, OPERATOR_DECISION_RECORD | READY / BLOCKED | PR_READY / PR_READY_BLOCKED |
| `POST_DRAFT_MERGE_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization for Merge operation) | stage-specific post-Draft binding for Merge operation |MERGE_APPROVAL_PACKET triple|`MERGE_APPROVAL_PACKET`, `MERGE_OPERATION_REF`, `MERGE_INVOCATION_REF`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE`, `PR_TRIPLE`|`OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT`, `ROLE_ACTIVATION_READINESS_RECEIPT`| AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET, OPERATOR_AUTHORIZED_MERGE), CURRENT(READY_TRANSITION_RECEIPT, PR_TRIPLE), STATE_CREATED(READY_TRANSITION_RECEIPT, PR_TRIPLE, PR_READY), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE), PACKET_BINDS(MERGE_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_APPROVAL_PACKET, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_INVOCATION_REF, MERGE_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE, MERGE_OPERATION_REF)) | MODEL_INVOCATION_RECORD (stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_MERGE_BOUND / GIT_INTEGRATOR_MERGE_UNBOUND |
| `MERGE_DELIVERY_RECEIPT` | git-integrator | operator Merge authorization + READY_TRANSITION_RECEIPT + MERGE_APPROVAL_PACKET assembly + freeze + validation + POST_DRAFT_MERGE_BINDING + git-integrator completion Evidence + STAGE_IS(MERGE) |PR_TRIPLE|`PR_TRIPLE`, `MERGE_APPROVAL_PACKET`, `MERGE_OPERATION_REF`, `MERGE_INVOCATION_REF`, `MERGE_CROSS_VERIFICATION_REF`, `MERGE_COMPLETION_REPORT`|`OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT`, `POST_DRAFT_MERGE_BINDING_RECEIPT`, `PACKET_ASSEMBLY_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`, `ROLE_CROSS_VERIFICATION_RECEIPT`| AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET, OPERATOR_AUTHORIZED_MERGE), CURRENT(READY_TRANSITION_RECEIPT, PR_TRIPLE), STATE_CREATED(READY_TRANSITION_RECEIPT, PR_TRIPLE, PR_READY), CURRENT(POST_DRAFT_MERGE_BINDING_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(POST_DRAFT_MERGE_BINDING_RECEIPT, MERGE_APPROVAL_PACKET, GIT_INTEGRATOR_MERGE_BOUND), CURRENT(PACKET_ASSEMBLY_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, MERGE_APPROVAL_PACKET, PACKET_ASSEMBLED), CURRENT(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(PACKET_VALIDATION_RECEIPT, MERGE_APPROVAL_PACKET, PACKET_VALIDATED), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, MERGE_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, MERGE_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, MERGE_CROSS_VERIFICATION_REF, MERGE_COMPLETION_REPORT), PACKET_BINDS(MERGE_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_APPROVAL_PACKET, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_INVOCATION_REF, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_CROSS_VERIFICATION_REF, MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_COMPLETION_REPORT, MERGE_OPERATION_REF)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | MERGED / FAILED | PR_MERGED / MERGE_FAILED |
| `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` | git-integrator (per-stage, post-Draft re-authorization for Branch Deletion operation) | stage-specific post-Draft binding for Branch Deletion operation |BRANCH_DELETION_APPROVAL_PACKET triple|`BRANCH_DELETION_APPROVAL_PACKET`, `BRANCH_DELETION_OPERATION_REF`, `BRANCH_DELETION_INVOCATION_REF`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD`, `BRANCH_TRIPLE`|`OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT`, `ROLE_ACTIVATION_READINESS_RECEIPT`| AND(CURRENT(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, OPERATOR_AUTHORIZED_BRANCH_DELETION), CURRENT(MERGE_DELIVERY_RECEIPT, PR_TRIPLE), STATE_CREATED(MERGE_DELIVERY_RECEIPT, PR_TRIPLE, PR_MERGED), CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD), PACKET_BINDS(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_TRIPLE), ROW_OPERATION_IS(BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_INVOCATION_REF, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD, BRANCH_DELETION_OPERATION_REF)) | MODEL_INVOCATION_RECORD (stage-specific), GIT_OBJECT, OPERATOR_DECISION_RECORD | BOUND / UNBOUND | GIT_INTEGRATOR_BRANCH_DELETION_BOUND / GIT_INTEGRATOR_BRANCH_DELETION_UNBOUND |
| `BRANCH_DELETION_RECEIPT` | git-integrator | operator Branch Deletion authorization + MERGE_DELIVERY + BRANCH_DELETION_APPROVAL_PACKET assembly + freeze + validation + POST_DRAFT_BRANCH_DELETION_BINDING + git-integrator completion Evidence + STAGE_IS(BRANCH_DELETION) |BRANCH_TRIPLE|`BRANCH_TRIPLE`, `PR_TRIPLE`, `BRANCH_DELETION_APPROVAL_PACKET`, `BRANCH_DELETION_OPERATION_REF`, `BRANCH_DELETION_INVOCATION_REF`, `BD_CROSS_VERIFICATION_REF`, `BRANCH_DELETION_COMPLETION_REPORT`|`OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT`, `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT`, `PACKET_ASSEMBLY_RECEIPT`, `PACKET_FREEZE_RECEIPT`, `PACKET_VALIDATION_RECEIPT`, `ROLE_CROSS_VERIFICATION_RECEIPT`| AND(CURRENT(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, OPERATOR_AUTHORIZED_BRANCH_DELETION), CURRENT(MERGE_DELIVERY_RECEIPT, PR_TRIPLE), STATE_CREATED(MERGE_DELIVERY_RECEIPT, PR_TRIPLE, PR_MERGED), CURRENT(POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, GIT_INTEGRATOR_BRANCH_DELETION_BOUND), CURRENT(PACKET_ASSEMBLY_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(PACKET_ASSEMBLY_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, PACKET_ASSEMBLED), CURRENT(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, PACKET_FROZEN), CURRENT(PACKET_VALIDATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(PACKET_VALIDATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, PACKET_VALIDATED), CURRENT(ROLE_CROSS_VERIFICATION_RECEIPT, BD_CROSS_VERIFICATION_REF), STATE_CREATED(ROLE_CROSS_VERIFICATION_RECEIPT, BD_CROSS_VERIFICATION_REF, ROLE_CROSS_VERIFIED), INSTANCE_SUBJECT_MATCH(ROLE_CROSS_VERIFICATION_RECEIPT, BD_CROSS_VERIFICATION_REF, BRANCH_DELETION_COMPLETION_REPORT), BRANCH_OF_PR(BRANCH_TRIPLE, PR_TRIPLE), PACKET_BINDS(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_TRIPLE), ROW_OPERATION_IS(BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_INVOCATION_REF, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BD_CROSS_VERIFICATION_REF, BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_COMPLETION_REPORT, BRANCH_DELETION_OPERATION_REF)) | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELETED / FAILED | BRANCH_DELETED / BRANCH_DELETION_FAILED |
| `CONSULTATION_ENDPOINT_RECEIPT` | vibedev/orchestrator | consultation scope + Work Order + Consultation Work Order (no role assignment) | CONSULTATION_DELIVERABLE triple | `CONSULTATION_SOURCE_PACKET`, `WORK_ORDER_DECLARATION` | `GLOBAL_READINESS_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(ROW_INSTANCE_IS(CONSULTATION_SOURCE_PACKET), ROW_SUBJECT_IS(CONSULTATION_SOURCE_PACKET), CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), STATE_CREATED(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET, OPERATOR_APPROVED_INTAKE_SCOPE), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER)) | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | DELIVERED / FAILED | CONSULTATION_DELIVERED / CONSULTATION_FAILED |
|`PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT`|vibedev/orchestrator (public-safe verification function)|PUBLIC_EVIDENCE_PROJECTION_ARTIFACT + source Evidence|PUBLIC_EVIDENCE_PROJECTION_ARTIFACT triple|`SOURCE_EVIDENCE_OBJECTS`|`ARTIFACT_VALIDATION_RECEIPT`| AND(ROW_INSTANCE_IS(SOURCE_EVIDENCE_OBJECTS), ROW_SUBJECT_IS(SUBJECT_ARTIFACT_TRIPLE), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER)) |FILE_SNAPSHOT, OPERATOR_DECISION_RECORD|SAFE / UNSAFE|PROJECTION_SAFE / PROJECTION_UNSAFE|
| `STOP_RECEIPT` | vibedev/orchestrator (only) | STOP_REPORT Artifact + Evidence; or hard STOP signal; or operator instruction (`TEST_GATE_FAIL_RECEIPT`, `REVIEW_GATE_FAIL_RECEIPT`, `GIT_INTEGRATION_GATE_FAIL_RECEIPT` trigger STOP) | subject STOP reason triple | `STOP_REPORT` | `ARTIFACT_VALIDATION_RECEIPT` | AND(ROW_INSTANCE_IS(STOP_REPORT), ROW_SUBJECT_IS(STOP_REPORT), CURRENT(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE), STATE_CREATED(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE, AUTHORISATION_REVOKED)) | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD, STOP signal catalog entry | STOPPED | TASK_STOPPED / WORKFLOW_STOPPED |
| `INVALIDATION_RECEIPT` | vibedev/orchestrator | Evidence of invalidation condition | single Artifact, Evidence, Packet, or Receipt triple | `SUBJECT_TRIPLE` | `ARTIFACT_VALIDATION_RECEIPT` | AND(ROW_INSTANCE_IS(SUBJECT_ARTIFACT_TRIPLE), ROW_SUBJECT_IS(SUBJECT_ARTIFACT_TRIPLE), CURRENT(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE), STATE_CREATED(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE, AUTHORISATION_REVOKED)) | COMMAND_RESULT, MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | INVALIDATED | SUBJECT_INVALIDATED |
| `SUPERSESSION_RECEIPT` | vibedev/orchestrator | Evidence of supersession + superseding-object Receipt triple (canonical object reference; uses `STATE_CREATED(SUPERSESSION_RECEIPT, SUPERSEDED)` downstream) | single Artifact, Evidence, Packet, or Receipt triple | `SUPERSEDING_OBJECT_TRIPLE` | `ARTIFACT_VALIDATION_RECEIPT` | AND(ROW_INSTANCE_IS(SUPERSEDING_OBJECT_TRIPLE), ROW_SUBJECT_IS(SUPERSEDING_OBJECT_TRIPLE), CURRENT(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE), STATE_CREATED(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE, AUTHORISATION_REVOKED), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER)) | FILE_SNAPSHOT, GIT_OBJECT | SUPERSEDED | OBJECT_SUPERSEDED |
| `OPERATOR_REVOCATION_RECEIPT` | vibedev/orchestrator | revocation of an active Receipt (target identity, type, and instance ref all preserved; revocation does NOT change target's canonical instance key) | AUTHORISATION_RECEIPT_TRIPLE | `AUTHORISATION_RECEIPT_TRIPLE`, `DRAFT_TO_READY_APPROVAL_PACKET`, `MERGE_APPROVAL_PACKET`, `BRANCH_DELETION_APPROVAL_PACKET`, `WORK_ORDER_DECLARATION`, `INTAKE_SCOPE_PACKET` | `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT`, `GLOBAL_READINESS_RECEIPT`, `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT`, `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | AND(ROW_SUBJECT_IS(AUTHORISATION_RECEIPT_TRIPLE), EXACTLY_ONE_OF(AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET)), AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET)), AND(CURRENT(OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET)), AND(CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION)), AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION)), AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), REVOCATION_TARGET_MATCH(AUTHORISATION_RECEIPT_TRIPLE, OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET)))) | OPERATOR_DECISION_RECORD | REVOKED | AUTHORISATION_REVOKED |
...
| `OPERATOR_SUBMODE_SELECTION_RECEIPT` | operator (only) | operator authority | `SUBMODE_DECLARATION` triple | `SUBMODE_DECLARATION` | `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | AND(ROW_INSTANCE_IS(SUBMODE_DECLARATION), ROW_SUBJECT_IS(SUBMODE_DECLARATION), CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), STATE_CREATED(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET, OPERATOR_APPROVED_INTAKE_SCOPE)) | OPERATOR_DECISION_RECORD | SELECTED | OPERATOR_SELECTED_SUBMODE |
| `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT` | operator (only) | operator authority | assignment baseline packet triple | `ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET` | `OPERATOR_SUBMODE_SELECTION_RECEIPT` | AND(ROW_INSTANCE_IS(ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), ROW_SUBJECT_IS(ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER), PACKET_BINDS(ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, WORK_ORDER_DECLARATION)) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT_BASELINE |
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | operator (only) | operator authority |work-order triple|`WORK_ORDER_DECLARATION`|`OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT`, `OPERATOR_SUBMODE_SELECTION_RECEIPT`| AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), STATE_CREATED(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET, OPERATOR_APPROVED_INTAKE_SCOPE), CURRENT(OPERATOR_SUBMODE_SELECTION_RECEIPT, SUBMODE_DECLARATION), STATE_CREATED(OPERATOR_SUBMODE_SELECTION_RECEIPT, SUBMODE_DECLARATION, SUBMODE_SELECTED)) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_WORK_ORDER |
| `OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT` | operator (only) | operator authority | implementation plan approval packet triple | `IMPLEMENTATION_PLAN_APPROVAL_PACKET` | `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | AND(ROW_INSTANCE_IS(IMPLEMENTATION_PLAN), ROW_SUBJECT_IS(IMPLEMENTATION_PLAN), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), CURRENT(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT), STATE_CREATED(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT, EXPLORER_VALIDATED), CURRENT(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN), STATE_CREATED(PLAN_VALIDATION_RECEIPT, IMPLEMENTATION_PLAN, PLAN_VALIDATED)) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_IMPLEMENTATION_PLAN |
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | operator (only, FULL/LIGHTWEIGHT only — not Consultation) | operator authority |DRAFT_TO_READY_APPROVAL_PACKET triple|`DRAFT_TO_READY_APPROVAL_PACKET`, `DRAFT_TO_READY_OPERATION_REF`, `PR_TRIPLE`|`OPERATOR_WORK_ORDER_APPROVAL_RECEIPT`, `GIT_INTEGRATION_GATE_PASS_RECEIPT`| AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, WORK_ORDER_DECLARATION, OPERATOR_APPROVED_WORK_ORDER), CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN, GATE_PASS), PACKET_BINDS(DRAFT_TO_READY_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(DRAFT_TO_READY_OPERATION_REF), OPERATION_MATCH(DRAFT_TO_READY_APPROVAL_PACKET, DRAFT_TO_READY_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_DRAFT_TO_READY |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | operator (only) | operator authority |MERGE_APPROVAL_PACKET triple|`MERGE_APPROVAL_PACKET`, `MERGE_OPERATION_REF`, `PR_TRIPLE`|`OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`, `READY_TRANSITION_RECEIPT`| AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, OPERATOR_AUTHORIZED_DRAFT_TO_READY), CURRENT(READY_TRANSITION_RECEIPT, PR_TRIPLE), STATE_CREATED(READY_TRANSITION_RECEIPT, PR_TRIPLE, PR_READY), CURRENT(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET, PACKET_FROZEN), PACKET_BINDS(MERGE_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(MERGE_OPERATION_REF), OPERATION_MATCH(MERGE_APPROVAL_PACKET, MERGE_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_MERGE |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | operator (only) | operator authority |BRANCH_DELETION_APPROVAL_PACKET triple|`BRANCH_DELETION_APPROVAL_PACKET`, `BRANCH_DELETION_OPERATION_REF`, `BRANCH_TRIPLE`|`OPERATOR_MERGE_AUTHORIZATION_RECEIPT`, `MERGE_DELIVERY_RECEIPT`| AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET, OPERATOR_AUTHORIZED_MERGE), CURRENT(MERGE_DELIVERY_RECEIPT, PR_TRIPLE), STATE_CREATED(MERGE_DELIVERY_RECEIPT, PR_TRIPLE, PR_MERGED), CURRENT(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, PACKET_FROZEN), PACKET_BINDS(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_TRIPLE), ROW_OPERATION_IS(BRANCH_DELETION_OPERATION_REF), OPERATION_MATCH(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_DELETION_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_BRANCH_DELETION |
| `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | operator (only) | operator authority (§3.8 V3) | VERSION_PREPARATION_PACKET triple | `VERSION_PREPARATION_PACKET` | `PACKET_VALIDATION_RECEIPT` | AND(ROW_INSTANCE_IS(VERSION_PREPARATION_PACKET), ROW_SUBJECT_IS(VERSION_PREPARATION_PACKET), CURRENT(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT, IMPLEMENTATION_PLAN), STATE_CREATED(OPERATOR_IMPLEMENTATION_PLAN_APPROVAL_RECEIPT, IMPLEMENTATION_PLAN, OPERATOR_APPROVED_IMPLEMENTATION_PLAN), CURRENT(ARTIFACT_VALIDATION_RECEIPT, VERSION_INVENTORY_ARTIFACT), STATE_CREATED(ARTIFACT_VALIDATION_RECEIPT, VERSION_INVENTORY_ARTIFACT, ARTIFACT_VALIDATED)) | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | APPROVED | VERSION_PREPARATION_APPROVED |
| `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | operator (only) | operator checkpoint decision (§3.8 V4) | VERSION_PRE_EXECUTION_CHECKPOINT_PACKET triple | `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | AND(ROW_INSTANCE_IS(VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), ROW_SUBJECT_IS(VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), CURRENT(OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT, VERSION_PREPARATION_PACKET), STATE_CREATED(OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT, VERSION_PREPARATION_PACKET, VERSION_PREPARATION_APPROVED), CURRENT(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT), STATE_CREATED(EXPLORER_VALIDATION_RECEIPT, EXPLORER_ARTIFACT, EXPLORER_VALIDATED), PACKET_BINDS(VERSION_PRE_EXECUTION_CHECKPOINT_PACKET, VERSION_CHANGE_PROPOSAL_ARTIFACT)) | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | REVIEWED / BLOCKED | VERSION_CHECKPOINT_REVIEWED / VERSION_CHECKPOINT_BLOCKED |
| `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | operator (only) | operator authority (§3.8 V5) | VERSION_EXECUTION_AUTHORIZATION_PACKET triple | `VERSION_EXECUTION_AUTHORIZATION_PACKET` | `VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT` | AND(ROW_INSTANCE_IS(VERSION_EXECUTION_AUTHORIZATION_PACKET), ROW_SUBJECT_IS(VERSION_EXECUTION_AUTHORIZATION_PACKET), CURRENT(OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT, VERSION_PREPARATION_PACKET), STATE_CREATED(OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT, VERSION_PREPARATION_PACKET, VERSION_PREPARATION_APPROVED), CURRENT(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET, VERSION_CHECKPOINT_REVIEWED)) | OPERATOR_DECISION_RECORD | AUTHORIZED | VERSION_EXECUTION_AUTHORIZED |
|`VERSION_QUALIFICATION_PASS_RECEIPT`|vibedev/orchestrator|V5 operator authorization + V6 execution Evidence (§3.8 V7)|VERSION_QUALIFICATION_PACKET triple|`VERSION_QUALIFICATION_PACKET`|`OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`| AND(ROW_INSTANCE_IS(VERSION_QUALIFICATION_PACKET), ROW_SUBJECT_IS(VERSION_QUALIFICATION_PACKET), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), EVIDENCE_OF(VERSION_EXECUTION_PASS, VERSION_EXECUTION_REPORT)) |FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT|QUALIFIED / NOT_QUALIFIED|VERSION_QUALIFIED / VERSION_NOT_QUALIFIED|
|`VERSION_QUALIFICATION_FAIL_RECEIPT`|vibedev/orchestrator|V5 operator authorization + V6 execution Evidence (§3.8 V7)|VERSION_QUALIFICATION_PACKET triple|`VERSION_QUALIFICATION_PACKET`|`OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`| AND(ROW_INSTANCE_IS(VERSION_QUALIFICATION_PACKET), ROW_SUBJECT_IS(VERSION_QUALIFICATION_PACKET), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), EVIDENCE_OF(VERSION_EXECUTION_FAIL, VERSION_EXECUTION_REPORT)) |FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT|FAILED / NOT_QUALIFIED|VERSION_FAILED_QUALIFIED / VERSION_NOT_QUALIFIED|
|`VERSION_GOVERNANCE_CLOSEOUT_RECEIPT`|vibedev/orchestrator|V0–V7 explicit list + lifecycle management (`STOP_RECEIPT`, `INVALIDATION_RECEIPT`, `SUPERSESSION_RECEIPT`, `CONSULTATION_ENDPOINT_RECEIPT`, `PACKET_CONSUMPTION_RECEIPT`, `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT`, `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT`)|version governance scope triple|`VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_INVENTORY_ARTIFACT`, `VERSION_CHANGE_PROPOSAL_ARTIFACT`, `VERSION_PREPARATION_PACKET`, `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET`, `VERSION_EXECUTION_AUTHORIZATION_PACKET`, `VERSION_EXECUTION_REPORT`, `VERSION_EXECUTION_EVIDENCE_PACKET`, `VERSION_QUALIFICATION_PACKET`, `VERSION_CLOSEOUT_REPORT`, `VERSION_OPERATION_REF`|`VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT`, `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`, `VERSION_QUALIFICATION_PASS_RECEIPT`| AND(CURRENT(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET, VERSION_CHECKPOINT_REVIEWED), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), EXACTLY_ONE_OF(STATE_CREATED(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_PACKET, VERSION_QUALIFIED), STATE_CREATED(VERSION_QUALIFICATION_FAIL_RECEIPT, VERSION_QUALIFICATION_PACKET, VERSION_NOT_QUALIFIED))) |all V0–V7 Evidence classes|CLOSED / CLOSED_AFTER_FAILURE_STOP / CLOSED_BLOCKED|VERSION_GOVERNANCE_CLOSED_PASS / VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP / VERSION_GOVERNANCE_CLOSED_BLOCKED|
...
- The credential state classification string (e.g., `PROCESS_ENV_STALE_ABSENT`, `CREDENTIAL_RECOVERY_READY`).
...
4. Branch Deletion (optional independent Post-Draft operation; the original Work Order is provenance only and provides no current Branch Deletion authority).
...
| `NON_FAST_FORWARD` | Rejected: ref update would require non-fast-forward. Remote unchanged. STOP — **must not** auto-force, auto-rebase, or auto-reset. |
...
17. `NON_FAST_FORWARD` (defined in §14.6.1 Group C — terminal STOP; remote unchanged; no auto-force/rebase)
