...
|---|---|---|---|---|---|---|---|---|---|
| `GLOBAL_READINESS_RECEIPT` | vibedev/orchestrator | CURRENT OPERATOR_WORK_ORDER_APPROVAL_RECEIPT + Work Order triple | scope packet triple | scope packet, Work Order | OPERATOR_WORK_ORDER_APPROVAL_RECEIPT | AND | COMMAND_RESULT, FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | PASS / FAIL | GLOBAL_READINESS_PASS / GLOBAL_READINESS_FAIL |
| `ROLE_ACTIVATION_READINESS_RECEIPT` | vibedev/orchestrator | CURRENT GLOBAL_READINESS_RECEIPT + assignment approval | activation packet triple | activation packet | GLOBAL_READINESS_RECEIPT, assignment approval Receipt | AND | READINESS_CHECK, NODE_ROUTE_PROBE, MODEL_INVOCATION_RECORD | READY / BLOCKED | ROLE_ACTIVATION_READY / ROLE_ACTIVATION_BLOCKED |
...
| `ARTIFACT_VALIDATION_RECEIPT` | vibedev/orchestrator or validator per mode | Artifact type + mode | any Artifact triple | as applicable per role/review | as applicable per artifact type | mode-discriminated | as applicable per artifact type | VALID / INVALID | ARTIFACT_VALIDATED / ARTIFACT_INVALID |
| `PACKET_ASSEMBLY_RECEIPT` | assembler (vibedev/orchestrator) | assembly completion | assembled Packet triple | upstream validated objects | upstream validation Receipts | AND | DIFF_MANIFEST, TREE_MANIFEST | ASSEMBLED | PACKET_ASSEMBLED |
| `PACKET_FREEZE_RECEIPT` | vibedev/orchestrator | PACKET_ASSEMBLY_RECEIPT | Packet triple (same as subject) | assembled Packet | PACKET_ASSEMBLY_RECEIPT | AND | FILE_SNAPSHOT, GIT_OBJECT | FROZEN | PACKET_FROZEN |
| `PACKET_VALIDATION_RECEIPT` | vibedev/orchestrator | PACKET_FREEZE_RECEIPT | Packet triple (same as subject) | frozen Packet | PACKET_FREEZE_RECEIPT | AND | DIFF_MANIFEST, TREE_MANIFEST, TEST_RESULT | VALIDATED / INVALID | PACKET_VALIDATED / PACKET_VALIDATION_FAILED |
| `PACKET_CONSUMPTION_RECEIPT` | consumer role/stage controller or vibedev/orchestrator | frozen (optionally validated) Packet | Packet triple | frozen Packet | PACKET_FREEZE_RECEIPT, PACKET_VALIDATION_RECEIPT (if applicable) | AND | as applicable per consumer | CONSUMED | PACKET_CONSUMED |
| `TEST_GATE_PASS_RECEIPT` | vibedev/orchestrator | tester-a + tester-b cross-verification + same candidate + input Packet freeze | test evidence packet triple | tester-a input packet, tester-b input packet, same candidate | tester-a ROLE_CROSS_VERIFICATION_RECEIPT, tester-b ROLE_CROSS_VERIFICATION_RECEIPT, TESTER_A_INPUT_PACKET freeze, TESTER_B_INPUT_PACKET freeze | AND | TEST_RESULT, MODEL_INVOCATION_RECORD | PASS | GATE_PASS |
| `TEST_GATE_FAIL_RECEIPT` | vibedev/orchestrator | tester-a or tester-b failure Evidence | test evidence packet triple | tester-a input packet, tester-b input packet, same candidate | tester-a or tester-b ROLE_CROSS_VERIFICATION_RECEIPT | OR (at least one failure) | TEST_RESULT, MODEL_INVOCATION_RECORD | FAIL | GATE_FAIL |
| `REVIEW_GATE_PASS_RECEIPT` | vibedev/orchestrator | reviewer-a + reviewer-b cross-verification + same Review Packet/candidate + TEST_GATE_PASS_RECEIPT | review input packet triple | review input packet, same candidate | reviewer-a ROLE_CROSS_VERIFICATION_RECEIPT, reviewer-b ROLE_CROSS_VERIFICATION_RECEIPT, TEST_GATE_PASS_RECEIPT | AND | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | PASS | GATE_PASS |
| `REVIEW_GATE_FAIL_RECEIPT` | vibedev/orchestrator | reviewer-a or reviewer-b failure Evidence | review input packet triple | review input packet, same candidate | reviewer-a or reviewer-b ROLE_CROSS_VERIFICATION_RECEIPT | OR (at least one failure) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | FAIL | GATE_FAIL |
| `GIT_INTEGRATION_GATE_PASS_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_PASS_RECEIPT + Integration Packet freeze/validation + git-integrator binding | INTEGRATION_INPUT_FROZEN triple | Integration Packet | REVIEW_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN freeze, INTEGRATION_INPUT_FROZEN validation, POST_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT | AND | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | PASS | GATE_PASS |
| `GIT_INTEGRATION_GATE_FAIL_RECEIPT` | vibedev/orchestrator | REVIEW_GATE_FAIL_RECEIPT or Integration Evidence failure | INTEGRATION_INPUT_FROZEN triple | Integration Packet | REVIEW_GATE_FAIL_RECEIPT or integration failure Evidence | OR (at least one failure) | GIT_OBJECT, DIFF_MANIFEST, PR_METADATA_SNAPSHOT | FAIL | GATE_FAIL |
...
| `READY_TRANSITION_RECEIPT` | vibedev/orchestrator | DRAFT_PR_DELIVERY_RECEIPT + DRAFT_TO_READY_APPROVAL_PACKET freeze | PR triple | DRAFT_TO_READY_APPROVAL_PACKET | DRAFT_PR_DELIVERY_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET freeze, DRAFT_TO_READY_APPROVAL_PACKET validation | AND | PR_METADATA_SNAPSHOT, OPERATOR_DECISION_RECORD | READY / BLOCKED | PR_READY / PR_READY_BLOCKED |
| `MERGE_DELIVERY_RECEIPT` | git-integrator | READY_TRANSITION_RECEIPT + MERGE_APPROVAL_PACKET freeze | PR triple | MERGE_APPROVAL_PACKET | READY_TRANSITION_RECEIPT, MERGE_APPROVAL_PACKET freeze, MERGE_APPROVAL_PACKET validation | AND | GIT_OBJECT, PR_METADATA_SNAPSHOT | MERGED / FAILED | PR_MERGED / MERGE_FAILED |
| `BRANCH_DELETION_RECEIPT` | git-integrator | MERGE_DELIVERY_RECEIPT + BRANCH_DELETION_APPROVAL_PACKET freeze | branch triple | BRANCH_DELETION_APPROVAL_PACKET | MERGE_DELIVERY_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET freeze | AND | GIT_OBJECT, PR_METADATA_SNAPSHOT | DELETED / FAILED | BRANCH_DELETED / BRANCH_DELETION_FAILED |
...
| `CONSULTATION_ENDPOINT_RECEIPT` | vibedev/orchestrator | consultation scope + Work Order | CONSULTATION_DELIVERABLE triple | consultation source packet, Work Order | GLOBAL_READINESS_RECEIPT, OPERATOR_WORK_ORDER_APPROVAL_RECEIPT (Consultation path) | AND | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | DELIVERED / FAILED | CONSULTATION_DELIVERED / CONSULTATION_FAILED |
| `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT` | vibedev/orchestrator (public-safe verification function) | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT + source Evidence | PUBLIC_EVIDENCE_PROJECTION_ARTIFACT triple | source Evidence | upstream Evidence validation | AND | FILE_SNAPSHOT, OPERATOR_DECISION_RECORD | SAFE / UNSAFE | PROJECTION_SAFE / PROJECTION_UNSAFE |
| `STOP_RECEIPT` | vibedev/orchestrator (only) | STOP_REPORT Artifact + Evidence; or hard STOP signal; or operator instruction | subject STOP reason triple | STOP_REPORT (if applicable) | as applicable per stop class | mode-discriminated | MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD, STOP signal catalog entry | STOPPED | TASK_STOPPED / WORKFLOW_STOPPED |
| `INVALIDATION_RECEIPT` | vibedev/orchestrator | Evidence of invalidation condition | single Artifact, Evidence, Packet, or Receipt triple | subject object | upstream evidence of invalidation | AND | as applicable per subject type | INVALIDATED | SUBJECT_INVALIDATED |
| `SUPERSESSION_RECEIPT` | vibedev/orchestrator | Evidence of supersession + superseding object | single Artifact, Evidence, Packet, or Receipt triple | superseding object | upstream evidence + superseding object receipt | AND | FILE_SNAPSHOT, GIT_OBJECT | SUPERSEDED | OBJECT_SUPERSEDED |
| `OPERATOR_REVOCATION_RECEIPT` | operator (only) | operator authority | single operator-authorisation Receipt triple | as applicable per revocation reason | as applicable per revocation reason | AND | OPERATOR_DECISION_RECORD | REVOKED | AUTHORISATION_REVOKED |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | operator (only) | operator authority | intake scope packet triple | none | none | — | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_INTAKE_SCOPE |
...
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | operator (only) | operator authority | Work Order triple | Work Order declaration | mode-discriminated: FULL/LIGHTWEIGHT → OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT; CONSULTATION_ONLY → not applicable (no non-orchestrator assignment); binds Intake + submode only | mode-discriminated | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_WORK_ORDER |
...
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | DRAFT_TO_READY_APPROVAL_PACKET triple | DRAFT_TO_READY_APPROVAL_PACKET | mode-discriminated: FULL/LIGHTWEIGHT → DRAFT_TO_READY_APPROVAL_PACKET assembly/freeze/validation + verified gate chain (Git Integration PASS via applicable mode); CONSULTATION_ONLY → not applicable (must not appear) | mode-discriminated | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_DRAFT_TO_READY |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | MERGE_APPROVAL_PACKET triple | MERGE_APPROVAL_PACKET | READY_TRANSITION_RECEIPT, MERGE_APPROVAL_PACKET assembly/freeze/validation | AND | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_MERGE |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | operator (only) | operator authority | BRANCH_DELETION_APPROVAL_PACKET triple | BRANCH_DELETION_APPROVAL_PACKET | OPERATOR_MERGE_AUTHORIZATION_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET assembly/freeze/validation | AND | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_BRANCH_DELETION |
| `OPERATOR_APPROVED_VERSION_PREPARATION_RECEIPT` | operator (only) | operator authority (§3.8 V3) | VERSION_PREPARATION_PACKET triple | VERSION_PREPARATION_PACKET | VERSION_GOVERNANCE_SCOPE_PACKET freeze, VERSION_INVENTORY_ARTIFACT validation, VERSION_CHANGE_PROPOSAL_ARTIFACT validation | AND | OPERATOR_DECISION_RECORD, FILE_SNAPSHOT, GIT_OBJECT | APPROVED | VERSION_PREPARATION_APPROVED |
...
| `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | operator (only) | operator authority (§3.8 V5) | VERSION_EXECUTION_AUTHORIZATION_PACKET triple | VERSION_EXECUTION_AUTHORIZATION_PACKET | VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT | AND | OPERATOR_DECISION_RECORD | AUTHORIZED | VERSION_EXECUTION_AUTHORIZED |
| `VERSION_QUALIFICATION_PASS_RECEIPT` | vibedev/orchestrator | V5 operator authorisation + V6 execution evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | VERSION_QUALIFICATION_PACKET | OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT | AND | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | PASS | VERSION_QUALIFICATION_PASS |
| `VERSION_QUALIFICATION_FAIL_RECEIPT` | vibedev/orchestrator | V5 operator authorisation + V6 execution evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | VERSION_QUALIFICATION_PACKET | OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT | AND | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | FAIL | VERSION_QUALIFICATION_FAIL |
| `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT` | vibedev/orchestrator | V7 qualification receipt + all V0–V7 objects (§3.8 V8) | VERSION_CLOSEOUT_REPORT triple | VERSION_CLOSEOUT_REPORT, all V0–V7 objects | exactly_one_of(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_FAIL_RECEIPT) | exactly_one_of | all V0–V7 class evidence as applicable | CLOSED / CLOSED_AFTER_FAILURE_STOP | VERSION_GOVERNANCE_CLOSED_PASS / VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP |
...
| Drift signals | 7 | expanded to 230-entry canonical signal catalog (31 + 9 + 5 + 6 + 4 new Gray4-sequence / result-semantics / prerequisite-expression signals) |
...
| Drift signals | 175 | expanded to 230-entry canonical signal catalog (31 + 9 + 5 + 6 + 4 new Gray4-sequence / result-semantics / prerequisite-expression signals) |
