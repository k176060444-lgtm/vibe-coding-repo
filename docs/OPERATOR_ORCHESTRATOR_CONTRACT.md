...
| `validator per mode` | `ARTIFACT_VALIDATION_RECEIPT` | OBJECT_CLASS_IS(ROLE_PRODUCED_ARTIFACT) → validator per mode; OBJECT_CLASS_IS(GOVERNANCE_ARTIFACT_OR_APPROVAL) → vibedev/orchestrator | per artifact class/mode (determined by OBJECT_CLASS_IS) | n/a (machine-determined) | |
...
| `ARTIFACT_VALIDATION_RECEIPT` | OBJECT_CLASS_IS(ROLE_PRODUCED_ARTIFACT) → validator per mode; OBJECT_CLASS_IS(GOVERNANCE_ARTIFACT_OR_APPROVAL) → vibedev/orchestrator | Artifact class mode | any Artifact triple | `SUBJECT_ARTIFACT_TRIPLE` | ∅ | EVIDENCE_OF(VALIDATION_EVIDENCE, SUBJECT_ARTIFACT_TRIPLE) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | VALID / INVALID | ARTIFACT_VALIDATED / ARTIFACT_INVALID |
