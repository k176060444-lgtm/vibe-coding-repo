...
| `CURRENT(RECEIPT_TYPE)` or `CURRENT(RECEIPT_TYPE, OBJECT_TRIPLE)` | canonical Receipt type (1st arg optional canonical object triple) | subject has a CURRENT Receipt of the given type optionally bound to given object triple | registry row | triple-bound via 2nd arg | no | returns FALSE | CURRENT only (`lifecycle_state_at_creation=CURRENT` AND no effective INVALIDATION/SUPERSESSION/REVOCATION on that subject/object triple); different subject/object triple or different version not CURRENT | NEVER `PASS`; receipts are CURRENT only when `lifecycle_state_at_creation=CURRENT` and not invalidated/superseded/revoked |
...
| `ARTIFACT_VALIDATION_RECEIPT` | validator per mode (role-produced) or vibedev/orchestrator (governance/approval) | Artifact type + mode | any Artifact triple | `SUBJECT_ARTIFACT_TRIPLE` | ∅ | AND(CURRENT(ARTIFACT_VALIDATION_RECEIPT, SUBJECT_ARTIFACT_TRIPLE), EVIDENCE_OF(VALIDATION_EVIDENCE, SUBJECT_ARTIFACT_TRIPLE)) | MODEL_INVOCATION_RECORD, FILE_SNAPSHOT | VALID / INVALID | ARTIFACT_VALIDATED / ARTIFACT_INVALID |
...
| `OPERATOR_REVOCATION_RECEIPT` | operator (only) | operator authority | single operator-authorisation Receipt triple | `AUTHORISATION_RECEIPT_TRIPLE` | ∅ | AND(CURRENT(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE), SUBJECT_MATCH(OPERATOR_REVOCATION_RECEIPT, AUTHORISATION_RECEIPT_TRIPLE)) | OPERATOR_DECISION_RECORD | REVOKED | AUTHORISATION_REVOKED |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | operator (only) | operator authority (may be followed by `OPERATOR_REVOCATION_RECEIPT`) | intake scope packet triple | `INTAKE_SCOPE_PACKET` | ∅ | AND(CURRENT(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET), SUBJECT_MATCH(OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT, INTAKE_SCOPE_PACKET)) | OPERATOR_DECISION_RECORD | APPROVED | OPERATOR_APPROVED_INTAKE_SCOPE |
