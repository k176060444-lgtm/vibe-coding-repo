...
| `INSTANCE_SUBJECT_MATCH_NEXT(RECEIPT_TYPE, INSTANCE_REF, SUBJECT_REF)` | three required params: same as INSTANCE_SUBJECT_MATCH | a **downstream consumer** semantic: a future Receipt will check that the to-be-issued Receipt's instance ref references the given subject ref. Used inside an issuance-time expression to declare that the issued Receipt binds the subject of a **next-stage** Receipt (e.g., that this Receipt's instance key is a placeholder pointing at the next-stage's invocation). The receipt identified by INSTANCE_REF is NOT the current Receipt (i.e., this is NOT a self-reference). Missing or type-mismatch returns FALSE. | registry row | n/a | no | FALSE if missing or type mismatch | n/a |
...
#### §10.3.10.0.5 Canonical Receipt Instance Key Mapping (Machine-Parseable, all 44 Registry Receipt Types)

Every Registry Receipt Type (44 types) is mapped to its unique `instance_key_type`, the set of `allowed concrete key class` references, the `canonical subject class`, whether `instance = subject`, and whether `INSTANCE_SUBJECT_MATCH` is mandatory at consumption.

The validator enforces a per-type **"allowed-type" rule**: for Receipt Types with `instance_key_type = PACKET_REF`, `STATE_CREATED`/`CURRENT`/`DECISION` MAY use any Catalog ref with `class = packet` that fits the row's stage/class — not a fixed literal. `GENERIC_PACKET_REF` is restricted to the Packet's own Registry row as a row-bound typed variable; downstream rows must use the concrete Packet ref (or another row-bound typed variable that resolves to one).

| Receipt Type | instance_key_type | allowed concrete key class | canonical subject class | instance = subject | INSTANCE_SUBJECT_MATCH required |
|---|---|---|---|---|---|
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | WORK_ORDER | `WORK_ORDER_DECLARATION` only | work-order triple | yes | no |
...
| `ROLE_ACTIVATION_READINESS_RECEIPT` | INVOCATION_REF | any `*_INVOCATION_REF` in Catalog (TESTER_A/B, REVIEWER_A/B, GIT_INTEGRATOR role activations, DRAFT_TO_READY, MERGE, BRANCH_DELETION) | activation packet triple | no | **yes (mandatory)** |
...
| `ARTIFACT_VALIDATION_RECEIPT` | ARTIFACT | `CANDIDATE_TRIPLE` OR any concrete artifact ref in Catalog (when used in Git Gate); the instance key is the actual artifact under validation, NOT a packet like `INTEGRATION_INPUT_FROZEN` | artifact triple | yes | no |
...
| `PACKET_ASSEMBLY_RECEIPT` | PACKET_REF (row-bound) | any `class = packet` Catalog ref matching current row's PACKET_CLASS branch (concrete: `INTEGRATION_INPUT_FROZEN`, `DRAFT_TO_READY_APPROVAL_PACKET`, `MERGE_APPROVAL_PACKET`, `BRANCH_DELETION_APPROVAL_PACKET`, `CONSULTATION_SOURCE_PACKET`, `VERSION_GOVERNANCE_SCOPE_PACKET`); or `GENERIC_PACKET_REF` as a Registry-row-bound typed variable | packet triple | yes | no |
| `PACKET_FREEZE_RECEIPT` | PACKET_REF (row-bound) | any `class = packet` Catalog ref; or `GENERIC_PACKET_REF` as a Registry-row-bound typed variable | packet triple | yes | no |
| `PACKET_VALIDATION_RECEIPT` | PACKET_REF (row-bound) | any `class = packet` Catalog ref; or `GENERIC_PACKET_REF` as a Registry-row-bound typed variable | packet triple | yes | no |
| `PACKET_CONSUMPTION_RECEIPT` | PACKET_REF (row-bound) | any `class = packet` Catalog ref; or `GENERIC_PACKET_REF` as a Registry-row-bound typed variable | packet triple | yes | no |
...
| `ROLE_CROSS_VERIFICATION_RECEIPT` | CROSS_VERIFICATION_REF | per-role `*_CROSS_VERIFICATION_REF` (`TESTER_A/B_CROSS_VERIFICATION_REF`, `REVIEWER_A/B_CROSS_VERIFICATION_REF`, `GIT_CROSS_VERIFICATION_REF`, `READY_CROSS_VERIFICATION_REF`, `MERGE_CROSS_VERIFICATION_REF`, `BD_CROSS_VERIFICATION_REF`) | role completion report | no | **yes (mandatory)** |
| `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | ROLE_TRIPLE | `GIT_INTEGRATOR_ROLE_TRIPLE` | activation evidence (`GIT_INTEGRATOR_ACTIVATION_EVIDENCE`) | no | **yes (mandatory)** |
...
| `OPERATOR_REVOCATION_RECEIPT` | AUTHORISATION_TRIPLE | `AUTHORISATION_RECEIPT_TRIPLE` (as revocation-target ref, NOT as primary instance key — primary instance key is determined by `REVOCATION_TARGET_MATCH`) | authorisation triple | yes | no |

**Operator-binding rules (enforced by typed AST validator):**

1. `CURRENT(RECEIPT_TYPE, INSTANCE_REF)` / `STATE_CREATED(RECEIPT_TYPE, INSTANCE_REF, STATE)` / `DECISION(RECEIPT_TYPE, INSTANCE_REF, DECISION)` MUST use, as `INSTANCE_REF`, a ref whose class appears in the `allowed concrete key class` column for the given `RECEIPT_TYPE`.
2. Same Receipt Type MAY NOT use different instance-key classes in different rows (e.g., `MERGE_DELIVERY_RECEIPT` MUST use `PR_TRIPLE` in every row).
3. For Receipt Types marked `INSTANCE_SUBJECT_MATCH required = yes`, every consuming row MUST invoke `INSTANCE_SUBJECT_MATCH(RECEIPT_TYPE, INSTANCE_REF, SUBJECT_REF)` against the matching completion report (or other subject artifact).
4. Cross-verification Receipts MUST use their per-stage `*_CROSS_VERIFICATION_REF` as instance key AND MUST bind the matching completion report via `INSTANCE_SUBJECT_MATCH`.
5. Revocation is NOT achieved by changing the target Receipt's instance key. `OPERATOR_REVOCATION_RECEIPT` references the target through `AUTHORISATION_RECEIPT_TRIPLE` + `REVOCATION_TARGET_MATCH` only.

#### §10.3.10.0.6#### §10.3.10.0.6 Canonical Operation Binding Mapping

Every ref that participates in `SAME_OPERATION` or `OPERATION_MATCH` MUST be classified as `operation_binding = yes` or `operation_binding = no`. The two sets are strictly disjoint (intersection = 0). Every Catalog ref has exactly one classification.

**`operation_binding = yes` (each carries operation_id/version/digest):**

| ref | operation_binding_class |
|---|---|
| `GIT_OPERATION_REF` | operation_ref |
| `PACKET_OPERATION_REF` | operation_ref |
| `DRAFT_TO_READY_OPERATION_REF` | operation_ref |
| `MERGE_OPERATION_REF` | operation_ref |
| `BRANCH_DELETION_OPERATION_REF` | operation_ref |
| `VERSION_OPERATION_REF` | operation_ref |
| `TEST_OPERATION_REF` | operation_ref |
| `REVIEW_OPERATION_REF` | operation_ref |
| `TEST_EVIDENCE_PACKET` | packet (concrete) |
| `TESTER_A_INPUT_PACKET` | packet (concrete) |
| `TESTER_B_INPUT_PACKET` | packet (concrete) |
| `REVIEW_INPUT_PACKET` | packet (concrete) |
| `INTEGRATION_INPUT_FROZEN` | packet (concrete) |
| `DRAFT_TO_READY_APPROVAL_PACKET` | approval_packet |
| `MERGE_APPROVAL_PACKET` | approval_packet |
| `BRANCH_DELETION_APPROVAL_PACKET` | approval_packet |
| `TESTER_A_INVOCATION_REF` | invocation_ref |
| `TESTER_B_INVOCATION_REF` | invocation_ref |
| `REVIEWER_A_INVOCATION_REF` | invocation_ref |
| `REVIEWER_B_INVOCATION_REF` | invocation_ref |
| `DRAFT_TO_READY_INVOCATION_REF` | invocation_ref |
| `MERGE_INVOCATION_REF` | invocation_ref |
| `BRANCH_DELETION_INVOCATION_REF` | invocation_ref |
| `TESTER_A_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `TESTER_B_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `REVIEWER_A_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `REVIEWER_B_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `GIT_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `READY_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `MERGE_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `BD_CROSS_VERIFICATION_REF` | cross_verification_ref |
| `TESTER_A_COMPLETION_REPORT` | completion_report | per-tester completion artifact | issued at end of tester-a run, consumed by Test Gate | TEST | post-stage evidence |
| `TESTER_B_COMPLETION_REPORT` | completion_report | per-tester completion artifact | issued at end of tester-b run, consumed by Test Gate | TEST | post-stage evidence |
| `REVIEWER_A_COMPLETION_REPORT` | completion_report | per-reviewer completion artifact | issued at end of reviewer-a run, consumed by Review Gate | REVIEW | post-stage evidence |
| `REVIEWER_B_COMPLETION_REPORT` | completion_report | per-reviewer completion artifact | issued at end of reviewer-b run, consumed by Review Gate | REVIEW | post-stage evidence |
| `GIT_INTEGRATION_COMPLETION_REPORT` | completion_report | git-integration completion artifact | issued at end of git-integrator run, consumed by Git Gate | GIT_INTEGRATION | post-stage evidence |
| `READY_TRANSITION_COMPLETION_REPORT` | completion_report |
| `MERGE_COMPLETION_REPORT` | completion_report |
| `BRANCH_DELETION_COMPLETION_REPORT` | completion_report |
| `GIT_INTEGRATOR_ROLE_TRIPLE` | role_triple |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE` | activation_evidence |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY` | activation_evidence |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE` | activation_evidence |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD` | activation_evidence |
| `VERSION_INVENTORY_ARTIFACT` | version_artifact |
| `VERSION_CHANGE_PROPOSAL_ARTIFACT` | version_artifact |
| `VERSION_PREPARATION_PACKET` | version_packet |
| `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET` | version_packet |
| `VERSION_EXECUTION_AUTHORIZATION_PACKET` | version_packet |
| `VERSION_EXECUTION_REPORT` | version_report |
| `VERSION_EXECUTION_EVIDENCE_PACKET` | version_packet |
| `VERSION_QUALIFICATION_PACKET` | version_packet |
| `VERSION_CLOSEOUT_REPORT` | version_report |
| `VERSION_GOVERNANCE_SCOPE_PACKET` | version_packet |
| `GENERIC_PACKET_REF` (only as Registry-row-bound typed variable; resolves to current row's concrete Packet ref) | packet_variable |

**`operation_binding = no` (no operation_id carried; MUST NOT appear in SAME_OPERATION or as OPERATION_MATCH first arg):**

| ref | reason |
|---|---|
| `PR_TRIPLE` | PR identifier is not an operation ref |
| `BRANCH_TRIPLE` | branch identifier is not an operation ref |
| `EXPLORER_ARTIFACT` | explorer subject artifact, no operation |
| `IMPLEMENTATION_PLAN` | plan triple, no operation |
| `CANDIDATE_TRIPLE` | candidate identity, no operation |
| `WORK_ORDER_DECLARATION` | work order declaration, no operation |
| `SUBMODE_DECLARATION` | submode declaration, no operation |
| `INTAKE_SCOPE_PACKET` | intake scope, no operation |
| `INTAKE_EVIDENCE_PACKET` | intake evidence, no operation |
| `CONSULTATION_SOURCE_PACKET` | consultation source, no operation |
| `SOURCE_EVIDENCE_OBJECTS` | source evidence, no operation |
| `SUBJECT_ARTIFACT_TRIPLE` | subject artifact, no operation |
| `SUBJECT_TRIPLE` | subject identifier, no operation |
| `SUPERSEDING_OBJECT_TRIPLE` | superseded object, no operation |
| `AUTHORISATION_RECEIPT_TRIPLE` | authorisation triple, no operation (used as revocation target reference) |
| `STOP_REPORT` | stop report, no operation |
|  | **removed — never defined; legacy placeholder eliminated** |

**OPERATION_MATCH replacement rule:** Every `OPERATION_MATCH(REF, OPERATION_REF)` MUST have REF in the `yes` set above. Any `OPERATION_MATCH` first argument in the `no` set (or not in any class) is a typed validator error.

**SAME_OPERATION argument rule:** `SAME_OPERATION(...)` requires at least 2 arguments; ALL arguments MUST be in the `yes` set (operation refs). Single-argument `SAME_OPERATION` calls are invalid and MUST be eliminated. Different stage operation refs MUST NOT be combined in a single `SAME_OPERATION` (e.g., `SAME_OPERATION(TEST_OPERATION_REF, REVIEW_OPERATION_REF)` is forbidden; each stage keeps its own operation).

| `GIT_INTEGRATOR_INVOCATION_REF` | invocation_ref | git-integrator role activation invocation | issued at role activation, consumed by Pre-Draft Binding | GIT_INTEGRATION | pre-stage evidence |
| `TESTER_A_ACTIVATION_EVIDENCE` | activation_evidence | tester-a role activation record | issued at tester-a activation, consumed by Test Gate | TEST | pre-stage evidence |
| `TESTER_B_ACTIVATION_EVIDENCE` | activation_evidence | tester-b role activation record | issued at tester-b activation, consumed by Test Gate | TEST | pre-stage evidence |
| `REVIEWER_A_ACTIVATION_EVIDENCE` | activation_evidence | reviewer-a role activation record | issued at reviewer-a activation, consumed by Review Gate | REVIEW | pre-stage evidence |
| `REVIEWER_B_ACTIVATION_EVIDENCE` | activation_evidence | reviewer-b role activation record | issued at reviewer-b activation, consumed by Review Gate | REVIEW | pre-stage evidence |
#### §10.3.10.1| `TESTER_A_COMPLETION_REPORT` | concrete ref | per-tester completion artifact |
| `TESTER_B_COMPLETION_REPORT` | concrete ref | per-tester completion artifact |
| `REVIEWER_A_COMPLETION_REPORT` | concrete ref | per-reviewer completion artifact |
| `REVIEWER_B_COMPLETION_REPORT` | concrete ref | per-reviewer completion artifact |
| `GIT_INTEGRATION_COMPLETION_REPORT` | concrete ref | Git integration completion artifact |
...
| `ROLE_ACTIVATION_READINESS_RECEIPT` | vibedev/orchestrator (FULL/LIGHTWEIGHT only — not Consultation) | Global Readiness + role/node/model assignment approval |activation packet triple (per invocation)|`TESTER_A_INVOCATION_REF`, `TESTER_B_INVOCATION_REF`, `REVIEWER_A_INVOCATION_REF`, `REVIEWER_B_INVOCATION_REF`, `GIT_INTEGRATOR_INVOCATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `MERGE_INVOCATION_REF`, `BRANCH_DELETION_INVOCATION_REF`, `TESTER_A_ACTIVATION_EVIDENCE`, `TESTER_B_ACTIVATION_EVIDENCE`, `REVIEWER_A_ACTIVATION_EVIDENCE`, `REVIEWER_B_ACTIVATION_EVIDENCE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE`, `GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD`|`GLOBAL_READINESS_RECEIPT`, `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT`| AND(ROW_INSTANCE_IS(ROW_INSTANCE_VAR), ROW_SUBJECT_IS(ROW_SUBJECT_VAR), CURRENT(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION), STATE_CREATED(GLOBAL_READINESS_RECEIPT, WORK_ORDER_DECLARATION, GLOBAL_READINESS_ACHIEVED), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), EXACTLY_ONE_OF(AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_A_INVOCATION_REF, TESTER_A_ACTIVATION_EVIDENCE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, TESTER_B_INVOCATION_REF, TESTER_B_ACTIVATION_EVIDENCE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_A_INVOCATION_REF, REVIEWER_A_ACTIVATION_EVIDENCE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, REVIEWER_B_INVOCATION_REF, REVIEWER_B_ACTIVATION_EVIDENCE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, GIT_INTEGRATOR_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, DRAFT_TO_READY_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_DRAFT_TO_READY)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, MERGE_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_MERGE)), AND(CURRENT(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF), STATE_CREATED(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, ROLE_ACTIVATION_READY), INSTANCE_SUBJECT_MATCH_NEXT(ROLE_ACTIVATION_READINESS_RECEIPT, BRANCH_DELETION_INVOCATION_REF, GIT_INTEGRATOR_ACTIVATION_EVIDENCE_BD)))) |READINESS_CHECK, NODE_ROUTE_PROBE, MODEL_INVOCATION_RECORD|READY / BLOCKED|ROLE_ACTIVATION_READY / ROLE_ACTIVATION_BLOCKED|
| `ROLE_CROSS_VERIFICATION_RECEIPT` | vibedev/orchestrator | role completion report Artifact | ROLE_COMPLETION_REPORT triple | `ROLE_COMPLETION_REPORT` | `GLOBAL_READINESS_RECEIPT` | AND(ROW_INSTANCE_IS(ROLE_CROSS_VERIFICATION_INSTANCE_VAR), ROW_SUBJECT_IS(ROLE_COMPLETION_REPORT), CURRENT(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET), STATE_CREATED(OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT, ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET, OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT), STATE_CREATED(ROLE_COMPLETION_REPORT, ROLE_ACTIVATION_READY, ROLE_ACTIVATION_READY)) | GIT_OBJECT, TEST_RESULT, MODEL_INVOCATION_RECORD, OPERATOR_DECISION_RECORD | VERIFIED / INCOMPLETE / REJECTED | ROLE_CROSS_VERIFIED / ROLE_CROSS_VERIFICATION_INCOMPLETE / ROLE_CROSS_VERIFICATION_REJECTED |
...
| `VERSION_QUALIFICATION_PASS_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution Evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(ROW_INSTANCE_IS(VERSION_QUALIFICATION_PACKET), ROW_SUBJECT_IS(VERSION_QUALIFICATION_PACKET), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), CURRENT(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT), STATE_CREATED(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT, VERSION_EXECUTION_REPORTED), EXACTLY_ONE_OF(STATE_CREATED(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT, VERSION_EXECUTION_PASS), STATE_CREATED(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT, VERSION_EXECUTION_FAIL)), DECISION(VERSION_QUALIFICATION_RECEIPT, VERSION_QUALIFICATION_PACKET, QUALIFIED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | PASS | VERSION_QUALIFICATION_PASS |
| `VERSION_QUALIFICATION_FAIL_RECEIPT` | vibedev/orchestrator | V5 operator authorization + V6 execution Evidence (§3.8 V7) | VERSION_QUALIFICATION_PACKET triple | `VERSION_QUALIFICATION_PACKET` | `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT` | AND(ROW_INSTANCE_IS(VERSION_QUALIFICATION_PACKET), ROW_SUBJECT_IS(VERSION_QUALIFICATION_PACKET), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), CURRENT(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT), STATE_CREATED(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_EXECUTION_REPORT, VERSION_EXECUTION_REPORTED), DECISION(VERSION_QUALIFICATION_RECEIPT, VERSION_QUALIFICATION_PACKET, NOT_QUALIFIED)) | FILE_SNAPSHOT, GIT_OBJECT, MODEL_INVOCATION_RECORD, TEST_RESULT | FAIL | VERSION_QUALIFICATION_FAIL |
| `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT` | vibedev/orchestrator | V0–V7 explicit list + lifecycle management (`STOP_RECEIPT`, `INVALIDATION_RECEIPT`, `SUPERSESSION_RECEIPT`, `CONSULTATION_ENDPOINT_RECEIPT`, `PACKET_CONSUMPTION_RECEIPT`, `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT`, `VERSION_GOVERNANCE_CLOSEOUT_RECEIPT`) |version governance scope triple|`VERSION_GOVERNANCE_SCOPE_PACKET`, `VERSION_INVENTORY_ARTIFACT`, `VERSION_CHANGE_PROPOSAL_ARTIFACT`, `VERSION_PREPARATION_PACKET`, `VERSION_PRE_EXECUTION_CHECKPOINT_PACKET`, `VERSION_EXECUTION_AUTHORIZATION_PACKET`, `VERSION_EXECUTION_REPORT`, `VERSION_EXECUTION_EVIDENCE_PACKET`, `VERSION_QUALIFICATION_PACKET`, `VERSION_CLOSEOUT_REPORT`, `VERSION_OPERATION_REF`|`VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT`, `OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT`, `VERSION_QUALIFICATION_PASS_RECEIPT`, `VERSION_QUALIFICATION_FAIL_RECEIPT`| AND(CURRENT(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET), STATE_CREATED(VERSION_PRE_EXECUTION_CHECKPOINT_REVIEW_RECEIPT, VERSION_PRE_EXECUTION_CHECKPOINT_PACKET, VERSION_CHECKPOINT_REVIEWED), CURRENT(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET), STATE_CREATED(OPERATOR_AUTHORIZED_VERSION_EXECUTION_RECEIPT, VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_EXECUTION_AUTHORIZED), OPERATION_MATCH(VERSION_INVENTORY_ARTIFACT, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_CHANGE_PROPOSAL_ARTIFACT, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_PREPARATION_PACKET, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_PRE_EXECUTION_CHECKPOINT_PACKET, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_EXECUTION_AUTHORIZATION_PACKET, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_EXECUTION_REPORT, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_EXECUTION_EVIDENCE_PACKET, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_QUALIFICATION_PACKET, VERSION_OPERATION_REF), OPERATION_MATCH(VERSION_CLOSEOUT_REPORT, VERSION_OPERATION_REF), EXACTLY_ONE_OF(AND(CURRENT(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_PACKET), STATE_CREATED(VERSION_QUALIFICATION_PASS_RECEIPT, VERSION_QUALIFICATION_PACKET, VERSION_QUALIFICATION_PASS)), AND(CURRENT(VERSION_QUALIFICATION_FAIL_RECEIPT, VERSION_QUALIFICATION_PACKET), STATE_CREATED(VERSION_QUALIFICATION_FAIL_RECEIPT, VERSION_QUALIFICATION_PACKET, VERSION_QUALIFICATION_FAIL)))) | all V0–V7 Evidence classes | CLOSED / CLOSED_AFTER_FAILURE_STOP / CLOSED_BLOCKED | VERSION_GOVERNANCE_CLOSED_PASS / VERSION_GOVERNANCE_CLOSED_AFTER_FAILURE_STOP / VERSION_GOVERNANCE_CLOSED_BLOCKED |
