...
### §10.1 Drift Signals
  ...
  - (kkkkkk) `WORK_ORDER_ROLE_ASSIGNMENT_INCOMPLETE` — `role_topology` or `authorization_bindings` mode-discriminated check:
    - **`FULL_9_ROLE_VIBECODING`**: lacks non-orchestrator `OPERATOR_APPROVED_ROLE_NODE_MODEL_ASSIGNMENT_BASELINE` with all 8 non-orchestrator roles, lacks content-author / independent-verifier / git-integrator, or `LIGHTWEIGHT` exceeds its approved actual role set (§4.17.3, §4.17.5).
    - **`LIGHTWEIGHT_OPERATION`**: lacks operator-approved actual non-orchestrator roles, or includes roles outside the approved set.
    - **`VIBECODING_CONSULTATION_ONLY`**: **not** applicable — `CONSULTATION_WORK_ORDER_REQUIRES_ASSIGNMENT_BASELINE` is the correct signal for baseline violations in Consultation. **Immediate STOP.**
  ...
  - (oooooo) `WORK_ORDER_BUDGET_UNBOUNDED` — `budget_contract` mode-discriminated check, field-by-field alignment with §4.17.9:
    - **`FULL_9_ROLE_VIBECODING`** and **`LIGHTWEIGHT_OPERATION`**: missing per-role / total / loop / push / PR API / proxy bypass budgets, or any automatic model substitution / quota retry / quota-reset wait enabled (§4.17.9).
