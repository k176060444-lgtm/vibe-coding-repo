...
#### §10.3.10.0.5 Canonical Receipt Instance Key Mapping (Machine-Parseable)

This subsection defines the unique `instance_key_class` and canonical instance key ref for every Registry Receipt Type. The mapping is binding: the typed AST validator MUST enforce these mappings against every `CURRENT`/`STATE_CREATED`/`DECISION` second argument and every `SAME_OPERATION`/`OPERATION_MATCH` parameter. No implementer may choose an instance key outside this mapping.

| Receipt Type | instance_key_class | canonical instance key ref | Subject Class | instance = subject | INSTANCE_SUBJECT_MATCH required |
|---|---|---|---|---|---|
| `OPERATOR_WORK_ORDER_APPROVAL_RECEIPT` | Work Order triple | `AUTHORISATION_RECEIPT_TRIPLE` | work-order triple | yes | no |
| `OPERATOR_INTAKE_SCOPE_APPROVAL_RECEIPT` | Intake scope triple | `INTAKE_SCOPE_PACKET` | intake-scope triple | yes | no |
| `OPERATOR_SUBMODE_SELECTION_RECEIPT` | Submode declaration | `SUBMODE_DECLARATION` | submode declaration | yes | no |
| `OPERATOR_ROLE_NODE_MODEL_ASSIGNMENT_APPROVAL_RECEIPT` | Baseline packet triple | `ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET` | baseline packet triple | yes | no |
| `GLOBAL_READINESS_RECEIPT` | Work Order triple | `WORK_ORDER_DECLARATION` | scope-packet triple | no | yes |
| `ROLE_ACTIVATION_READINESS_RECEIPT` | Invocation ref | `DRAFT_TO_READY_INVOCATION_REF` / `MERGE_INVOCATION_REF` / `BRANCH_DELETION_INVOCATION_REF` (per stage) | activation packet triple | no | yes |
| `EXPLORER_VALIDATION_RECEIPT` | Subject artifact | `EXPLORER_ARTIFACT` | subject artifact | yes | no |
| `PLAN_VALIDATION_RECEIPT` | Implementation plan | `IMPLEMENTATION_PLAN` | plan triple | yes | no |
| `ARTIFACT_VALIDATION_RECEIPT` | Subject artifact | `SUBJECT_ARTIFACT_TRIPLE` | artifact triple | yes | no |
| `PACKET_ASSEMBLY_RECEIPT` | Packet ref | `GENERIC_PACKET_REF` | packet triple | yes | no |
| `PACKET_FREEZE_RECEIPT` | Packet ref | `GENERIC_PACKET_REF` | packet triple | yes | no |
| `PACKET_VALIDATION_RECEIPT` | Packet ref | `GENERIC_PACKET_REF` | packet triple | yes | no |
| `PACKET_CONSUMPTION_RECEIPT` | Packet ref | `GENERIC_PACKET_REF` | packet triple | yes | no |
| `TEST_GATE_PASS_RECEIPT` / `TEST_GATE_FAIL_RECEIPT` | Test evidence packet | `TEST_EVIDENCE_PACKET` | test evidence packet triple | yes | no |
| `REVIEW_GATE_PASS_RECEIPT` / `REVIEW_GATE_FAIL_RECEIPT` | Review input packet | `REVIEW_INPUT_PACKET` | review input packet triple | yes | no |
| `GIT_INTEGRATION_GATE_PASS_RECEIPT` / `GIT_INTEGRATION_GATE_FAIL_RECEIPT` | Integration Packet | `INTEGRATION_INPUT_FROZEN` | integration packet triple | yes | no |
| `ROLE_CROSS_VERIFICATION_RECEIPT` | Cross-verification ref | `TESTER_A/B_CROSS_VERIFICATION_REF` / `REVIEWER_A/B_CROSS_VERIFICATION_REF` / `GIT_CROSS_VERIFICATION_REF` / `READY_CROSS_VERIFICATION_REF` / `MERGE_CROSS_VERIFICATION_REF` / `BD_CROSS_VERIFICATION_REF` (per role/stage) | role completion report | no | **yes (mandatory)** |
| `PRE_DRAFT_GIT_INTEGRATOR_BINDING_RECEIPT` | Role triple | `GIT_INTEGRATOR_ROLE_TRIPLE` | activation evidence | no | yes |
| `POST_DRAFT_DRAFT_TO_READY_BINDING_RECEIPT` | Approval packet | `DRAFT_TO_READY_APPROVAL_PACKET` | approval packet triple | yes | no |
| `POST_DRAFT_MERGE_BINDING_RECEIPT` | Approval packet | `MERGE_APPROVAL_PACKET` | approval packet triple | yes | no |
| `POST_DRAFT_BRANCH_DELETION_BINDING_RECEIPT` | Approval packet | `BRANCH_DELETION_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | Approval packet | `DRAFT_TO_READY_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | Approval packet | `MERGE_APPROVAL_PACKET` | approval packet triple | yes | no |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | Approval packet | `BRANCH_DELETION_APPROVAL_PACKET` | approval packet triple | yes | no |
| `DRAFT_PR_DELIVERY_RECEIPT` | PR triple | `PR_TRIPLE` | PR triple | yes | no |
| `READY_TRANSITION_RECEIPT` | PR triple | `PR_TRIPLE` | PR triple | yes | no |
| `MERGE_DELIVERY_RECEIPT` | PR triple | `PR_TRIPLE` | PR triple | yes | no |
| `BRANCH_DELETION_RECEIPT` | Branch triple | `BRANCH_TRIPLE` | branch triple | yes | no |
| `PUBLIC_PROJECTION_SAFETY_CHECK_RECEIPT` | Source artifacts | `SOURCE_EVIDENCE_OBJECTS` | subject artifact triple | no | yes |
| `CONSULTATION_ENDPOINT_RECEIPT` | Consultation source | `CONSULTATION_SOURCE_PACKET` | consultation-source triple | yes | no |
| `INVALIDATION_RECEIPT` | Subject artifact | `SUBJECT_ARTIFACT_TRIPLE` | artifact triple | no | yes |
| `SUPERSESSION_RECEIPT` | Superseded object | `SUPERSEDING_OBJECT_TRIPLE` | superseded-object triple | no | yes |
| `STOP_RECEIPT` | Stop report | `STOP_REPORT` | stop report triple | yes | no |
| `OPERATOR_REVOCATION_RECEIPT` | Authorisation triple | `AUTHORISATION_RECEIPT_TRIPLE` | authorisation triple | yes | no |
| `VERSION_*_RECEIPT` (governance series) | Per-version packet | `VERSION_GOVERNANCE_SCOPE_PACKET` / `VERSION_PREPARATION_PACKET` / `VERSION_QUALIFICATION_PACKET` / etc. | per-version triple | yes | no |

**Operator-binding rules (enforced by typed AST validator):**

1. `CURRENT(RECEIPT_TYPE, INSTANCE_REF)` and `STATE_CREATED(RECEIPT_TYPE, INSTANCE_REF, STATE)` and `DECISION(RECEIPT_TYPE, INSTANCE_REF, DECISION)` MUST use, as `INSTANCE_REF`, exactly the canonical instance key ref listed in this mapping for the given `RECEIPT_TYPE`.
2. The same Receipt Type may not use different instance-key classes in different rows (e.g., `MERGE_DELIVERY_RECEIPT` MUST use `PR_TRIPLE` in every Registry row; using `BRANCH_TRIPLE` is a typed validator error).
3. For Receipt Types marked `INSTANCE_SUBJECT_MATCH required = yes`, the Registry row MUST also reference `INSTANCE_SUBJECT_MATCH(RECEIPT_TYPE, INSTANCE_REF, SUBJECT_REF)` in the Prerequisite Expression when the row's subject differs from the instance key.
4. Cross-verification Receipts (`ROLE_CROSS_VERIFICATION_RECEIPT` per role/stage) MUST use their per-stage `*_CROSS_VERIFICATION_REF` as instance key AND MUST bind the corresponding completion report via `INSTANCE_SUBJECT_MATCH`.

#### §10.3.10.0.6 Canonical Operation Binding Mapping

Every ref that participates in `SAME_OPERATION` or `OPERATION_MATCH` MUST be classified by its `operation_binding_class`. The typed AST validator MUST reject any `SAME_OPERATION` argument or `OPERATION_MATCH` first argument whose class is not listed here as having `operation_binding = yes`.

| ref | class | operation_binding |
|---|---|---|
| `*_OPERATION_REF` (all 8 canonical: INTEGRATION / PACKET / DRAFT_TO_READY / MERGE / BRANCH_DELETION / VERSION / TEST / REVIEW) | operation_ref | yes (canonical operation ref) |
| `GENERIC_PACKET_REF` (only as Registry-row-bound typed variable) | packet_variable | yes (bound to current row operation) |
| `DRAFT_TO_READY_APPROVAL_PACKET`, `MERGE_APPROVAL_PACKET`, `BRANCH_DELETION_APPROVAL_PACKET`, `CONSULTATION_SOURCE_PACKET`, `VERSION_*_PACKET`, `INTAKE_SCOPE_PACKET`, `ROLE_NODE_MODEL_ASSIGNMENT_BASELINE_PACKET` | approval_packet | yes (each carries operation_id/version/digest) |
| `TESTER_A/B_INVOCATION_REF`, `REVIEWER_A/B_INVOCATION_REF`, `DRAFT_TO_READY_INVOCATION_REF`, `MERGE_INVOCATION_REF`, `BRANCH_DELETION_INVOCATION_REF` | invocation_ref | yes (each carries operation_id/version/digest) |
| `TESTER_A/B_CROSS_VERIFICATION_REF`, `REVIEWER_A/B_CROSS_VERIFICATION_REF`, `GIT_CROSS_VERIFICATION_REF`, `READY_CROSS_VERIFICATION_REF`, `MERGE_CROSS_VERIFICATION_REF`, `BD_CROSS_VERIFICATION_REF` | cross_verification_ref | yes (each carries operation_id/version/digest) |
| `READY_TRANSITION_COMPLETION_REPORT`, `MERGE_COMPLETION_REPORT`, `BRANCH_DELETION_COMPLETION_REPORT` | completion_report | yes (each carries operation_id/version/digest) |
| `*_ROLE_TRIPLE` (e.g., `GIT_INTEGRATOR_ROLE_TRIPLE`) | role_triple | yes (each carries operation_id/version/digest of the role activation) |
| `GIT_INTEGRATOR_ACTIVATION_EVIDENCE[_BD/_DRAFT_TO_READY/_MERGE]` | activation_evidence | yes (each carries operation_id/version/digest of its binding stage) |
| `PR_TRIPLE`, `BRANCH_TRIPLE`, `TEST_EVIDENCE_PACKET`, `REVIEW_INPUT_PACKET`, `INTEGRATION_INPUT_FROZEN`, `EXPLORER_ARTIFACT`, `IMPLEMENTATION_PLAN`, `*_PACKET` (above), `CANDIDATE_TRIPLE`, `SOURCE_EVIDENCE_OBJECTS`, `SUBJECT_ARTIFACT_TRIPLE`, `AUTHORISATION_RECEIPT_TRIPLE`, `SUBJECT_TRIPLE`, `SUPERSEDING_OBJECT_TRIPLE`, `WORK_ORDER_DECLARATION`, `SUBMODE_DECLARATION`, `STOP_REPORT`, `INTAKE_EVIDENCE_PACKET` | concrete_object / concrete_ref | **no** — these MUST NOT appear in `SAME_OPERATION` or `OPERATION_MATCH` as operation-bearers. |

`OPERATION_MATCH(REF, OPERATION_REF)` returns `TRUE` iff REF has `operation_binding = yes` and the `operation_id + version + digest` recorded by REF equals that of OPERATION_REF. If REF has `operation_binding = no`, or REF is missing, the predicate returns `FALSE`.

**SAME_OPERATION replacement policy (Round-80):**

`SAME_OPERATION(...)` may ONLY be invoked with arguments whose class is `operation_ref` (i.e., exactly one or more `*_OPERATION_REF` tokens). All other instance/packet/invocation/cross-verification/completion-report/role-triple/activation-evidence bindings MUST be expressed via `OPERATION_MATCH(REF, OPERATION_REF)`. The typed validator MUST count the non-operation-ref arguments in `SAME_OPERATION` and reject rows where the count is greater than 0.
...
| `OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT` | operator (only, FULL/LIGHTWEIGHT only — not Consultation) | operator authority |DRAFT_TO_READY_APPROVAL_PACKET triple|`DRAFT_TO_READY_APPROVAL_PACKET`, `DRAFT_TO_READY_OPERATION_REF`, `PR_TRIPLE`|`OPERATOR_WORK_ORDER_APPROVAL_RECEIPT`, `GIT_INTEGRATION_GATE_PASS_RECEIPT`| AND(CURRENT(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(OPERATOR_WORK_ORDER_APPROVAL_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, OPERATOR_APPROVED_WORK_ORDER), CURRENT(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN), STATE_CREATED(GIT_INTEGRATION_GATE_PASS_RECEIPT, INTEGRATION_INPUT_FROZEN, GATE_PASS), PACKET_BINDS(DRAFT_TO_READY_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(DRAFT_TO_READY_OPERATION_REF), SAME_OPERATION(DRAFT_TO_READY_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_DRAFT_TO_READY |
| `OPERATOR_MERGE_AUTHORIZATION_RECEIPT` | operator (only) | operator authority |MERGE_APPROVAL_PACKET triple|`MERGE_APPROVAL_PACKET`, `MERGE_OPERATION_REF`, `PR_TRIPLE`|`OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT`| AND(CURRENT(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET), STATE_CREATED(OPERATOR_DRAFT_TO_READY_AUTHORIZATION_RECEIPT, DRAFT_TO_READY_APPROVAL_PACKET, OPERATOR_AUTHORIZED_DRAFT_TO_READY), CURRENT(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, MERGE_APPROVAL_PACKET, PACKET_FROZEN), PACKET_BINDS(MERGE_APPROVAL_PACKET, PR_TRIPLE), ROW_OPERATION_IS(MERGE_OPERATION_REF), SAME_OPERATION(MERGE_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_MERGE |
| `OPERATOR_BRANCH_DELETION_AUTHORIZATION_RECEIPT` | operator (only) | operator authority |BRANCH_DELETION_APPROVAL_PACKET triple|`BRANCH_DELETION_APPROVAL_PACKET`, `BRANCH_DELETION_OPERATION_REF`, `BRANCH_TRIPLE`|`OPERATOR_MERGE_AUTHORIZATION_RECEIPT`| AND(CURRENT(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET), STATE_CREATED(OPERATOR_MERGE_AUTHORIZATION_RECEIPT, MERGE_APPROVAL_PACKET, OPERATOR_AUTHORIZED_MERGE), CURRENT(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET), STATE_CREATED(PACKET_FREEZE_RECEIPT, BRANCH_DELETION_APPROVAL_PACKET, PACKET_FROZEN), PACKET_BINDS(BRANCH_DELETION_APPROVAL_PACKET, BRANCH_TRIPLE), ROW_OPERATION_IS(BRANCH_DELETION_OPERATION_REF), SAME_OPERATION(BRANCH_DELETION_OPERATION_REF)) | OPERATOR_DECISION_RECORD | AUTHORIZED | OPERATOR_AUTHORIZED_BRANCH_DELETION |
