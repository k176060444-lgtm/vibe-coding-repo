#!/usr/bin/env python3
"""vibe_job_orchestrator.py — Durable Job Orchestrator v3.0.0
...
FAIL-CLOSED runtime closure:
  ...
  - ClaimStore with corruption latch, fsync, schema validation
  - Auto heartbeat daemon thread during RUNNING
  - SSH key: explicit Windows controller paths + registry only
  - Remote process control via setsid process groups + streaming PID
...
# ===========================================================================
# ClaimStore — FAIL-CLOSED with corruption latch
# ===========================================================================
...
class ClaimStore:
    """Persistent, file-locked claim store with corruption latch and fsync."""
    ...
    def _validate_store_or_latch(self):
        """Validate store schema. Latch on corruption."""
        try:
            data = self._raw_read()
            ...
            if not isinstance(data["claims"], dict):
                ...
                return
            # Version check (warn but don't block if missing)
            ver = data.get("version")
            if ver and ver != CLAIM_STORE_SCHEMA_VERSION:
                # Schema version mismatch — log but continue
                pass
    ...
    def _read_store(self) -> dict:
        """Read with latch check and validation."""
        self._check_latch()
        try:
            data = self._raw_read()
            ...
            for job_id in active:
                try:
                    ...
                    if not result.get("ok"):
                        # Heartbeat failed — mark as needing recovery
                        with self._lock:
                            self._active_jobs.pop(job_id, None)
                except Exception:
                    pass
...
class JobOrchestrator:
    ...
    def execute_job(self, job_id: str, timeout: int = 600) -> dict:
        """Execute a CLAIMED job with fail-closed preflight, heartbeat, and
        streaming remote PID capture."""
        ...
        # Remote runner: wrap command in setsid for process group isolation
        # Echo PGID (= PID of the new session leader) before exec
        inner_cmd = "cd %s && %s" % (
            _shell_quote(manifest.remote_job_dir),
            manifest.command,
        )
        # Setsid creates a new session; the shell PID = PGID
        remote_cmd = (
            "REMOTE_PID=$$; REMOTE_PGID=$$; "
            "echo REMOTE_PID=$REMOTE_PID; "
            "echo REMOTE_PGID=$REMOTE_PGID; "
            "exec %s" % inner_cmd
        )
        ...
        try:
            ...
            # Stream-read first two lines (REMOTE_PID, REMOTE_PGID)
            remote_pid = None
            remote_pgid = None
            stdout_lines = []
            try:
                for i in range(2):
                    ...
                    if line_str.startswith("REMOTE_PID="):
                        try:
                            remote_pid = int(line_str.split("=", 1)[1])
                        except (ValueError, IndexError):
                            pass
                    elif line_str.startswith("REMOTE_PGID="):
                        try:
                            remote_pgid = int(line_str.split("=", 1)[1])
                        except (ValueError, IndexError):
                            pass
            except Exception:
                pass
            ...
            if remote_pgid:
                ...
                self.claim_store.update_claim(job_id, {"remote_pgid": remote_pgid})
            self._persist_manifest(manifest)
            ...
            stdout_text = remaining_stdout.decode("utf-8", errors="replace")
            stderr_text = remaining_stderr.decode("utf-8", errors="replace")
...
def run_self_check() -> dict:
    """Comprehensive self-check for orchestrator v3.0.0."""
    import tempfile
