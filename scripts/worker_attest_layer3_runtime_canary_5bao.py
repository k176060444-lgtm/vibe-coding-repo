#!/usr/bin/env python3
"""
G-L3R — 5bao Sanctioned SSH Canary Receipt (Schema+Gate+SSH Validation).

Implements the 5bao remote SSH worker canary for G-L3R.
**Sanctioned SSH is the designated transport** for collecting runtime
evidence from 5bao. The SSH operation is concentrated in a single
function (_ssh_collect_5bao_evidence) and is ONLY allowed for node=5bao
with collector_mode=sanctioned_ssh_canary_5bao.

=== SCOPE ===
- Node: 5bao only (remote SSH worker).
- Transport: sanctioned SSH via _ssh_collect_5bao_evidence.
- Evidence sources: model_pool.yaml, node_model_capability.yaml,
  5bao fixture JSON, and SSH read-only commands on 5bao.
...
- collector_mode=sanctioned_ssh_canary_5bao is the sanctioned mode;
  dry_run uses fixture data only (no SSH).
- Rejects 21bao, 9bao, unknown nodes, and unauthorized collector modes.

=== GATES ===
Double-gate: operator_approval_id + node scope=5bao + collector_mode.
Authorized = operator_approval_id is non-empty AND node="5bao"
AND collector_mode in {sanctioned_ssh_canary_5bao, dry_run} ONLY.
Unauthorized → G_L3R_NOT_COLLECTED (advisory), no evidence collected.

=== SSH FUNCTION CONTRACT ===
_ssh_collect_5bao_evidence():
  - ONLY connects to 5bao (hardcoded target).
...
import argparse
import json
import subprocess  # sanctioned: used ONLY in _ssh_collect_5bao_evidence
import sys
...
# Sanctioned collector modes for 5bao SSH canary.
VALID_5BAO_COLLECTOR_MODES = frozenset({"sanctioned_ssh_canary_5bao", "dry_run"})
...
# ═══════════════════════════════════════════════════════════════════════════════
# Sanctioned SSH — single function, node-gated to 5bao only
# ═══════════════════════════════════════════════════════════════════════════════


def _ssh_collect_5bao_evidence(ssh_host: str = SSH_HOST) -> dict:
    """Run read-only SSH commands on 5bao to collect runtime evidence.

    This is the SINGLE sanctioned function that performs SSH operations.
    It is ONLY intended for node=5bao with collector_mode=sanctioned_ssh_canary_5bao.

    Commands (all read-only):
      - test -f /home/k/opencode/opencode.env -> env present
      - cat /home/k/.config/hermes/nodes/5bao/node_model_capability.yaml -> NMC
    ...
    try:
        ...
        nmc_cmd = ssh_common + [
            "cat", "/home/k/.config/hermes/nodes/5bao/node_model_capability.yaml",
        ]
...
def check_operator_gate(
    ...
    """Check the operator gate for the 5bao SSH canary.

    The gate passes only if:
      - operator_approval_id is a non-empty string
      - node is "5bao"
      - collector_mode is one of {sanctioned_ssh_canary_5bao, dry_run} ONLY.
    ...
    if not node or node not in VALID_NODES:
        ...
        errors.append(
            f"5bao SSH canary rejects node='{node}' "
    ...
    if not collector_mode:
        ...
        errors.append(
            f"5bao SSH canary rejects collector_mode='{collector_mode}'. "
            f"Valid modes for 5bao: {sorted(VALID_5BAO_COLLECTOR_MODES)}"
        )
...
def _make_anchor(model_id: str) -> str:
    ...
    return f"5bao-ssh-{prefix}"


# ═══════════════════════════════════════════════════════════════════════════════
# 5bao receipt collection
# ═══════════════════════════════════════════════════════════════════════════════


def collect_5bao_receipt_for_model(
    model_data: dict,
    nmc_entry: dict | None,
    ...
    """Build a single G-L3R 5bao SSH canary receipt for one model.

    Combines data from:
      - model_pool.yaml (declared config)
      - node_model_capability.yaml (declared capability)
      - 5bao fixture (alias metadata)
...
def collect_5bao_all_receipts(
    operator_approval_id: str,
    collector_mode: str = "sanctioned_ssh_canary_5bao",
    pool_path: Path = POOL_PATH,
    nmc_path: Path = NMC_PATH,
    fixture_dir: Path = FIXTURE_DIR,
    use_real_ssh: bool = False,
) -> dict:
    """Collect canary receipts for all models on 5bao.

    This is the main collection function. It:
    1. Checks operator gate (must pass)
    2. Optionally collects SSH runtime evidence from 5bao
    3. Reads local model_pool.yaml + node_model_capability.yaml
    4. Reads 5bao fixture alias data
    5. Produces one receipt per model

    When use_real_ssh=True and collector_mode=sanctioned_ssh_canary_5bao,
    _ssh_collect_5bao_evidence() is called to gather live runtime data.
    ...
    if collector_mode == "sanctioned_ssh_canary_5bao" and use_real_ssh:
        ssh_evidence = _ssh_collect_5bao_evidence()
    ...
    fixture: dict = {}
    try:
        fixture = _load_json(fixture_dir / "worker_attest_5bao.json")
    ...
    for model in pool.get("models", []):
        ...
        receipt = collect_5bao_receipt_for_model(
    ...
    scope_note = (
        "G-L3R 5bao sanctioned SSH canary. Evidence from repo YAML/fixture files "
        "and optional SSH read-only collection on 5bao. No model calls, no "
...
def self_check() -> dict:
    ...
    # sc-02: 5bao is the target node
    _ck("sc-02-target-node-5bao", TARGET_NODE == "5bao",
    ...
    g1 = check_operator_gate("", TARGET_NODE, "sanctioned_ssh_canary_5bao")
    _ck("sc-04-gate-rejects-empty-approval", not g1["passed"],
        f"reason={g1['reason']}")

    # sc-05: Gate rejects wrong node (21bao)
    g2 = check_operator_gate("op-001", "21bao", "sanctioned_ssh_canary_5bao")
    _ck("sc-05-gate-rejects-21bao", not g2["passed"],
        f"reason={g2['reason']}")

    # sc-06: Gate rejects 9bao
    g3 = check_operator_gate("op-001", "9bao", "sanctioned_ssh_canary_5bao")
    _ck("sc-06-gate-rejects-9bao", not g3["passed"],
        f"reason={g3['reason']}")

    # sc-07: Gate accepts 5bao with sanctioned mode
    g4 = check_operator_gate("op-001", TARGET_NODE, "sanctioned_ssh_canary_5bao")
    _ck("sc-07-gate-accepts-5bao-sanctioned", g4["passed"],
        f"status={g4['collection_status']}")

    # sc-08: Gate accepts dry_run on 5bao
    g5 = check_operator_gate("op-001", TARGET_NODE, "dry_run")
    _ck("sc-08-gate-accepts-5bao-dry-run", g5["passed"],
    ...
    # sc-10: Gate rejects ssh_canary (not sanctioned for 5bao)
    ...
    result = collect_5bao_all_receipts(
    ...
    result2 = collect_5bao_all_receipts(
    ...
    return {
        "schema_version": _l3rp.SCHEMA_VERSION,
        "name": "worker_attest_layer3_runtime_canary_5bao",
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="worker_attest_layer3_runtime_canary_5bao.py",
        description="G-L3R 5bao sanctioned SSH canary receipt.",
    )
    parser.add_argument(
        "cmd",
        nargs="?",
    ...
    parser.add_argument(
        "--collector-mode",
        default="sanctioned_ssh_canary_5bao",
        choices=["sanctioned_ssh_canary_5bao", "dry_run"],
        help=(
            "Collector mode (default: sanctioned_ssh_canary_5bao). "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable real SSH collection (requires 5bao reachable)",
    )
