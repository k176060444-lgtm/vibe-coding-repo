#!/usr/bin/env python3
"""Autonomous Merge Wrapper - Controlled merge execution via gate verification.

Usage:
    python scripts/vibe_autonomous_merge.py --repo <owner/repo> --pr <number> \
        --expected-base-sha <sha> --expected-head-sha <sha> \
        --allowed-path <path> [--allowed-path <path2> ...] \
        [--jobs-dir <dir>] [--job-id <id>] [--json] [--dry-run]

Constraints:
    - Merge only when gate returns allow_merge=true.
...
def run_gate_check(args):
    """Run gate checks (same logic as vibe_merge_gate.py)."""
    blockers = []
    warnings = []
    pr_info = {}
    job_info = {}
    ...
    if pr_data is None:
        ...
        return _build_gate_result(False, blockers, warnings, pr_info, job_info, checks_info)
    ...
    return _build_gate_result(allow_merge, blockers, warnings, pr_info, job_info, checks_info)


def _build_gate_result(allow_merge, blockers, warnings, pr_info, job_info, checks_info):
...
def execute_merge(repo, pr_number):
    """Execute merge via gh pr merge --merge."""
    stdout, stderr, rc = _run_cmd(
...
def run_wrapper(args):
    """Run the autonomous merge wrapper."""
    # Step 1: Run gate check
    gate_result = run_gate_check(args)

    # Step 2: Build result
    result = {
        "allow_merge": gate_result["allow_merge"],
        ...
        "pr": gate_result["pr"],
        "job": gate_result["job"],
        "changed_paths": gate_result["pr"].get("changed_paths", []),
        "checks": gate_result["checks"],
        "merge_command_summary": f"gh pr merge {args.pr} -R {args.repo} --merge",
    }

    # Step 3: If gate failed, stop
    if not gate_result["allow_merge"]:
        result["merge_executed"] = False
        return result

    # Step 4: If dry-run, stop
    if args.dry_run:
        result["merge_executed"] = False
        return result

    # Step 5: Execute merge
    success, stdout, stderr = execute_merge(args.repo, args.pr)
    result["merge_executed"] = success
    if not success:
        result["blockers"].append(f"Merge execution failed: {stderr}")
...
def _format_text(result):
    ...
    if result["merge_executed"]:
        lines.append("  Result: ✅ MERGE EXECUTED")
    elif result["allow_merge"]:
        lines.append("  Result: ✅ ALLOW MERGE (dry-run)")
    else:
    ...
    lines.append("----------------------------------------")
    lines.append(f"  Merge Command: {result.get('merge_command_summary')}")
    lines.append("========================================")
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="vibe_autonomous_merge",
        description="Autonomous Merge Wrapper - Controlled merge execution via gate verification.",
    )
    parser.add_argument(
        "--repo",
        required=True,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Only check gate, do not execute merge.",
    )
    return parser
...
def main(argv=None):
    ...
    return 0 if result["merge_executed"] or (result["allow_merge"] and args.dry_run) else 1
