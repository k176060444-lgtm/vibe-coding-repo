"""Tests for scripts/worker_attest_layer3_runtime_canary_9bao.py — G-L3R 9bao
...
- 9bao with sanctioned_ssh_canary_9bao accepted
- 9bao with dry_run accepted
...
from scripts import worker_attest_layer3_runtime_canary_9bao as l3rc9
from scripts import worker_attest_layer3_runtime_plan as l3rp

REPO = Path(__file__).resolve().parents[1]
SCRIPT = REPO / "scripts" / "worker_attest_layer3_runtime_canary_9bao.py"
...
class TestOperatorGate9Bao:
    def test_empty_approval_rejected(self):
        g = l3rc9.check_operator_gate("", "9bao", "sanctioned_ssh_canary_9bao")
        assert not g["passed"]
        assert g["collection_status"] == "not_collected"

    def test_21bao_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "21bao", "sanctioned_ssh_canary_9bao")
        assert not g["passed"]
        assert "21bao" in g["reason"].lower()

    def test_5bao_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "5bao", "sanctioned_ssh_canary_9bao")
        assert not g["passed"]
        assert "5bao" in g["reason"].lower()

    def test_9bao_accepted(self):
        g = l3rc9.check_operator_gate("op-001", "9bao", "sanctioned_ssh_canary_9bao")
        assert g["passed"]
        assert g["collection_status"] == "collected"

    def test_9bao_dry_run_accepted(self):
        g = l3rc9.check_operator_gate("op-001", "9bao", "dry_run")
        assert g["passed"]
        assert g["collection_status"] == "collected"

    def test_real_read_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "9bao", "real_read")
        assert not g["passed"]
        assert "real_read" in g["reason"].lower()

    def test_ssh_canary_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "9bao", "ssh_canary")
        assert not g["passed"]
        assert "ssh_canary" in g["reason"].lower()

    def test_invalid_node_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "invalid-node", "sanctioned_ssh_canary_9bao")
        assert not g["passed"]

    def test_invalid_mode_rejected(self):
        g = l3rc9.check_operator_gate("op-001", "9bao", "invalid-mode")
        assert not g["passed"]
...
class TestCollectionNotCollected9Bao:
    ...
    def test_all_18_fields_present(self):
        ...
        for receipt in result["receipts"]:
            for field in l3rc9.REQUIRED_RECEIPT_FIELDS:
                assert field in receipt, f"Missing field '{field}' in receipt for {receipt['model_id']}"
            assert len(receipt.keys() & set(l3rc9.REQUIRED_RECEIPT_FIELDS)) >= 18
...
class TestSshFunctionContract:
    def test_ssh_function_exists(self):
        assert hasattr(l3rc9, "_ssh_collect_9bao_evidence")
        assert callable(l3rc9._ssh_collect_9bao_evidence)

    def test_ssh_function_returns_dict(self):
        """SSH function returns a structured dict even when called
        (it will fail/timeout since no real 9bao, but should not crash)."""
        result = l3rc9._ssh_collect_9bao_evidence(ssh_host="nonexistent.local")
...
class TestNoForbiddenOps9Bao:
    ...
    def test_ssh_sanctioned_only(self):
        ...
        # Verify subprocess usage is ONLY in _ssh_collect_9bao_evidence
        ssh_func_found = False
        for i, line in enumerate(source.split("\n"), 1):
            if "def _ssh_collect_9bao_evidence" in line:
                ssh_func_found = True
...
class TestNoRuntimePromotion9Bao:
...
class TestReceiptFormat9Bao:
    def test_receipt_count_matches_non_other_models(self):
        ...
        assert result["receipt_count"] > 0
        # All receipts reference 9bao node
        for receipt in result["receipts"]:
            assert receipt["node"] == "9bao"
            assert receipt["source_node"] == "9bao"
    ...
    def test_receipt_anchor_is_deterministic(self):
        ...
        for receipt in result["receipts"]:
            assert receipt["receipt_anchor"].startswith("9bao-ssh-")
    ...
    def test_collector_mode_is_recorded(self):
        ...
        for receipt in result["receipts"]:
            assert receipt["collector_mode"] == "sanctioned_ssh_canary_9bao"
