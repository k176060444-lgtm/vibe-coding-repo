"""G-L4 D4 9bao Model-Call Canary Evidence Tests.

Verifies the canary receipt at .hermes/evidence/g-l4-d4-model-call-canary-9bao-v1/9bao-receipt.json
and docs/baseline02/g-l4-d4-model-call-canary-9bao.md:

1. Receipt schema valid with required fields
2. Canary performed on 9bao ONLY (no 21bao/5bao)
...
10. Distinct from PR #341 21bao and PR #342 5bao canaries
11. opencode-go/deepseek-v4-pro works directly (no deepseek-plan needed)
"""
...
RECEIPT_PATH = os.path.join(REPO_ROOT, ".hermes/evidence/g-l4-d4-model-call-canary-9bao-v1/9bao-receipt.json")
DOC_PATH = os.path.join(REPO_ROOT, "docs/baseline02/g-l4-d4-model-call-canary-9bao.md")

EXPECTED_ANCHOR = "7fc883b2e376307a704ec9b190c9af2cd7ef6674"
...
class TestSchema:
    ...
    def test_gate_id_correct(self):
        r = _load_receipt()
        assert r["gate_id"] == "G-L4-D4-MODEL-CALL-CANARY-9BAO"

    def test_canary_verdict_present(self):
        r = _load_receipt()
        assert r["canary_verdict"] == "G_L4_D4_9BAO_CANARY_PASS"


class TestNodeScope:
    def test_node_is_9bao(self):
        r = _load_receipt()
        assert r["ssh_transport"]["node"] == "9bao"
    ...
    def test_no_5bao_execution(self):
        r = _load_receipt()
        nf = r["forbidden_claims_NOT_made"]
        assert any("5bao" in c for c in nf), "5bao must be in forbidden claims"
...
class TestModelCall:
    ...
    def test_d4_model_identity(self):
        ...
        # On 9bao, opencode-go resolves directly (no deepseek-plan needed)
        assert r["model_call"]["concrete_invocation"] == "opencode-go/deepseek-v4-pro"
        assert r["model_call"]["concrete_provider"] == "opencode-go"

    def test_opencode_go_success_not_asymmetry(self):
        r = _load_receipt()
        note = r["model_call"]["opencode_go_provider_availability_note"]
        assert "WORKS directly" in note
        assert "NOT a fallback" in note
        assert "NOT a model switch" in note
...
class TestDistinctFromOtherCanaries:
    def test_21bao_canary_pr_referenced(self):
        r = _load_receipt()
        assert r["post_call_state"]["21bao_canary_pr"] == 341

    def test_5bao_canary_pr_referenced(self):
        r = _load_receipt()
        assert r["post_call_state"]["5bao_canary_pr"] == 342

    def test_distinct_note_present(self):
        r = _load_receipt()
        note = r["post_call_state"]["21bao_5bao_canary_distinct_note"]
        assert "separate execution" in note

    def test_9bao_verdict_distinct(self):
        r = _load_receipt()
        assert "9BAO" in r["canary_verdict"]
...
class TestDocStructure:
    ...
    def test_doc_references_341_and_342(self):
        doc = _load_doc()
        assert "PR #341" in doc
        assert "PR #342" in doc

    def test_doc_9bao_specific_marker(self):
        doc = _load_doc()
        assert "D4_CANARY_OK_9BAO" in doc
